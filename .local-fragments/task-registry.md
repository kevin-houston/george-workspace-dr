...
- **Recurred 2026-08-21 night, this time with zero log output at all** (vs. the 2026-08-20 incidents which at least reached the "Loading ProsusAI/finbert…" line): a single attempt run with `timeout 600` produced no stdout/stderr whatsoever before being killed (exit code 124) — the script never even printed its startup banner. `uptime` at diagnosis time showed `load average: 21.43, 19.50, 11.65` and `free -h` showed 759Mi free RAM with 3.4Gi/8Gi swap in use — same host-contention signature as the 2026-08-20 incidents, just severe enough this time to stall before any Python output flushed. Did not retry, per the existing mitigation above — `pead_watchlist.json` stayed stale at its 2026-08-21 00:22 mtime (last night's real run), so `pead_open.py` this morning (2026-08-22) will correctly see a non-matching date and no-op. No message sent to Kevin (not a strategy-affecting error, just a missed nightly scan — consistent with prior incidents where this was logged, not escalated).
