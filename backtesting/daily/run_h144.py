"""
H144 — H041a Vol-Target Level Refinement Post-Threshold
=======================================================

H041a vol-target was set at 25% in H133 on the original 0% TSMOM threshold
system. H140 raised the threshold to +0.5%, filtering out barely-breakeven
global equity ETFs. The composition change is mild (only ~5 cash months added),
but worth verifying the vol-target level is still optimal.

H041a global equity ETFs (QQQ, SPY, EFA, EEM, etc.) typically carry 15-25%
annualized vol. At 25% target:
  - When realized vol = 25%, scalar = 1.0x (full weight)
  - When realized vol = 12.5%, scalar = 2.0x (max)
  - When realized vol = 50%, scalar = 0.5x (min, crash protection)

Testing whether 30% or 35% provides better returns (less scale-down during
normal market vol), or whether lower targets (20%) reduce risk more efficiently.

Baseline: H143 system (H026 vol-target=20%, all H139-H142 upgrades)

Reference: H143 baseline OOS 31.0072, AltOOS 113.0695, Sharpe 4.932
"""
...
VOL_TARGET_H026 = 0.20   # H143: raised from 15% to 20%
VOL_WINDOW      = 6
VOL_CLAMP       = (0.5, 2.0)
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 145)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h144_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h144_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def blend_vol_targeted(sub_rets, vol_target_h041a):
...
def main():
    print("=" * 80)
    print("H144 — H041a Vol-Target Level Refinement Post-Threshold")
    print("=" * 80)
    print("H143 baseline: OOS 31.0072, AltOOS 113.0695, Sharpe 4.932\n")
    ...
    print("\nPre-computing H026 (+5% threshold, 20% vol-target), H041a (+0.5%), H045 (top-1, +1%) …")
    ...
    VARIANTS = [
        ("A) H041a vol-target=25% (current)", 0.25),
        ("B) H041a vol-target=20%",           0.20),
        ("C) H041a vol-target=30%",           0.30),
        ("D) H041a vol-target=35%",           0.35),
        ("E) H041a vol-target=40%",           0.40),
    ]
    ...
    for label, vt in VARIANTS:
        port = blend_vol_targeted(sub_rets_base, vol_target_h041a=vt)
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H143 baseline (A = 25% vol-target):\n")
    confirmed_any = False
    for label, vt in VARIANTS:
        if label.startswith("A)"):
            continue
    ...
    if confirmed_any:
        ...
        print(f"\n  VERDICT: NOT CONFIRMED — 25% vol-target remains optimal for H041a")
    print()
