...
Purpose:
  H026 current: 11-sector + BIL + GLD + TLT (14-asset).
  Safe havens already covered: cash (BIL), gold (GLD), long bonds (TLT).
  Missing macro asset classes:
    IEF  — 7-10yr Treasury (intermediate duration; already in H041a, not H026)
    TIP  — iShares TIPS (inflation-protected bonds; starts 2003)
    AGG  — iShares Core US Aggregate Bond (total bond market; starts 2003)
    SLV  — iShares Silver Trust (industrial commodity; different cycle from GLD; starts 2006)
    MDY  — SPDR S&P MidCap 400 (mid-cap equity; starts 1995; different momentum cycle from sectors)

  H026_GLD_TLT = 11-sector+BIL+GLD+TLT (14-asset)
...
H026_BASE    = ["XLK","XLE","XLF","XLV","XLI","XLB","XLU","XLRE","XLY","XLP","XLC","BIL","GLD","TLT"]
...
CANDIDATES = [
    ...
    ("+IEF+SLV",     ["IEF","SLV"]),
    ("+TIP+SLV",     ["TIP","SLV"]),
    ("+GLD+IEF",     ["IEF"]),   # redundant since GLD already in base; skip
    ("+IEF+TIP+SLV", ["IEF","TIP","SLV"]),
    ("+MDY+IEF",     ["MDY","IEF"]),
]
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 102)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h102_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h102_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H102 — H026 Universe Expansion: IEF, TIP, AGG, SLV, MDY")
print("="*80)
...
print("\n[1] Building H026 baseline (14-asset: 11-sector+BIL+GLD+TLT) …")
h026_base = build_rotation_monthly(H026_BASE, FULL_START, FULL_END, 1)
cidx_b    = common_idx(h41, h026_base, h045, xlk_r, smh_r, igv_r)
rd_b      = {"h041a":h41,"h026":h026_base,"h045":h045,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}
...
# ── [2] Candidate sweep ──────────────────────────────────────────────────────
print("\n[2] H026 candidate sweep …")
print(f"\n  {'Candidate':>16}  {'H026 IS':>8}  {'H026 OOS':>9}  {'H026 AltOOS':>12}  "
      f"{'Port OOS':>9}  {'Port AltOOS':>11}  {'MaxDD':>7}  {'WF':>7}  {'Both↑':>5}")
print("  "+"-"*103)

results = {}
for label, extra in CANDIDATES:
    universe = H026_BASE + extra
    h026_v   = build_rotation_monthly(universe, FULL_START, FULL_END, 1)
    cidx_v   = common_idx(h41, h026_v, h045, xlk_r, smh_r, igv_r)
    rd_v     = {"h041a":h41,"h026":h026_v,"h045":h045,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}
    s_is     = stats(h026_v[is_mask(h026_v.index)])
    s_oos    = stats(h026_v[oos_mask(h026_v.index)])
    s_ao     = stats(h026_v[ao_mask(h026_v.index)])
    ...
    print(f"  {label:>16}  {s_is['sharpe']:>8.4f}  {s_oos['sharpe']:>9.4f}  "
    ...
    results[label] = {
        ...
        "h026_is": s_is["sharpe"], "h026_oos": s_oos["sharpe"], "h026_ao": s_ao["sharpe"],
        ...
        "h026_v": h026_v, "cidx_v": cidx_v, "rd_v": rd_v,
    }
...
if winners:
    ...
    for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
        ("H099 baseline (H026 14-asset)", sb_is, sb_oos, sb_ai, sb_ao, wf_b),
        (f"H102 {best_label}",            s_is,  s_oos,  s_ai,  s_ao,  wf)
    ]:
    ...
    print(f"  {'Year':>5}  {'H099 baseline':>14}  {'H102 best':>10}  {'Delta':>7}")
    print("  "+"-"*52)
    neg_b = neg_n = 0
    for yr in range(2004, 2026):
        ...
        cal.append({"year":yr,"baseline":round(rb,4),"h102":round(rn,4)})
    print(f"  Baseline: {'ZERO' if neg_b==0 else neg_b} neg | H102: {'ZERO' if neg_n==0 else neg_n} neg")
    ...
    if confirmed:
        print(f"\n  *** H102 CONFIRMED — H026 expanded with {best_label} ***")
    else:
        print(f"\n  H102 not confirmed — H026 (14-asset) unchanged.")
else:
    print("\n[4] Summary …")
    print("  No candidate passed dual-window criterion.")
    print("  H102 not confirmed — H026 (14-asset) unchanged.")

output = {
    "sweep": {k: {kk: vv for kk, vv in v.items()
                  if kk not in ("h026_v","cidx_v","rd_v")}
              for k, v in results.items()},
    "winner": best_label,
    "confirmed": bool(confirmed),
    "calendar": cal,
}
out_path = RESULT_DIR / "h102_results.json"
