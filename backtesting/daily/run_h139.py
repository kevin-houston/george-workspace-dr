"""
H139 — H026 TSMOM Threshold Sweep
==================================

Tests whether the H026 12m TSMOM filter threshold (currently 12m_return > 0%)
is optimal, or whether a tighter/looser threshold improves both OOS windows.

Current production: hold H026 asset only if its 12m return > 0%.

Motivation:
  TIGHTER (+5%, +10%): Filter out "borderline positive" sectors that are barely
  trending up but may be in early stages of reversal. Requires confirmed momentum.
  Risk: over-constrains the universe, reduces diversification.

  LOOSER (-2%, -5%): Allow sectors slightly below the 0% line — could capture
  early recovery rotations when a sector has been recovering recently but hasn't
  recouped all 12m losses. Risk: stays in deteriorating sectors too long.

Variants:
  A) 0%   (current production, H134 baseline)
  B) -5%  (very loose — allow up to -5% 12m return)
  C) -2%  (slightly loose)
  D) +2%  (slightly tight)
  E) +5%  (tight — require meaningful positive momentum)
  F) +10% (very tight — strong trend confirmation required)
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 140)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h139_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h139_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def build_rotation_tsmom(tickers, start, end, n_hold=1,
    ...
    """
    tsmom_threshold: minimum return required to pass the TSMOM filter.
    Default 0.0 = positive return required (current production behavior).
    """
    closes = {}
    for t in tickers:
        try:
...
def main():
    print("=" * 80)
    print("H139 — H026 TSMOM Threshold Sweep")
    print("=" * 80)
    ...
    print("\nPre-computing H041a (3m filter, threshold=0%) and H045 (3m filter) …")
    h041a = build_rotation_tsmom(H041A_ASSETS, FULL_START, FULL_END,
                                  n_hold=1, tsmom_lookbacks=[3], tsmom_threshold=0.0)
    h045  = build_rotation_tsmom(H045_ASSETS,  FULL_START, FULL_END,
                                  n_hold=2, tsmom_lookbacks=[3], tsmom_threshold=0.0)
    print("  done")

    VARIANTS = [
        ("A) threshold=0.0% (baseline)",  0.00),
        ("B) threshold=-5%  (very loose)", -0.05),
        ("C) threshold=-2%  (loose)",      -0.02),
        ("D) threshold=+2%  (tight)",       0.02),
        ("E) threshold=+5%  (tighter)",     0.05),
        ("F) threshold=+10% (very tight)",  0.10),
    ]

    print("\nBuilding H026 variants (12m TSMOM with varied threshold) …")
    h026_series = {}
    for label, thresh in VARIANTS:
        tag = label[:2]
        print(f"  {label} …", end=" ", flush=True)
        h026_series[tag] = build_rotation_tsmom(
            H026_ASSETS, FULL_START, FULL_END,
            n_hold=1, tsmom_lookbacks=[12], tsmom_threshold=thresh
        )
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H134 baseline (A = 0% threshold):\n")
    confirmed_any = False
    for label, thresh in VARIANTS:
        if label.startswith("A)"):
            continue
    ...
    if confirmed_any:
        ...
        print(f"\n  VERDICT: NOT CONFIRMED — 0% threshold is optimal for H026 TSMOM filter")
