#!/usr/bin/env python3
"""
H527 — Amihud Illiquidity Premium as PRIMARY Signal on H241's 200-Stock Universe

Motivation: Amihud (2002) ILLIQ = mean(|daily_ret| / dollar_volume) is a classic
liquidity-premium proxy: less-liquid stocks command a return premium as
compensation for price-impact/execution risk. H393 tested ILLIQ only as a
SECONDARY tiebreaker layered on top of an already-strong momentum composite
(H386, 30-stock ultra-liquid mega-cap universe) and found it purely dilutive —
its own write-up attributed the failure explicitly to insufficient
cross-sectional ILLIQ variation in a 30-name mega-cap universe, and
recommended re-testing on a broader universe with genuine liquidity spread
("Russell 1000 ex-S&P 500 top 100" or similar) as PRIMARY signal, not tiebreaker.

H527 does exactly that: standalone ILLIQ as the PRIMARY cross-sectional
ranking signal (not a composite weight), on H241's 200-stock broad-sector
universe, which spans small/mid float-adjusted names (e.g. HRL, CPB, JNPR,
ZBH) alongside mega-caps (AAPL, MSFT) — genuine liquidity dispersion.

Universe: H241's 195-stock universe (11 GICS sectors).
Signal: Amihud ILLIQ, trailing 21-trading-day mean(|daily_ret|/dollar_vol),
  computed strictly using data through month-end t-1, applied to month t+1
  holding period (one full month of separation between signal date and the
  return being credited — see build_panel() for the explicit index arithmetic).
IS: 2013-01-01 to 2020-12-31 / OOS: 2021-01-01 to 2026-XX (present).
Gate: OOS Sharpe > 1.174 (H198/H241-family baseline used throughout this
  universe's prior hypotheses, e.g. H526) AND walk-forward worst-fold ratio
  (OOS/IS) > ... not applicable for a single-signal non-ML test; instead we
  apply the standard WF_WORST_MIN = 1.75 style sanity check via a 3-fold
  OOS split (see wf_worst_fold_ratio()).

Variants:
  A — pure ILLIQ top-20 (illiquidity premium: long the LEAST liquid names)
  B — pure ILLIQ top-20, inverse (long the MOST liquid names — sanity check,
      should underperform if illiquidity premium is real)
  C — ILLIQ x momentum dual rank (0.5 rank(illiq) + 0.5 rank(mom_6_1))
  D — momentum top-40 -> filter to most-illiquid 20 (liquidity-tilted momentum)
  E — baseline: plain 6-1m momentum top-20 (H241-A style, no illiquidity)
"""
...
DATA_START = "2011-06-01"   # need ILLIQ trailing window + mom lookback before IS start
DATA_END   = "2026-08-20"
...
TC      = 0.001
GATE    = 1.174
ILLIQ_WIN = 21  # trailing trading days for Amihud ILLIQ estimate


def load_daily():
    cache = CACHE_DIR / "h527_daily_close_volume.parquet"
    if cache.exists():
    ...
    # Retry loop to avoid transient yfinance sqlite-cache 'database is locked' errors
    ...
    # merge frames, preferring later (more complete) attempts for each ticker
    df = frames[0]
    for f in frames[1:]:
        df = df.combine_first(f)
        # combine_first can reintroduce NaN-preference issues; explicitly overlay non-null
...
def build_monthly_signals(daily: pd.DataFrame):
    ...
    # daily Amihud illiquidity contribution: |ret| / dollar_vol (scaled 1e6 for numeric sanity)
    illiq_daily = (ret.abs() / dollar_vol.replace(0, np.nan)) * 1e6
    illiq_21d = illiq_daily.rolling(ILLIQ_WIN).mean()

    # resample to month-end: illiq signal + prices for momentum + fwd return
    month_end_close = close.resample("ME").last()
    month_end_illiq = illiq_21d.resample("ME").last()
    monthly_ret = month_end_close.pct_change()

    return month_end_close, month_end_illiq, monthly_ret


def build_panel(month_end_close, month_end_illiq, monthly_ret):
    dates = month_end_close.index
    rows = []
    for i in range(13, len(dates) - 1):
        date = dates[i]
        # signal formation strictly at month-end t-1 (one full month before the
        # holding period begins) -> holding period return is month t -> t+1
        illiq_signal = month_end_illiq.iloc[i - 1]
        mom_6_1 = month_end_close.iloc[i - 1] / month_end_close.iloc[i - 7] - 1
        fwd_ret = monthly_ret.iloc[i + 1]  # return earned over month t -> t+1

        tickers = month_end_close.columns[
            month_end_close.iloc[i - 1].notna()
            & month_end_illiq.iloc[i - 1].notna()
            & fwd_ret.notna()
        ]
        for t in tickers:
            rows.append({
                "date": date, "ticker": t,
                "illiq": illiq_signal[t],
                "mom_6_1": mom_6_1[t],
                "fwd_ret": fwd_ret[t],
            })
    df = pd.DataFrame(rows).dropna(subset=["illiq", "mom_6_1", "fwd_ret"])
...
def run_backtest(panel: pd.DataFrame):
    ...
    for date in dates:
        ...
        illiq_rank = df_t["illiq"].rank(pct=True)
        mom_rank = df_t["mom_6_1"].rank(pct=True)

        picks = {
            "A": df_t.loc[illiq_rank.nlargest(TOP_N).index],       # most illiquid
            "B": df_t.loc[illiq_rank.nsmallest(TOP_N).index],      # most liquid (sanity)
            "E": df_t.loc[mom_rank.nlargest(TOP_N).index],         # baseline momentum
        }

        composite = 0.5 * mom_rank + 0.5 * illiq_rank
        picks["C"] = df_t.loc[composite.nlargest(TOP_N).index]

        top_mom40 = mom_rank.nlargest(min(40, len(df_t))).index
        sub_illiq = illiq_rank.loc[top_mom40].nlargest(TOP_N).index
        picks["D"] = df_t.loc[sub_illiq]
...
def wf_worst_fold(s: pd.Series):
    """3-fold OOS split, worst-fold Sharpe / IS Sharpe ratio (walk-forward sanity check)."""
...
def main():
    print(f"=== {STRATEGY} — Amihud Illiquidity Premium (Primary Signal) on H241 Universe ===")
    print(f"IS: {IS_START.date()}-{IS_END.date()} | OOS: {OOS_START.date()}-present")
    print(f"Gate: OOS Sharpe > {GATE} (H198/H241/H526-family baseline)")
    print()

    daily = load_daily()
    month_end_close, month_end_illiq, monthly_ret = build_monthly_signals(daily)
    panel = build_panel(month_end_close, month_end_illiq, monthly_ret)
    ...
    out_path = RESULT_DIR / "h527_results.json"
