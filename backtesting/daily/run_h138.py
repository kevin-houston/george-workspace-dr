"""
H138 — H041a Top-N Retest on Current System
============================================

Tests whether H041a should hold top-2 or top-3 assets instead of top-1.

This was last tested in H083 with only 9-10 assets and no TSMOM filter.
The current system is substantially different:
  - 19-asset universe (added EWG, EWQ, EWU, EWD, EWN, EWY, EWS, EWT, EWH, EPHE)
  - 3m TSMOM filter (H130 confirmed): only eligible if 3m return > 0
  - Full 4-term ensemble scoring: rank(3m)+rank(6m)+rank(12m)+rank(inv_vol)
  - Vol-targeting at 25% (H133 confirmed)

With the 3m TSMOM filter active, the typical filtered pool is ~12-15 assets
(roughly 4-7 excluded each month). Selecting top-2 from this filtered, high-
quality pool is meaningfully different from the unfiltered top-2 that failed in
H083 — both picks now must have confirmed uptrends AND strong ensemble scores.

Variants:
  A) top-1 (current production, should match H134 baseline)
  B) top-2 (equal-weight across 2 top-ranked filtered assets)
  C) top-3 (equal-weight across 3 top-ranked filtered assets)
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 139)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h138_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h138_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def main():
    print("=" * 80)
    print("H138 — H041a Top-N Retest on Current System")
    print("=" * 80)
    ...
    for label, n in VARIANTS:
        ...
        h041a_series[tag] = build_rotation_tsmom(
            H041A_ASSETS, FULL_START, FULL_END, n_hold=n, tsmom_lookbacks=[3]
        )
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H134 baseline (A = top-1):\n")
    confirmed_any = False
    ...
    if confirmed_any:
        ...
        print(f"\n  VERDICT: NOT CONFIRMED — top-1 remains optimal for H041a")
