"""
H084 — Full Cross-Validation: H026 top-1 & H041a+EWH
=====================================================

Purpose:
  H083 found two dual-window improvements over H082 (OOS 3.2018):

  A) H026 top-1 (12-asset, hold 1): Port OOS +0.048, AltOOS +0.051, WF 2.087 ✓
  B) H041a+EWH (10-asset, top-1):  Port OOS +0.055, AltOOS +0.056, WF 2.137 ✓

  EWH (iShares MSCI Hong Kong) adds Asian equity diversification — HK/China equity
  has distinct cycles vs US (China property cycle, tech regulation, etc.) and vs
  EWJ (Japan). H041a's top-1 signal can select HK/China in their bull markets.

  H026 top-1 continues the concentration pattern: H041a top-1 (+0.093), H026 top-1
  (+0.048), both benefit from selecting the single highest-scoring asset/sector.

  Full cross-validation:
    [1] Scorecard — incremental breakdown
    [2] Calendar year 2004-2025
    [3] WF 5-fold detail
    [4] H041a standalone: 9-asset top-1 vs 10-asset+EWH top-1
    [5] H026 standalone: 12-asset top-2 vs top-1
    [6] Further geographic sweep on 10-asset H041a (EWS, EWT, EWA)
...
H041A_EWH = ["SPY","QQQ","TLT","GLD","IEF","EFA","EEM","BIL","EWJ","EWH"]
H026_BIL  = ["XLK","XLE","XLF","XLV","XLI","XLB","XLU","XLRE","XLY","XLP","XLC","BIL"]
H045_BIL  = ["SHY","IEI","IEF","TLT","TIP","HYG","LQD","BKLN","EMB","BIL"]

# Additional Asian geographies for extended sweep in section [6]
ASIA_EXTRA = {
    "+EWS": H041A_EWH + ["EWS"],   # Singapore
    "+EWT": H041A_EWH + ["EWT"],   # Taiwan
    "+EWA": H041A_EWH + ["EWA"],   # Australia
}
    ...
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h084_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h084_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H084 — Full Cross-Validation: H026 top-1 & H041a+EWH")
print("="*80)
...
h41_base  = build_rotation_monthly(H041A_9,   FULL_START, FULL_END, 1)
h41_ewh   = build_rotation_monthly(H041A_EWH, FULL_START, FULL_END, 1)
h026_top2 = build_rotation_monthly(H026_BIL,  FULL_START, FULL_END, 2)
h026_top1 = build_rotation_monthly(H026_BIL,  FULL_START, FULL_END, 1)

# H082 baseline
cidx_h082 = common_idx(h045_bil, h026_top2, xlk_r, smh_r, igv_r, h41_base)
r_h082    = {"h041a":h41_base,"h026":h026_top2,"h045":h045_bil,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}

# ── [1] Incremental scorecard ─────────────────────────────────────────────────
print("\n[1] Incremental scorecard …")

portfolios = {
    "H082 baseline":      (h41_base, h026_top2, "9-asset t1 / top-2"),
    "A only (H026 top1)": (h41_base, h026_top1, "9-asset t1 / top-1"),
    "B only (EWH)":       (h41_ewh,  h026_top2, "EWH-10 t1 / top-2"),
    "H084 (A+B)":         (h41_ewh,  h026_top1, "EWH-10 t1 / top-1"),
}
...
for name, (h41, h26, desc) in portfolios.items():
    cidx = common_idx(h045_bil, h26, xlk_r, smh_r, igv_r, h41)
    rd   = {"h041a":h41,"h026":h26,"h045":h045_bil,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}
...
base_d = scorecard["H082 baseline"]
new_d  = scorecard["H084 (A+B)"]
print(f"\n  {'Year':>5}  {'H082 base':>10}  {'H084 (A+B)':>10}  {'Delta':>7}")
print("  "+"-"*44)
neg_base = neg_new = 0
cal = []
for yr in range(2004, 2026):
    ...
    cal.append({"year":yr,"h082":round(rb,4),"h084":round(rn,4)})
print(f"  H082 baseline: {'ZERO' if neg_base==0 else neg_base} negative years")
print(f"  H084 (A+B):    {'ZERO' if neg_new==0 else neg_new} negative years")
...
for label, h41 in [("9-asset top-1", h41_base), ("10-asset+EWH top-1", h41_ewh)]:
    idx  = h41.index
...
for label, h26 in [("12-asset top-2", h026_top2), ("12-asset top-1", h026_top1)]:
    idx  = h26.index
...
# ── [6] Extended Asian geographic sweep on H041a+EWH ─────────────────────────
print("\n[6] Extended Asian geographic sweep on 10-asset+EWH (top-1) …")
print(f"\n  {'Candidate':12}  {'H041a OOS':>9}  {'H041a AltOOS':>12}  "
      f"{'Port OOS':>9}  {'Port AltOOS':>11}  {'MaxDD':>7}  {'WF':>7}  {'Both↑':>5}")
print("  "+"-"*95)

h41_ewh_oos = stats(h41_ewh[oos_mask(h41_ewh.index)])["sharpe"]
h41_ewh_ao  = stats(h41_ewh[ao_mask(h41_ewh.index)])["sharpe"]
new_base    = scorecard["H084 (A+B)"]
s_new_oos   = new_base["oos"]["sharpe"]
s_new_ao    = new_base["ao"]["sharpe"]
print(f"  {'EWH base':12}  {h41_ewh_oos:>9.4f}  {h41_ewh_ao:>12.4f}  "
      f"{s_new_oos:>9.4f}  {s_new_ao:>11.4f}  "
      f"{new_base['oos']['max_drawdown']*100:>6.2f}%  {new_base['wf_worst']:>7.3f}  —")

sec6_results = {}
for name, tickers in ASIA_EXTRA.items():
    try:
...
# ── Summary ───────────────────────────────────────────────────────────────────
print("\n[7] Summary …")
b = scorecard["H082 baseline"]
h = scorecard["H084 (A+B)"]
...
if (h["oos"]["sharpe"] > b["oos"]["sharpe"] and
    ...
    h["wf_ok"]):
    print(f"\n  *** H084 CONFIRMED on BOTH OOS WINDOWS ***")
    print(f"  *** H041a: 10-asset+EWH top-1 ***")
    print(f"  *** H026: 12-asset top-1 ***")
else:
    print(f"\n  H084 not confirmed")
...
output = {
    ...
    "calendar": cal,
    "sec6_asian_sweep": sec6_results,
}
out_path = RESULT_DIR / "h084_results.json"
