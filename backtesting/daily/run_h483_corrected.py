"""
H483 CORRECTED (H506 follow-up, 2026-08-13) — h411_signal now shift(1)'d before
use, and the OB "as of" date uses the prior month-end, fixing the look-ahead
bias identified in H506. Despite the original file's own docstring/comments
implying care was taken, direct code inspection (H506) found no actual
.shift(1) call in backtest_ob() -- confirmed bug, not just a flag.
...
def backtest_ob(monthly_px, h411_signal, daily_data, ob_window, min_filter, swing_len,
    ...
    # H506 fix: h411_signal must be shift(1)'d (month M's own close-based value/
    # drift signal is not knowable at the start of month M), and the OB "as of"
    # date must be the PRIOR month-end for the same reason.
    monthly_ret = monthly_px.pct_change()
    h411_signal_shifted = h411_signal.shift(1)
    idx = monthly_ret.index[monthly_ret.index >= IS_START]
    port_rets = []
    for month_end in idx:
        loc_all = monthly_ret.index.get_loc(month_end)
        if loc_all == 0:
            continue
        signal_asof = monthly_ret.index[loc_all - 1]  # prior month-end
        scores = h411_signal_shifted.loc[month_end].dropna()
...
def main():
    ...
    # Baseline replication (sanity check vs H411 log ORIGINAL uncorrected number,
    # ~4.825 -- intentionally left unshifted here for comparability; the true
    # corrected H411 Var B baseline is OOS Sharpe 0.900, see H506/H411 log entry)
    ...
    outpath = RESULT_DIR / "h483_corrected_results.json"
