"""
H108 — Weight Re-optimization Post-H107
========================================

Purpose:
  H107 expanded H026 to 21-asset (+GDX+DBA+SLV), gaining +0.130/+0.065 OOS/AltOOS.
  Current weights (22%/27%/21%) were set with 18-asset H026; the richer signal
  may justify a different allocation.

  H105 showed H026 weight jumped 18%→27% after the DBC+AGG addition. Another
  large jump possible now that the full commodity complex is assembled.

  Sweep:
    Phase 1 — H026 sweep (H041a=22% fixed): 15% to 35% in 1pp steps
    Phase 2 — H041a sweep (H026=best from P1): 15% to 35% in 1pp steps

  H026_FULL (21-asset): 11-sector+BIL+GLD+TLT+IEF+TIP+DBC+AGG+GDX+DBA+SLV
  Baseline: H107 (OOS 4.0717, AltOOS 3.9901, WF 3.020)
  Current weights: H041a 22% / H026 27% / H045 21%
"""
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 108)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h108_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h108_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H108 — Weight Re-optimization Post-H107")
print("="*80)
...
print(f"  Baseline (22%/27%/21%): OOS {s_curr_oos['sharpe']:.4f}, AltOOS {s_curr_ao['sharpe']:.4f}")

# ── Phase 1: H026 sweep (H041a=22%) ─────────────────────────────────────────
print("\n[1] Phase 1 — H026 sweep (H041a=22% fixed) …")
print(f"  {'H026':>7}  {'H045':>6}  {'Port OOS':>9}  {'Port AltOOS':>11}  {'Sum':>8}  {'MaxDD':>7}")
print("  "+"-"*62)

p1_results = []
for pct in range(15, 36):
    w = make_weights(0.22, pct/100)
    if w is None:
        continue
    ...
    p1_results.append({
        "h041a": 0.22, "h026": pct/100, "h045": w["h045"],
...
for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
    ("H107 (22%/27%/21%)", c_is, s_curr_oos, c_ai, s_curr_ao, wf_curr),
    (f"H108 ({new_h041a_pct}%/{new_h026_pct}%/{new_h045_pct}%)", s_is, s_oos, s_ai, s_ao, wf),
]:
...
print(f"  {'Year':>5}  {'H107 (22%/27%/21%)':>19}  {'H108 best':>10}  {'Delta':>7}")
print("  "+"-"*58)
cal = []
neg_b = neg_n = 0
for yr in range(2004, 2026):
    ...
    cal.append({"year":yr,"h107_weights":round(rb,4),"h108_weights":round(rn,4)})
print(f"  H107: {'ZERO' if neg_b==0 else neg_b} neg | H108: {'ZERO' if neg_n==0 else neg_n} neg")
...
if confirmed:
    print(f"\n  *** H108 CONFIRMED — new weights {new_h041a_pct}%/{new_h026_pct}%/{new_h045_pct}% ***")
else:
    print(f"\n  H108 not confirmed — weights 22%/27%/21% unchanged.")
...
out_path = RESULT_DIR / "h108_results.json"
