"""
H086 — Full Cross-Validation: H041a+EWY (South Korea)
======================================================

Purpose:
  H085's extended Pacific Rim sweep found EWY (iShares MSCI South Korea) adds
  dual-window improvement on top of the H085 baseline (OOS 3.3548, AltOOS 3.2178):

    +EWY: Port OOS 3.3958 (+0.0410), AltOOS 3.2800 (+0.0622), WF 2.318 ✓

  EWY adds South Korea's semiconductor/memory cycle (Samsung ~25%, SK Hynix ~8%).
  Korea's DRAM/memory cycle is semi-correlated but distinct from Taiwan's logic/foundry
  cycle (TSMC). The top-1 signal can select Korea during memory upcycles.

  Pacific Rim tech cluster in H041a is now: EWJ (Japan), EWH (HK/China), EWT (Taiwan),
  + EWY (Korea). Each contributes a distinct return regime.

  Full cross-validation:
    [1] Scorecard — incremental breakdown (H085 base → H086)
    [2] Calendar year 2004-2025
    [3] WF 5-fold detail
    [4] H041a standalone: 11-asset+EWT vs 12-asset+EWY
    [5] H026 standalone (unchanged, top-1 confirmation)
    [6] Further geographic and strategy sweep on 12-asset universe
...
H041A_EWT = ["SPY","QQQ","TLT","GLD","IEF","EFA","EEM","BIL","EWJ","EWH","EWT"]
H041A_EWY = ["SPY","QQQ","TLT","GLD","IEF","EFA","EEM","BIL","EWJ","EWH","EWT","EWY"]
H026_BIL  = ["XLK","XLE","XLF","XLV","XLI","XLB","XLU","XLRE","XLY","XLP","XLC","BIL"]
H045_BIL  = ["SHY","IEI","IEF","TLT","TIP","HYG","LQD","BKLN","EMB","BIL"]

# Further sweep candidates on 12-asset universe
EXTEND_EXTRA = {
    "+EWS":  H041A_EWY + ["EWS"],   # Singapore (tested earlier — retest on new base)
    "+VNM":  H041A_EWY + ["VNM"],   # Vietnam (tech mfg shift)
    "+EWZ":  H041A_EWY + ["EWZ"],   # Brazil (EM commodity — control, expected fail)
}
    ...
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h086_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h086_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H086 — Full Cross-Validation: H041a+EWY (South Korea)")
print("="*80)
...
h41_ewt   = build_rotation_monthly(H041A_EWT, FULL_START, FULL_END, 1)
h41_ewy   = build_rotation_monthly(H041A_EWY, FULL_START, FULL_END, 1)
...
portfolios = {
    "H085 baseline":  (h41_ewt, "11-asset top-1"),
    "H086 (+EWY)":    (h41_ewy, "12-asset top-1"),
}
...
base_d = scorecard["H085 baseline"]
new_d  = scorecard["H086 (+EWY)"]
print(f"\n  {'Year':>5}  {'H085 base':>10}  {'H086 (+EWY)':>11}  {'Delta':>7}")
print("  "+"-"*44)
neg_base = neg_new = 0
cal = []
for yr in range(2004, 2026):
    ...
    if rb < 0: neg_base += 1
    if rn < 0: neg_new  += 1
    print(f"  {yr:>5}  {rb*100:>9.2f}%  {rn*100:>10.2f}%  {(rn-rb)*100:>+6.2f}pp")
    cal.append({"year":yr,"h085":round(rb,4),"h086":round(rn,4)})
print(f"  H085 baseline: {'ZERO' if neg_base==0 else neg_base} negative years")
print(f"  H086 (+EWY):   {'ZERO' if neg_new==0 else neg_new} negative years")
...
for label, h41 in [("11-asset+EWT top-1", h41_ewt), ("12-asset+EWY top-1", h41_ewy)]:
    idx  = h41.index
...
# ── [6] Further sweep on 12-asset universe ────────────────────────────────────
print("\n[6] Further geographic sweep on 12-asset+EWY (top-1) …")
print(f"\n  {'Candidate':14}  {'H041a OOS':>9}  {'H041a AltOOS':>12}  "
      f"{'Port OOS':>9}  {'Port AltOOS':>11}  {'MaxDD':>7}  {'WF':>7}  {'Both↑':>5}")
print("  "+"-"*97)

h41_ewy_oos = stats(h41_ewy[oos_mask(h41_ewy.index)])["sharpe"]
h41_ewy_ao  = stats(h41_ewy[ao_mask(h41_ewy.index)])["sharpe"]
new_base    = scorecard["H086 (+EWY)"]
s_new_oos   = new_base["oos"]["sharpe"]
s_new_ao    = new_base["ao"]["sharpe"]
print(f"  {'EWY base':14}  {h41_ewy_oos:>9.4f}  {h41_ewy_ao:>12.4f}  "
...
# ── Summary ───────────────────────────────────────────────────────────────────
print("\n[7] Summary …")
b = scorecard["H085 baseline"]
h = scorecard["H086 (+EWY)"]
...
confirmed = (h["oos"]["sharpe"] > b["oos"]["sharpe"] and
             ...
             h["wf_ok"])
if confirmed:
    print(f"\n  *** H086 CONFIRMED on BOTH OOS WINDOWS ***")
    print(f"  *** H041a: 12-asset+EWY top-1 ***")
else:
    print(f"\n  H086 not confirmed")
...
output = {
    ...
    "calendar": cal,
    "sec6_geographic_sweep": sec6_results,
    "confirmed": bool(confirmed),
}
out_path = RESULT_DIR / "h086_results.json"
