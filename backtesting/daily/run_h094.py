...
Purpose:
  H093 significantly improved H026 standalone quality by adding GLD+TLT:
    H026 OOS: 2.140 (base) → 2.352 (+GLD+TLT) — major quality improvement

  With a stronger H026 signal, the optimal portfolio weight balance may shift.
  H026 was previously only 7% because of its moderate standalone quality.
  Now that H026 can rotate into gold/bonds, it may warrant higher allocation.

  H093 baseline: OOS 3.6090, AltOOS 3.5387, MaxDD -2.26%, WF 2.595

  Strategy:
    - IBS (XLK+SMH+IGV) fixed at 30% total, internal ratio preserved
    - Sweep H041a: 15% to 35% (step 1%)
    - Sweep H026: 2% to 18% (step 1%) — extended range given higher quality
    - H045 = remainder
    - Find joint optimum: both OOS and AltOOS improve vs H093, WF ≥ 1.75
"""
...
# H093 production weights (baseline)
    ...
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h094_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h094_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H094 — Weight Re-Optimization: H041a, H026, H045 (post-H093)")
print("="*80)
...
print(f"  H093 baseline: OOS {s_b_oos['sharpe']:.4f}, AltOOS {s_b_ao['sharpe']:.4f}, "
      f"WF {min(wf_b):.3f}")
...
# ── [2] H026 weight sweep (extended range: 2-18%) ────────────────────────────
...
h026_sweep = {}
for h26_wt in [w/100 for w in range(2, 19, 1)]:
...
for label, si, so, sa, sao, wf_ in [
    ("H093 baseline (prod)", s_b_is, s_b_oos, s_b_ai, s_b_ao, wf_b),
    ("H094 best weights",    s_is,   s_oos,   s_ai,   s_ao,   wf),
]:
...
print(f"\n  {'Year':>5}  {'H093 (prod)':>11}  {'H094 (new wt)':>13}  {'Delta':>7}")
print("  "+"-"*47)
neg_base = neg_new = 0
cal = []
for yr in range(2004, 2026):
    ...
    cal.append({"year":yr,"h093_prod":round(rb,4),"h094_opt":round(rn,4)})
print(f"  H093 baseline: {'ZERO' if neg_base==0 else neg_base} negative years")
print(f"  H094 new wt:   {'ZERO' if neg_new==0 else neg_new} negative years")

# ── [5] WF fold detail ────────────────────────────────────────────────────────
print("\n[5] WF 5-fold detail …")
print(f"  H093 prod: {[round(f,3) for f in wf_b]} → min {min(wf_b):.3f}")
print(f"  H094 best: {[round(f,3) for f in wf]} → min {ww:.3f}")
...
if confirmed:
    print(f"\n  *** H094 CONFIRMED — NEW PRODUCTION WEIGHTS ***")
    print(f"  *** H041a {w_best['h041a']*100:.1f}% / H026 {w_best['h026']*100:.1f}% / "
          f"H045 {w_best['h045']*100:.1f}% / IBS 30% ***")
else:
    print(f"\n  H094 not confirmed — production weights unchanged")

output = {
    "h093_baseline": {"oos": s_b_oos["sharpe"], "ao": s_b_ao["sharpe"], "wf": min(wf_b)},
    "h094_best": {"oos": s_oos["sharpe"], "ao": s_ao["sharpe"], "wf": ww,
...
out_path = RESULT_DIR / "h094_results.json"
