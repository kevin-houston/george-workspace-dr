"""
H148 — H026 Weight Extreme Concentration (46%–62%)
====================================================

H147 confirmed the full range up to 46% H026 (H041a=H045=12% each), still
with monotonically increasing returns. The ceiling has not been found.

Testing whether higher concentration eventually hits a wall:
  - At 50% H026: H041a=H045=10% each
  - At 55% H026: H041a=H045=7.5% each
  - At 60% H026: H041a=H045=5% each
  - At 62% H026: H041a=H045=4% each

At these extreme levels, H041a and H045 provide minimal diversification.
The question is whether the combined portfolio still benefits from having
them at all, or whether the system is effectively pure H026 + IBS.

Hypothesis: the pattern will eventually flatten or reverse as H041a and H045
reach such small allocations that they cannot absorb H026 drawdowns.

Baseline: H147 system — OOS 56.3034, AltOOS 237.1446, Sharpe 4.146
"""
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 149)]

# (label, h041a_w, h026_w, h045_w) — all sum to 0.70
VARIANTS = [
    ("A) H026=46%, H041a=12%, H045=12% (H147)",  0.12,   0.46,   0.12),
    ("B) H026=50%, H041a=10%, H045=10%",          0.10,   0.50,   0.10),
    ("C) H026=55%, H041a=7.5%, H045=7.5%",        0.075,  0.55,   0.075),
    ("D) H026=60%, H041a=5%, H045=5%",            0.05,   0.60,   0.05),
    ("E) H026=62%, H041a=4%, H045=4%",            0.04,   0.62,   0.04),
    ("F) H026=70%, H041a=0%, H045=0%",            0.00,   0.70,   0.00),
]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h148_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h148_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def main():
    print("=" * 80)
    print("H148 — H026 Weight Extreme Concentration (46%–70%)")
    print("=" * 80)
    print("H147 baseline: OOS 56.3034, AltOOS 237.1446, Sharpe 4.146\n")
    ...
    print("\nPre-computing rotation legs (H147 full system) …")
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H147 baseline (A = 46% H026):\n")
    confirmed_any = False
    for label, w041a, w026, w045 in VARIANTS:
        if label.startswith("A)"):
            continue
    ...
    if confirmed_any:
        ...
        print(f"\n  VERDICT: NOT CONFIRMED — 46% H026 weight is the ceiling")
    print()
