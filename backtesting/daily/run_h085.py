"""
H085 — Full Cross-Validation: H041a+EWT (Taiwan)
=================================================

Purpose:
  H084's extended Asian sweep found EWT (iShares MSCI Taiwan) adds dual-window
  improvement on top of the H084 baseline (OOS 3.3061, AltOOS 3.1838):

    +EWT: Port OOS 3.3548 (+0.0487), AltOOS 3.2178 (+0.0340), Both↑: True

  EWT adds Taiwan semiconductor/tech cycle (TSMC ~25% of index). Distinct from
  EWJ (Japan monetary policy cycle) and EWH (HK/China property/regulation cycle).
  Together: three distinct Pacific Rim equity return streams in H041a's top-1 signal.

  Full cross-validation:
    [1] Scorecard — incremental breakdown (H084 base → H085)
    [2] Calendar year 2004-2025
    [3] WF 5-fold detail
    [4] H041a standalone: 10-asset+EWH vs 11-asset+EWH+EWT
    [5] H026 standalone (unchanged, top-1 confirmation)
    [6] Further Pacific Rim sweep on 11-asset universe (EWY Korea, EWC Canada, EWY+EWC)
...
# Extended sweep: Pacific Rim continuation + control
EXTEND_EXTRA = {
    "+EWY":      H041A_EWT + ["EWY"],        # South Korea (Samsung/SK Hynix cycle)
    "+EWC":      H041A_EWT + ["EWC"],        # Canada (resource/energy diversification)
    "+EWY+EWC":  H041A_EWT + ["EWY","EWC"],  # both together
}
    ...
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h085_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h085_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H085 — Full Cross-Validation: H041a+EWT (Taiwan)")
print("="*80)
...
portfolios = {
    "H084 baseline":  (h41_ewh, "10-asset top-1"),
    "H085 (+EWT)":    (h41_ewt, "11-asset top-1"),
}
...
base_d = scorecard["H084 baseline"]
new_d  = scorecard["H085 (+EWT)"]
print(f"\n  {'Year':>5}  {'H084 base':>10}  {'H085 (+EWT)':>11}  {'Delta':>7}")
print("  "+"-"*44)
neg_base = neg_new = 0
cal = []
for yr in range(2004, 2026):
    ...
    cal.append({"year":yr,"h084":round(rb,4),"h085":round(rn,4)})
print(f"  H084 baseline: {'ZERO' if neg_base==0 else neg_base} negative years")
print(f"  H085 (+EWT):   {'ZERO' if neg_new==0 else neg_new} negative years")
...
for label, h41 in [("10-asset+EWH top-1", h41_ewh), ("11-asset+EWT top-1", h41_ewt)]:
    idx  = h41.index
...
# ── [6] Further Pacific Rim sweep on 11-asset universe ───────────────────────
print("\n[6] Further Pacific Rim sweep on 11-asset+EWT (top-1) …")
print(f"\n  {'Candidate':14}  {'H041a OOS':>9}  {'H041a AltOOS':>12}  "
      f"{'Port OOS':>9}  {'Port AltOOS':>11}  {'MaxDD':>7}  {'WF':>7}  {'Both↑':>5}")
print("  "+"-"*97)

h41_ewt_oos = stats(h41_ewt[oos_mask(h41_ewt.index)])["sharpe"]
h41_ewt_ao  = stats(h41_ewt[ao_mask(h41_ewt.index)])["sharpe"]
new_base    = scorecard["H085 (+EWT)"]
s_new_oos   = new_base["oos"]["sharpe"]
s_new_ao    = new_base["ao"]["sharpe"]
print(f"  {'EWT base':14}  {h41_ewt_oos:>9.4f}  {h41_ewt_ao:>12.4f}  "
...
# ── Summary ───────────────────────────────────────────────────────────────────
print("\n[7] Summary …")
b = scorecard["H084 baseline"]
h = scorecard["H085 (+EWT)"]
...
confirmed = (h["oos"]["sharpe"] > b["oos"]["sharpe"] and
             ...
             h["wf_ok"])
if confirmed:
    print(f"\n  *** H085 CONFIRMED on BOTH OOS WINDOWS ***")
    print(f"  *** H041a: 11-asset+EWT top-1 ***")
else:
    print(f"\n  H085 not confirmed")
...
output = {
    ...
    "calendar": cal,
    "sec6_pacific_rim_sweep": sec6_results,
    "confirmed": bool(confirmed),
}
out_path = RESULT_DIR / "h085_results.json"
