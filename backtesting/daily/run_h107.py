...
Purpose:
  H026 is 18-asset (11-sector+BIL+GLD+TLT+IEF+TIP+DBC+AGG). New candidates:
  - GDX  (gold miners ETF, VanEck Vectors, starts May 2006): distinct from GLD —
         miners have operating leverage to gold prices, also driven by equity beta
  - DBA  (Invesco DB Agriculture, starts Jan 2007): distinct from DBC (broad
         commodity basket). Agriculture commodity cycle, food inflation hedge.
  - SLV  (iShares Silver Trust, starts May 2006): silver vs gold — industrial
         demand component (electronics, solar panels). Failed on 16-asset (H104)
         but worth retrying on 18-asset with DBC already in universe.
  - EZU  (iShares MSCI Eurozone, starts Feb 2001): broad Eurozone equity. Distinct
         from individual country ETFs in H041a. H026 has no broad EM/developed equity.

  H026_DBC_AGG (18-asset) baseline, production weights (H105): 22%/27%/21%
...
CANDIDATES = [
    ...
    ("+DBA+SLV",     ["DBA","SLV"]),
    ("+GDX+SLV",     ["GDX","SLV"]),
    ("+GDX+DBA+SLV", ["GDX","DBA","SLV"]),
    ("+DBA+EZU",     ["DBA","EZU"]),
    ("+GDX+DBA+EZU", ["GDX","DBA","EZU"]),
]

_PREFIXES = [f"h{i:03d}" for i in range(62, 107)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h107_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h107_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H107 — H026 Universe Expansion: GDX, DBA, SLV, EZU")
print("="*80)
...
print("\n[1] Building H026 baseline (H104: 18-asset) …")
...
header = (f"  {'Candidate':>15}  {'H026 IS':>8}  {'H026 OOS':>9}  {'H026 AltOOS':>12}  "
...
for label, extras in CANDIDATES:
    ...
    print(f"  {label:>15}  {h026_is['sharpe']:>8.4f}  {h026_oos['sharpe']:>9.4f}  "
...
if confirmed:
    ...
    print(f"\n  {'Portfolio':35}  {'IS S':>7}  {'OOS S':>7}  {'AltIS S':>8}  "
          f"{'AltOOS S':>9}  {'CAGR':>7}  {'MaxDD':>8}  {'WF':>7}")
    print("  "+"-"*103)
    for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
        ("H105 baseline (18-asset)", c_is, s_base_oos, c_ai, s_base_ao, wf_base),
        (f"H107 {best['label']}", s_is, s_oos, s_ai, s_ao, wf_b),
    ]:
        print(f"  {lbl:35}  {s_i['sharpe']:>7.4f}  {s_o['sharpe']:>7.4f}  "
    ...
    print(f"\n  *** H107 CONFIRMED — H026 expanded with {best['label']} ***")
...
out_path = RESULT_DIR / "h107_results.json"
