"""
H093 — H026 Universe Expansion: GLD, TLT, IEF
===============================================

Purpose:
  H026 (sector rotation) current universe: 11 SPDR sectors + BIL (12-asset, top-1)
  The BIL addition (H082) was the largest single OOS improvement in this research chain
  (+0.272 from H045 + H041a combined) because it let the signal rotate to cash when
  all equity sectors are weak.

  By analogy: adding GLD (gold) and TLT (long-term Treasuries) allows H026 to rotate
  into traditional safe havens during equity stress — not just cash. These are distinct
  from BIL:
    - GLD: crisis hedge and inflation hedge; negative correlation to equities in 2008/2020
    - TLT: flight-to-quality; thrives in deflationary recessions (2001/2008/2020)
    - IEF: intermediate Treasuries; same cycle as TLT but less extreme

  Sweep structure:
  - "+GLD"         : add GLD to 12-asset universe
  - "+TLT"         : add TLT (note: TLT already in H041a but not H026)
  - "+IEF"         : add IEF intermediate bonds
  - "+GLD+TLT"     : both crisis assets
  - "+GLD+IEF"     : gold + intermediate bonds
  - "+TLT+IEF"     : dual duration bonds (likely redundant)
  - "+GLD+TLT+IEF" : full safe-haven suite
...
H026_BASE   = ["XLK","XLE","XLF","XLV","XLI","XLB","XLU","XLRE","XLY","XLP","XLC","BIL"]
...
H026_CANDIDATES = {
    ...
    "+TLT":          H026_BASE + ["TLT"],
    "+IEF":          H026_BASE + ["IEF"],
    "+GLD+TLT":      H026_BASE + ["GLD","TLT"],
    "+GLD+IEF":      H026_BASE + ["GLD","IEF"],
    "+TLT+IEF":      H026_BASE + ["TLT","IEF"],
    "+GLD+TLT+IEF":  H026_BASE + ["GLD","TLT","IEF"],
}
    ...
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h093_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h093_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H093 — H026 Universe Expansion: GLD, TLT, IEF")
print("="*80)
...
# ── [1] H026 candidate sweep ──────────────────────────────────────────────────
print("\n[1] H026 universe candidate sweep …")
print(f"\n  {'Candidate':18}  {'H026 IS':>8}  {'H026 OOS':>9}  {'H026 AltOOS':>12}  "
...
for name, tickers in H026_CANDIDATES.items():
    try:
        h026_v = build_rotation_monthly(tickers, FULL_START, FULL_END, 1)
...
if winners:
    ...
    cidx_b = common_idx(h026_base, h045, xlk_r, smh_r, igv_r, h41)
    ...
    for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
        ...
        (f"H093 ({best_name})", s_is, s_oos, s_ai, s_ao, wf)
    ]:
    ...
    print(f"  {'Year':>5}  {'H090 base':>10}  {f'H093 ({best_name})':>18}  {'Delta':>7}")
    print("  "+"-"*53)
    cal = []
    neg_b = neg_n = 0
    for yr in range(2004, 2026):
        ...
        if rb < 0: neg_b += 1
        if rn < 0: neg_n += 1
        print(f"  {yr:>5}  {rb*100:>9.2f}%  {rn*100:>17.2f}%  {(rn-rb)*100:>+6.2f}pp")
        cal.append({"year":yr,"h090":round(rb,4),"h093":round(rn,4)})
    print(f"  H090: {'ZERO' if neg_b==0 else neg_b} neg yrs  |  H093: {'ZERO' if neg_n==0 else neg_n} neg yrs")
    ...
    if confirmed:
        print(f"\n  *** H093 CONFIRMED — H026 += {best_name} ***")
    else:
        print(f"\n  H093 not confirmed — H026 universe unchanged.")
else:
    cal = []
    confirmed = False
    best_name = "none"
    ...
    print("  H093 not confirmed — H026 universe unchanged.")
...
out_path = RESULT_DIR / "h093_results.json"
