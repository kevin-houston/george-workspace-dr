...
Purpose:
  H096 tested top-N on the 14-asset H026. Both top-2 and top-3 failed because
  GLD/TLT were the only non-sector assets, and holding 2 simultaneously hurt.

  H026 is now 18-asset (11-sector+BIL+GLD+TLT+IEF+TIP+DBC+AGG) with much more
  diversity: equities, treasuries, TIPS, commodities, broad bonds, cash.
  With 18 varied assets, a top-2 or top-3 selection could capture both momentum
  AND asset-class diversification in each month.

  New production weights: H041a 22% / H026 27% / H045 21% / IBS 30%

  H026_DBC_AGG (18-asset): 11-sector+BIL+GLD+TLT+IEF+TIP+DBC+AGG
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h106_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h106_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H106 — H026 Top-N Sweep (18-asset Universe)")
print("="*80)
...
print("\n[1] Building H026 variants (top-1, top-2, top-3) …")
h026_top1 = build_rotation_monthly(H026_DBC_AGG, FULL_START, FULL_END, 1)
h026_top2 = build_rotation_monthly(H026_DBC_AGG, FULL_START, FULL_END, 2)
h026_top3 = build_rotation_monthly(H026_DBC_AGG, FULL_START, FULL_END, 3)

cidx = common_idx(h41, h026_top1, h026_top2, h026_top3, h045, xlk_r, smh_r, igv_r)

print("\n[2] Evaluating H026 variants …")
header = f"  {'Variant':10}  {'H026 IS':>8}  {'H026 OOS':>9}  {'H026 AltOOS':>12}  "
header += f"{'Port OOS':>9}  {'Port AltOOS':>11}  {'MaxDD':>7}  {'WF':>7}  {'Both↑':>5}"
print(header)
print("  "+"-"*103)

results = []
base_oos = base_ao = base_wf = None
for label, h026_r in [("top-1 (base)", h026_top1), ("top-2", h026_top2), ("top-3", h026_top3)]:
    rd = {"h041a":h41,"h026":h026_r,"h045":h045,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}
    cidx_v = common_idx(h41, h026_r, h045, xlk_r, smh_r, igv_r)
    w = PROD_W

    h026_is  = stats(h026_r[is_mask(h026_r.index)])
    h026_oos = stats(h026_r[oos_mask(h026_r.index)])
    h026_ao  = stats(h026_r[ao_mask(h026_r.index)])
    ...
    if base_oos is None:
        base_oos = port_oos["sharpe"]; base_ao = port_ao["sharpe"]; base_wf = ww
        both_up = True
    else:
        both_up = bool(port_oos["sharpe"] > base_oos and port_ao["sharpe"] > base_ao and ww >= WF_WORST_MIN)

    mark = "✓" if both_up else "✗"
    print(f"  {label:10}  {h026_is['sharpe']:>8.4f}  {h026_oos['sharpe']:>9.4f}  "
          f"{h026_ao['sharpe']:>12.4f}  {port_oos['sharpe']:>9.4f}  "
          f"{port_ao['sharpe']:>11.4f}  {port_oos['max_drawdown']*100:>6.2f}%  "
          f"{ww:>7.3f}  {mark:>5}")

    results.append({
        "label": label, "h026_is": h026_is["sharpe"],
        "h026_oos": h026_oos["sharpe"], "h026_ao": h026_ao["sharpe"],
        "port_oos": port_oos["sharpe"], "port_ao": port_ao["sharpe"],
        "maxdd": port_oos["max_drawdown"], "wf": ww,
        "both_up": bool(both_up),
    })

winners = [r for r in results[1:] if r["both_up"]]
print(f"\n[3] Winners: {[r['label'] for r in winners] if winners else 'None'}")

# Find best winner by sum
best = max(results, key=lambda x: x["port_oos"] + x["port_ao"])
confirmed = len(winners) > 0

if confirmed:
    best_winner = max(winners, key=lambda x: x["port_oos"] + x["port_ao"])
    print(f"\n  Best winner: {best_winner['label']} "
          f"(OOS {best_winner['port_oos']:.4f}, AltOOS {best_winner['port_ao']:.4f})")
    n_top = int(best_winner["label"].split("-")[1])
    print(f"\n  *** H106 CONFIRMED — H026 top-{n_top} on 18-asset universe ***")
else:
    print(f"\n  H026 top-N: top-1 remains optimal on 18-asset universe.")

print(f"\n[4] Summary …")
print(f"  Baseline (top-1): OOS {base_oos:.4f}, AltOOS {base_ao:.4f}")
if confirmed:
    bw = max(winners, key=lambda x: x["port_oos"] + x["port_ao"])
    print(f"  Best candidate:   OOS {bw['port_oos']:.4f}, AltOOS {bw['port_ao']:.4f}")

output = {
    "candidates": results,
    "confirmed": bool(confirmed),
    "base_oos": base_oos, "base_ao": base_ao,
}
out_path = RESULT_DIR / "h106_results.json"
