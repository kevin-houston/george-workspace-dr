"""
H134 — Enhanced H045 Bond Rotation Scoring (Full Rank Ensemble)
===============================================================

H045 currently selects top-2 bonds using: rank(12m_mom) + rank(inv_6m_vol).
H026 and H041a use the full ensemble: rank(3m) + rank(6m) + rank(12m) + rank(inv_vol).

H134 tests whether adding 3m and/or 6m momentum ranks to H045's scoring formula
improves selection quality among bonds that pass the 3m TSMOM filter.

Note: the 3m TSMOM filter (H128) is unchanged — it gates which bonds are eligible.
This hypothesis only changes the *ranking* within eligible bonds.

Economic logic: among bonds with positive 3m momentum, ranking by only 12m+inv_vol
may miss recent trend acceleration. Adding 3m/6m ranks would prefer bonds that are
accelerating vs just having a long-term positive trend.

Variants:
  A) rank(12m) + rank(inv_vol)                 ← current H128/H130 production
  B) rank(3m) + rank(12m) + rank(inv_vol)      ← add short-term momentum
  C) rank(6m) + rank(12m) + rank(inv_vol)      ← add medium-term momentum
  D) rank(3m) + rank(6m) + rank(12m) + rank(inv_vol)  ← full ensemble

Reference: H133 production OOS 23.2803, AltOOS 79.9716, Sharpe 4.846
"""
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 135)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h134_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h134_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def build_h045_scored(tickers, start, end, n_hold=2, score_terms: list = None):
    """
    H045 bond rotation with 3m TSMOM filter and configurable score terms.
    score_terms: subset of ['3m', '6m', '12m', 'inv_vol'] — all included if None.
    Default (production): ['12m', 'inv_vol']
    """
    if score_terms is None:
        score_terms = ["12m", "inv_vol"]
    ...
    for i in range(12, len(monthly_px)):
        ...
        valid   = (m12_row.index.intersection(vol_row.index)
                   .intersection(m6_row.index).intersection(m3_row.index))
        # 3m TSMOM filter (H128 — unchanged)
        ...
        score = pd.Series(0.0, index=passing)
        if "12m" in score_terms:
            score = score + m12_row.reindex(passing).rank()
        if "6m" in score_terms:
            score = score + m6_row.reindex(passing).rank()
        if "3m" in score_terms:
            score = score + m3_row.reindex(passing).rank()
        if "inv_vol" in score_terms:
            score = score + vol_row.reindex(passing).rank(ascending=False)
...
def print_stats(r, label):
    ...
    print(f"  {label:<56} IS={is_['sharpe']:.3f}/{is_['cagr']*100:.1f}%  "
...
def main():
    print("=" * 80)
    print("H134 — Enhanced H045 Bond Rotation Scoring (Full Rank Ensemble)")
    print("=" * 80)
    print("H133 reference: OOS 23.2803, AltOOS 79.9716, Sharpe 4.846\n")
    ...
    print("\nPre-computing H041a (3m filter) and H026 (12m filter) …")
    ...
    VARIANTS = [
        ("A) rank(12m) + rank(inv_vol)  [baseline]",
         ["12m", "inv_vol"]),
        ("B) rank(3m) + rank(12m) + rank(inv_vol)",
         ["3m", "12m", "inv_vol"]),
        ("C) rank(6m) + rank(12m) + rank(inv_vol)",
         ["6m", "12m", "inv_vol"]),
        ("D) rank(3m) + rank(6m) + rank(12m) + rank(inv_vol)",
         ["3m", "6m", "12m", "inv_vol"]),
    ]
    ...
    for label, terms in VARIANTS:
        ...
        h045_series[tag] = build_h045_scored(H045_ASSETS, FULL_START, FULL_END,
                                              n_hold=2, score_terms=terms)
        n_cash = sum(1 for v in h045_series[tag].values if v == 0.0)
    ...
    for label, terms in VARIANTS:
        tag = label[:2]
        sub_rets = {"h041a": h041a, "h026": h026, "h045": h045_series[tag], **ibs_rets}
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H133 baseline (A):\n")
    confirmed_any = False
