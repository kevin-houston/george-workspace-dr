"""
H141 — H045 TSMOM Threshold Sweep
=====================================

H139 raised H026 TSMOM threshold to +5% (confirmed).
H140 raised H041a TSMOM threshold to +0.5% (confirmed).
Testing whether H045 also benefits from a tighter TSMOM threshold.

Current production: H045 holds bond ETF if 3m return > 0%.

For bonds in normal conditions:
  - Short-term bonds (SHY, IEI): ~0.1-0.5% per quarter at 2025 rates
  - BIL: ~1.25% per quarter (5% annual at 2025 rates)
  - TLT/TIP (duration): can range -15% to +15% per quarter
  - BKLN/FLOT (floating): ~1-1.5% per quarter (track SOFR)

A +0.25% threshold would filter bonds barely above breakeven.
A +0.5% threshold would require at least 2% annualized 3m return.
Higher thresholds may exclude too many valid bonds (fixed-rate short-duration).

Note: "bonds need fast re-entry" is the concern for tighter thresholds.
Tighter thresholds might miss early recovery after rate normalization.

Baseline is the H140 production system:
  H026 tsmom_threshold=+5%, H041a tsmom_threshold=+0.5%, H045=0%

Variants:
  A) H045 threshold=0.0% (current)
  B) H045 threshold=+0.25%
  C) H045 threshold=+0.5%
  D) H045 threshold=+1.0%
  E) H045 threshold=+2.0%

Reference: H140 baseline OOS 26.9943, AltOOS 95.6933, Sharpe 5.012
"""
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 142)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h141_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h141_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def main():
    print("=" * 80)
    print("H141 — H045 TSMOM Threshold Sweep")
    print("=" * 80)
    print("H140 baseline: OOS 26.9943, AltOOS 95.6933, Sharpe 5.012")
    print("(H026 threshold=+5%, H041a threshold=+0.5% already applied)\n")
    ...
    print("\nPre-computing H026 (+5% threshold) and H041a (+0.5% threshold) …")
    ...
    VARIANTS = [
        ("A) H045 threshold=0.0% (current baseline)", 0.000),
        ("B) H045 threshold=+0.25%",                   0.0025),
        ("C) H045 threshold=+0.5%",                    0.005),
        ("D) H045 threshold=+1.0%",                    0.010),
        ("E) H045 threshold=+2.0%",                    0.020),
    ]

    print("\nBuilding H045 variants (3m TSMOM with varied threshold) …")
    h045_series = {}
    for label, thresh in VARIANTS:
        tag = label[:2]
        print(f"  {label} …", end=" ", flush=True)
        h045_series[tag] = build_rotation_tsmom(
            H045_ASSETS, FULL_START, FULL_END,
            n_hold=2, tsmom_lookbacks=[3], tsmom_threshold=thresh
        )
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H140 baseline (A = 0% H045 threshold):\n")
    confirmed_any = False
    for label, thresh in VARIANTS:
        if label.startswith("A)"):
            continue
    ...
    if confirmed_any:
        ...
        print(f"\n  VERDICT: NOT CONFIRMED — 0% threshold is optimal for H045")
