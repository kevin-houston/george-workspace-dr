...
Purpose:
  H090 significantly improved H045 standalone quality by adding MBB+FLOT:
    H045 OOS: 2.227 (H082) → 2.583 (H090) — major quality improvement

  With a stronger H045 signal, the optimal portfolio weight balance may shift.
  Current production weights (H089): H041a 23%, H026 7%, H045 40%, IBS 30%.

  H090 baseline: OOS 3.5171, AltOOS 3.4382, MaxDD -2.26%, WF 2.444

  Strategy:
    - IBS (XLK+SMH+IGV) fixed at 30% total, internal ratio preserved
    - Sweep H041a: 15% to 35% (step 1%)
    - Sweep H026: 2% to 15% (step 1%)
    - H045 = remainder (after H041a + H026 + 30% IBS)
    - Find joint optimum: both OOS and AltOOS improve vs H090, WF ≥ 1.75

  [1] H041a weight sweep (H026=7.0% fixed)
  [2] H026 weight sweep (H041a fixed at best from [1])
  [3] Full stats at best joint weights vs H090 baseline
...
# H045 after H090: 12-asset with MBB+FLOT
H045_MBB_FLOT = ["SHY","IEI","IEF","TLT","TIP","HYG","LQD","BKLN","EMB","BIL","MBB","FLOT"]

# H090 production weights (baseline for comparison)
    ...
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h092_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h092_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H092 — Weight Re-Optimization: H041a, H026, H045 (post-H090)")
print("="*80)
...
h045_new  = build_rotation_monthly(H045_MBB_FLOT, FULL_START, FULL_END, 2)
...
cidx   = common_idx(h045_new, h026_top1, xlk_r, smh_r, igv_r, h41_ephe)
r_dict = {"h041a":h41_ephe,"h026":h026_top1,"h045":h045_new,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}

# H090 baseline reference (same H045 universe, current weights)
...
print(f"  H090 baseline: OOS {s_b_oos['sharpe']:.4f}, AltOOS {s_b_ao['sharpe']:.4f}, "
      f"WF {min(wf_b):.3f}")
...
h041a_sweep = {}
for h41_wt in [w/100 for w in range(15, 36, 1)]:
    w = make_weights(h41_wt, 0.07)
...
h026_sweep = {}
for h26_wt in [w/100 for w in range(2, 16, 1)]:
    ...
    wf    = run_wf(cidx, r_dict, w)
    ww    = min(wf) if wf else 0.0
    marker = " ← current" if abs(h26_wt - 0.07) < 0.001 else ""
...
best_h26 = max(
    ...
    key=lambda kv: kv[1]["oos"] + kv[1]["ao"],
    default=(0.07, {"oos": s_b_oos["sharpe"], "ao": s_b_ao["sharpe"], "wf": min(wf_b)})
)
best_h026_wt = best_h26[0]
print(f"\n  Best H026 weight: {best_h026_wt*100:.1f}%")

# ── [3] Full stats at best weights ────────────────────────────────────────────
...
for label, si, so, sa, sao, wf_ in [
    ("H090 baseline (prod)", s_b_is, s_b_oos, s_b_ai, s_b_ao, wf_b),
    ("H092 best weights",    s_is,   s_oos,   s_ai,   s_ao,   wf),
]:
...
print(f"\n  {'Year':>5}  {'H090 (prod)':>11}  {'H092 (new wt)':>13}  {'Delta':>7}")
print("  "+"-"*47)
neg_base = neg_new = 0
cal = []
for yr in range(2004, 2026):
    ...
    cal.append({"year":yr,"h090_prod":round(rb,4),"h092_opt":round(rn,4)})
print(f"  H090 baseline: {'ZERO' if neg_base==0 else neg_base} negative years")
print(f"  H092 new wt:   {'ZERO' if neg_new==0 else neg_new} negative years")

# ── [5] WF fold detail ────────────────────────────────────────────────────────
print("\n[5] WF 5-fold detail …")
print(f"  H090 prod: {[round(f,3) for f in wf_b]} → min {min(wf_b):.3f}")
print(f"  H092 best: {[round(f,3) for f in wf]} → min {ww:.3f}")
...
if confirmed:
    print(f"\n  *** H092 CONFIRMED — NEW PRODUCTION WEIGHTS ***")
    print(f"  *** H041a {w_best['h041a']*100:.1f}% / H026 {w_best['h026']*100:.1f}% / "
          f"H045 {w_best['h045']*100:.1f}% / IBS 30% ***")
else:
    print(f"\n  H092 not confirmed — production weights unchanged")

output = {
    "h090_baseline": {"oos": s_b_oos["sharpe"], "ao": s_b_ao["sharpe"], "wf": min(wf_b)},
    "h092_best": {"oos": s_oos["sharpe"], "ao": s_ao["sharpe"], "wf": ww,
...
out_path = RESULT_DIR / "h092_results.json"
