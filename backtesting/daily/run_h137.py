"""
H137 — H026 Universe Cleanup: Remove Problematic Commodity ETFs
===============================================================

Current H026 universe (25 assets) includes commodity futures ETFs with
structural problems:
  - UNG (natural gas): extreme contango roll yield (~20-30% annual drag),
    multiple reverse splits (5-for-1 in 2020, others). Price chart is
    mostly downward trend interrupted by occasional spikes.
  - USO (crude oil): April 2020 crisis (WTI went negative), emergency
    restructuring, shifted to spread contracts. Structural change corrupts
    pre/post-2020 continuity.
  - DBA (agricultural): less severe than UNG/USO but has roll costs and
    seasonal patterns that fight momentum signals.

These assets may create spurious momentum signals (brief spike looks like
uptrend, but 12m TSMOM would confirm if spike persists) or drag the H026
signal quality by occupying a slot in the ranking.

Variants:
  A) Full 25-asset universe (baseline, current production)
  B) Remove UNG only (24 assets)
  C) Remove USO only (24 assets)
  D) Remove UNG + USO (23 assets)
  E) Remove UNG + USO + DBA (22 assets)
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 138)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h137_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h137_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def main():
    print("=" * 80)
    print("H137 — H026 Universe Cleanup: Remove Problematic Commodity ETFs")
    print("=" * 80)
    ...
    VARIANTS = [
        ("A) Full 25-asset universe (baseline)", H026_BASE[:]),
        ("B) Remove UNG (24 assets)",            [t for t in H026_BASE if t != "UNG"]),
        ("C) Remove USO (24 assets)",            [t for t in H026_BASE if t != "USO"]),
        ("D) Remove UNG + USO (23 assets)",      [t for t in H026_BASE if t not in ("UNG","USO")]),
        ("E) Remove UNG + USO + DBA (22 assets)",[t for t in H026_BASE if t not in ("UNG","USO","DBA")]),
    ]
    ...
    for label, universe in VARIANTS:
        tag = label[:2]
        print(f"  {label} ({len(universe)} assets) …", end=" ", flush=True)
        h026_series[tag] = build_rotation_tsmom(universe, FULL_START, FULL_END,
    ...
    for label, (oos_s, alt_s) in results.items():
        ...
        if d_oos > 0 and d_alt > 0:
            confirmed_any = True
        removed = label.split("Remove ")[-1].split(" (")[0] if "Remove" in label else "—"
        print(f"  {label}")
