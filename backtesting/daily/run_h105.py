"""
H105 — Weight Re-optimization Post-H104
========================================

Purpose:
  H104 expanded H026 to 18-asset (+DBC+AGG), gaining +0.142/+0.095 OOS/AltOOS
  and improving MaxDD from -3.04% to -2.73%.  This is the largest single-step
  OOS gain since H099. The Pareto frontier may have shifted.

  H100 and H103 both showed 23%/18%/29% as the joint optimum, but the H026
  signal is now much richer (commodity cycle + full bond spectrum).  Re-test.

  Sweep:
    Phase 1 — H026 sweep (H041a=23% fixed): 10% to 30% in 1pp steps
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 105)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h105_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h105_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H105 — Weight Re-optimization Post-H104")
print("="*80)
...
h026  = build_rotation_monthly(H026_DBC_AGG, FULL_START, FULL_END, 1)
...
for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
    ("H104 (23%/18%/29%)", c_is, s_curr_oos, c_ai, s_curr_ao, wf_curr),
    (f"H105 ({new_h041a_pct}%/{new_h026_pct}%/{new_h045_pct}%)", s_is, s_oos, s_ai, s_ao, wf),
]:
...
print(f"  {'Year':>5}  {'H104 (23%/18%/29%)':>19}  {'H105 best':>10}  {'Delta':>7}")
print("  "+"-"*58)
cal = []
neg_b = neg_n = 0
for yr in range(2004, 2026):
    ...
    cal.append({"year":yr,"h104_weights":round(rb,4),"h105_weights":round(rn,4)})
print(f"  H104: {'ZERO' if neg_b==0 else neg_b} neg | H105: {'ZERO' if neg_n==0 else neg_n} neg")
...
if confirmed:
    print(f"\n  *** H105 CONFIRMED — new weights {new_h041a_pct}%/{new_h026_pct}%/{new_h045_pct}% ***")
else:
    print(f"\n  H105 not confirmed — weights 23%/18%/29% unchanged.")
...
out_path = RESULT_DIR / "h105_results.json"
