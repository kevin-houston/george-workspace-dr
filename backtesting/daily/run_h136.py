"""
H136 — Vol-Targeting on H045 Bond ETF Rotation
===============================================

H122: H026 vol-targeted at 15% — CONFIRMED (Sharpe improvement, MaxDD -2.4%)
H133: H041a vol-targeted at 25% — CONFIRMED (Sharpe 4.600→4.846)
H136: Test vol-targeting on H045 (the only rotation leg without vol-scaling).

H045 bond rotation typical realized vol when invested: ~5-8% annualized.
The 3m TSMOM filter (H128) already exits to cash during severe bond stress
(2022 rising-rate crash). Vol-targeting would activate during moderate stress
periods where H045 remains invested but with elevated vol.

Variants:
  A) H045 fixed weight 21% (baseline, current production)
  B) Vol-target H045 at 6%  (conservative — most likely to scale down)
  C) Vol-target H045 at 8%  (moderate — activates during stress periods)
  D) Vol-target H045 at 10% (permissive — only extreme events)
  E) Vol-target H045 at 12% (loose — near equity-like events)

Vol window: 6 months, clamp 0.5x–2x (same as H026/H041a).
H026 vol-target 15% and H041a vol-target 25% remain active.
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 137)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h136_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h136_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def blend_triple_vol_targeted(sub_rets, vol_target_h045):
    """
    Triple vol-targeting: H026 at 15%, H041a at 25%, H045 variable.
    vol_target_h045=None → fixed weight (baseline).
    """
    ...
    for i in range(len(df)):
        ...
        if vol_target_h045 is not None and "h045" in df.columns and i >= VOL_WINDOW:
            window = df["h045"].iloc[i - VOL_WINDOW:i]
            if len(window) >= 3:
                rv = float(window.std(ddof=1)) * np.sqrt(12)
                if rv > 0:
                    row_weights["h045"] = PROD_W["h045"] * float(np.clip(vol_target_h045 / rv, *VOL_CLAMP))
...
def main():
    print("=" * 80)
    print("H136 — Vol-Targeting on H045 Bond ETF Rotation")
    print("=" * 80)
    ...
    print("\nPre-computing rotation legs (H041a 3m, H026 12m) …")
    ...
    VARIANTS = [
        ("A) H045 fixed 21% (baseline)",       None),
        ("B) Vol-target H045 at 6%",           0.06),
        ("C) Vol-target H045 at 8%",           0.08),
        ("D) Vol-target H045 at 10%",          0.10),
        ("E) Vol-target H045 at 12%",          0.12),
    ]
    ...
    for label, vt in VARIANTS:
        port = blend_triple_vol_targeted(sub_rets, vol_target_h045=vt)
