"""
H133 — Vol-Targeting on H041a Global Equity Rotation
=====================================================

H122 confirmed vol-targeting on H026 (scale weight by target_vol / realized_6m_vol).
H041a has similar equity-driven dynamics: high vol in 2022, 2020, 2008 — all periods
where reducing H041a weight would have protected the portfolio.

H131 showed the 12m filter is optimal for H026 (shorter filters add whipsaw).
H130 confirmed 3m TSMOM on H041a (exit to cash in broad equity downturns).
Vol-targeting is orthogonal: it adjusts H041a *weight* when invested, rather than
gating entry/exit. During moderate high-vol periods (e.g. 2018 correction) where
H041a is still invested but vol is elevated, scaling down weight protects returns.

Variants:
  A) H041a fixed weight 22% (baseline, current production)
  B) Vol-target H041a at 15% annualized (conservative — same as H026)
  C) Vol-target H041a at 18% annualized (moderate)
  D) Vol-target H041a at 20% annualized (permissive)
  E) Vol-target H041a at 25% annualized (loose — near typical equity vol)

Vol window: 6 months, clamp 0.5x–2x (same implementation as H026/H122).
When H041a vol-target scales H041a weight, rotation total is renormed to 70%.
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 134)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h133_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h133_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def build_rotation_tsmom(tickers, start, end, n_hold=1, tsmom_lookbacks=None):
    ...
    # Also return monthly returns for vol-targeting use
    monthly_rt_filtered = pd.Series(vals, index=idx)
    return monthly_rt_filtered
...
def blend_dual_vol_targeted(sub_rets, vol_target_h041a):
    """
    Vol-target both H026 (H122 production) and H041a (H133 test).
    vol_target_h041a=None means fixed weight (baseline).
    """
    ...
    for i in range(len(df)):
        ...
        # H026 vol-targeting (H122 — always active)
        if "h026" in df.columns and i >= VOL_WINDOW:
            window = df["h026"].iloc[i - VOL_WINDOW:i]
            if len(window) >= 3:
                rv = float(window.std(ddof=1)) * np.sqrt(12)
                if rv > 0:
                    scalar = float(np.clip(VOL_TARGET_H026 / rv, *VOL_CLAMP))
                    row_weights["h026"] = PROD_W["h026"] * scalar

        # H041a vol-targeting (H133 test — only if vol_target_h041a is set)
        if vol_target_h041a is not None and "h041a" in df.columns and i >= VOL_WINDOW:
            window = df["h041a"].iloc[i - VOL_WINDOW:i]
            if len(window) >= 3:
                rv = float(window.std(ddof=1)) * np.sqrt(12)
                if rv > 0:
                    scalar = float(np.clip(vol_target_h041a / rv, *VOL_CLAMP))
                    row_weights["h041a"] = PROD_W["h041a"] * scalar
...
def main():
    print("=" * 80)
    print("H133 — Vol-Targeting on H041a Global Equity Rotation")
    print("=" * 80)
    ...
    VARIANTS = [
        ("A) H041a fixed 22% (baseline)",        None),
        ("B) Vol-target H041a at 15%",           0.15),
        ("C) Vol-target H041a at 18%",           0.18),
        ("D) Vol-target H041a at 20%",           0.20),
        ("E) Vol-target H041a at 25%",           0.25),
    ]
    ...
    for label, vt in VARIANTS:
        port = blend_dual_vol_targeted(sub_rets, vol_target_h041a=vt)
    ...
    for label, (oos_s, alt_s) in results.items():
        ...
        vt_label = label.split("at ")[-1] if "at" in label else "—"
        print(f"  {label}")
