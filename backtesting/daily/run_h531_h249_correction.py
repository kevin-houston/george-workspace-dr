"""
H531 — H249 Look-Ahead Bias Correction (bug-audit re-run)
============================================
H249 (CONFIRMED, "+0.282 Sharpe improvement") computes prior_dt =
dt - MonthEnd(1) but never uses it: regime_df.loc[dt, ...] and
rate_hike.loc[dt] both read the CURRENT month's own regime, letting month
t's own month-end SPY/VIX close leak into the weight decision credited
with that same month's return. Same as-of-date look-ahead bug class that
retracted the H343/.../H493 OB-filter family (flagged by H523's side-by-side
check, OOS Sharpe 1.103 unlagged vs 0.769 lag-fixed).

This script fixes ONLY that lag (regime/rate-hike lookups now use prior_dt)
and reports both the as-shipped (unlagged, for reference) and corrected
(lag-fixed) regime-conditional portfolios side by side, matching the
rigor of the H510-H514 corrections.
...
def main():
    print("=" * 60)
    print("H531 — H249 Look-Ahead Bias Correction (bug-audit re-run)")
    print("=" * 60)
    ...
    # Regime-conditional portfolio — computed TWICE:
    #   (a) as-shipped/unlagged (H249's actual code path, for reference only)
    #   (b) lag-fixed/corrected (uses prior_dt, the honest one-month lag)
    def build_regime_portfolio(use_prior_dt):
        rets, wts_used = [], []
        for dt in common_idx:
            prior_dt = dt - pd.offsets.MonthEnd(1)
            lookup_dt = prior_dt if use_prior_dt else dt
            mkt = regime_df.loc[lookup_dt, "market"] if lookup_dt in regime_df.index else "bull"
            vol = regime_df.loc[lookup_dt, "vol"] if lookup_dt in regime_df.index else "calm"
            hike = rate_hike.loc[lookup_dt] if lookup_dt in rate_hike.index else False
            ...
            rets.append(r)
            wts_used.append(wts)
        return pd.Series(rets, index=common_idx), wts_used

    r_regime_unlagged, _ = build_regime_portfolio(use_prior_dt=False)
    r_regime, regime_wt_used = build_regime_portfolio(use_prior_dt=True)
    ...
    # Regime-conditional — as-shipped (unlagged, reference only)
    regime_unlagged_is  = calc_stats(r_regime_unlagged[is_mask],  label="Regime as-shipped IS")
    regime_unlagged_oos = calc_stats(r_regime_unlagged[oos_mask], label="Regime as-shipped OOS")
    print(f"    Regime blend (AS-SHIPPED, unlagged, reference only): "
          f"IS Sharpe={regime_unlagged_is['sharpe']:.3f}  OOS Sharpe={regime_unlagged_oos['sharpe']:.3f}  "
          f"MaxDD={regime_unlagged_oos['max_dd']:.3f}")

    # Regime-conditional — corrected (lag-fixed, prior_dt actually used)
    regime_is  = calc_stats(r_regime[is_mask],  label="Regime corrected IS")
    regime_oos = calc_stats(r_regime[oos_mask], label="Regime corrected OOS")
    print(f"    Regime blend (CORRECTED, lag-fixed): IS  Sharpe={regime_is['sharpe']:.3f}  "
          f"MaxDD={regime_is['max_dd']:.3f}  NegYrs={regime_is['neg_yrs']}")
    print(f"    Regime blend (CORRECTED, lag-fixed): OOS Sharpe={regime_oos['sharpe']:.3f}  "
          f"MaxDD={regime_oos['max_dd']:.3f}  NegYrs={regime_oos['neg_yrs']}")

    sharpe_improvement = regime_oos["sharpe"] - static_oos["sharpe"]
    sharpe_improvement_unlagged = regime_unlagged_oos["sharpe"] - static_oos["sharpe"]
    print(f"\n    OOS Sharpe improvement (CORRECTED vs static): {sharpe_improvement:+.3f}")
    print(f"    OOS Sharpe improvement (as-shipped vs static, reference): {sharpe_improvement_unlagged:+.3f}")
    ...
    results = {
        "hypothesis":          "H531",
        "description":         "H249 look-ahead bias correction (bug-audit re-run)",
        "bug":                 "H249's loop computed prior_dt but read regime_df.loc[dt,...]/rate_hike.loc[dt] "
                                "(current month's own close) instead of prior_dt (honest one-month lag)",
        "confirmed":           confirmed,
        "confirm_gate":        "OOS Sharpe improvement > 0.20 vs static (H249's own gate, applied to corrected series)",
        "sharpe_improvement":            round(sharpe_improvement, 4),
        "sharpe_improvement_unlagged":   round(sharpe_improvement_unlagged, 4),
        "static_is":           static_is,
        "static_oos":          static_oos,
        "regime_is":           regime_is,
        "regime_oos":          regime_oos,
        "regime_unlagged_is":  regime_unlagged_is,
        "regime_unlagged_oos": regime_unlagged_oos,
        "regime_weights": {
    ...
    out_path = RESULT_DIR / "h531_results.json"
