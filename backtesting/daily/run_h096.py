"""
H096 — H026 Top-N Sweep: top-1, top-2, top-3
=============================================

Purpose:
  H026 current: 14-asset (11-sector+BIL+GLD+TLT), top-1.
  H083 previously found top-2 worse than top-1 on the OLD 11-sector universe.

  With GLD and TLT now in the universe, the dynamics change fundamentally:
  - top-1: holds one asset at a time (either a sector OR a safe haven)
  - top-2: can simultaneously hold an equity sector AND a safe haven
           e.g., XLK (tech) + GLD, or XLE (energy) + TLT

  Top-2 may provide better risk-adjusted returns by combining momentum equity
  exposure with concurrent safe-haven hedging, rather than forcing a binary
  equity-vs-safety choice.
...
def fetch_ohlc(ticker, start, end):
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
                   ...
                   "h089","h090","h091","h092","h093","h094","h095"]:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h096_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h096_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H096 — H026 Top-N Sweep: top-1, top-2, top-3 (14-asset universe)")
print("="*80)
...
# ── [1] H026 top-N sweep ──────────────────────────────────────────────────────
print("\n[1] H026 top-N sweep …")
print(f"\n  {'Top-N':>6}  {'H026 IS':>8}  {'H026 OOS':>9}  {'H026 AltOOS':>12}  "
...
for n_hold in [1, 2, 3]:
    h026_v = build_rotation_monthly(H026_GLD_TLT, FULL_START, FULL_END, n_hold)
    cidx   = common_idx(h026_v, h045, xlk_r, smh_r, igv_r, h41)
    rd     = {"h041a":h41,"h026":h026_v,"h045":h045,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}
    ...
    print(f"  top-{n_hold:>1}      {s_is['sharpe']:>8.4f}  {s_oos['sharpe']:>9.4f}  "
          f"{s_ao['sharpe']:>12.4f}  {p_oos['sharpe']:>9.4f}  {p_ao['sharpe']:>11.4f}  "
          f"{p_oos['max_drawdown']*100:>6.2f}%  {ww:>7.3f}  "
          f"{'—' if n_hold==1 else ('✓' if both_up else '✗')}")
    results[n_hold] = {
        "h026_is": s_is["sharpe"], "h026_oos": s_oos["sharpe"],
        "h026_ao": s_ao["sharpe"], "port_oos": p_oos["sharpe"],
        "port_ao": p_ao["sharpe"], "maxdd": p_oos["max_drawdown"],
        "wf_worst": ww, "both_up": bool(both_up),
        "h026_r": h026_v, "cidx": cidx, "rd": rd
    }

# ── [2] Full cross-validation of winner ──────────────────────────────────────
winners = {k: v for k, v in results.items() if v.get("both_up", False)}
print(f"\n[2] Winners: {list(winners.keys()) if winners else 'NONE'}")

if winners:
    best_n = max(winners, key=lambda k: winners[k]["port_oos"] + winners[k]["port_ao"])
    best   = winners[best_n]
    cidx_b = results[1]["cidx"]
    rd_b   = results[1]["rd"]

    s_is  = stats(make_port(best["rd"], PROD_W, best["cidx"][is_mask(best["cidx"])]))
    s_oos = stats(make_port(best["rd"], PROD_W, best["cidx"][oos_mask(best["cidx"])]))
    s_ai  = stats(make_port(best["rd"], PROD_W, best["cidx"][ai_mask(best["cidx"])]))
    s_ao  = stats(make_port(best["rd"], PROD_W, best["cidx"][ao_mask(best["cidx"])]))
    wf    = run_wf(best["cidx"], best["rd"], PROD_W)
    ww    = min(wf) if wf else 0.0
    ...
    print(f"\n  {'Portfolio':26}  {'IS S':>7}  {'OOS S':>7}  {'AltIS S':>8}  "
          f"{'AltOOS S':>9}  {'CAGR':>7}  {'MaxDD':>8}  {'WF':>7}")
    print("  "+"-"*89)
    for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
        ("H094 baseline (top-1)", sb_is, sb_oos, sb_ai, sb_ao, wf_b),
        (f"H096 (top-{best_n})",  s_is,  s_oos,  s_ai,  s_ao,  wf)
    ]:
        print(f"  {lbl:26}  {s_i['sharpe']:>7.4f}  {s_o['sharpe']:>7.4f}  "
    ...
    print(f"  {'Year':>5}  {'H094 (top-1)':>12}  {'H096 (top-{:d})'.format(best_n):>14}  {'Delta':>7}")
    print("  "+"-"*52)
    cal = []
    neg_b = neg_n = 0
    for yr in range(2004, 2026):
        yib = cidx_b[cidx_b.year == yr]
        yin = best["cidx"][best["cidx"].year == yr]
        if len(yib) == 0 or len(yin) == 0: continue
        rb = float((1+make_port(rd_b, PROD_W, yib)).prod()-1)
        rn = float((1+make_port(best["rd"], PROD_W, yin)).prod()-1)
        if rb < 0: neg_b += 1
        if rn < 0: neg_n += 1
        print(f"  {yr:>5}  {rb*100:>11.2f}%  {rn*100:>13.2f}%  {(rn-rb)*100:>+6.2f}pp")
        cal.append({"year":yr,"h094_top1":round(rb,4),"h096_topN":round(rn,4)})
    print(f"  H094: {'ZERO' if neg_b==0 else neg_b} neg | H096: {'ZERO' if neg_n==0 else neg_n} neg")
    ...
    if confirmed:
        print(f"\n  *** H096 CONFIRMED — H026 top-{best_n} ***")
    else:
        print(f"\n  H096 not confirmed — H026 top-1 unchanged.")
else:
    cal = []
    confirmed = False
    best_n = 1
    print("\n[3] Summary …")
    print("  No top-N change passed dual-window criterion.")
    print("  H096 not confirmed — H026 top-1 unchanged.")

output = {
    "sweep": {k: {kk: vv for kk, vv in v.items() if kk not in ("h026_r","cidx","rd")}
              for k, v in results.items()},
    "winner_n": best_n,
    "calendar": cal,
    "confirmed": bool(confirmed),
}
out_path = RESULT_DIR / "h096_results.json"
