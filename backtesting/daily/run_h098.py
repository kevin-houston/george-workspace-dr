...
Purpose:
  H097 confirmed adding EWG (Germany) to H041a. H041a is now 15-asset.
  Europe is still underrepresented: only EWG + broad EFA.
  Also retry EWA (Australia) on the new EWG-enriched baseline — it was
  close in H097 (OOS 3.6106 < 3.6284 baseline but AltOOS 3.6463 < 3.6740).

  Candidates:
    EWU  — iShares MSCI United Kingdom (started 1996)
    EWL  — iShares MSCI Switzerland (started 1996; high-quality defensives)
    EWQ  — iShares MSCI France (started 1996)
    EWA  — iShares MSCI Australia (started 1996; retry on new baseline)
    EWC  — iShares MSCI Canada (started 1996; commodity/financials)

  H041A_EWG (15-asset): SPY/QQQ/TLT/GLD/IEF/EFA/EEM/BIL/EWJ/EWH/EWT/EWY/EWS/EPHE/EWG
...
H041A_EWG    = ["SPY","QQQ","TLT","GLD","IEF","EFA","EEM","BIL",
                "EWJ","EWH","EWT","EWY","EWS","EPHE","EWG"]
...
CANDIDATES = [
    ...
    ("+EWU+EWA",    ["EWU","EWA"]),
    ("+EWL+EWA",    ["EWL","EWA"]),
    ("+EWU+EWL+EWA",["EWU","EWL","EWA"]),
]

_PREFIXES = [f"h{i:03d}" for i in range(62, 98)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h098_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h098_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H098 — H041a Further Geographic Expansion: EWU, EWL, EWQ, EWA, EWC")
print("="*80)
...
print("\n[1] Building H041a baseline (H097: 15-asset with EWG) …")
h41_base = build_rotation_monthly(H041A_EWG, FULL_START, FULL_END, 1)
...
# ── [2] Candidate sweep ──────────────────────────────────────────────────────
print("\n[2] H041a candidate sweep (on EWG baseline) …")
...
if winners:
    ...
    for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
        ("H097 baseline (15-asset+EWG)", sb_is, sb_oos, sb_ai, sb_ao, wf_b),
        (f"H098 {best_label}",           s_is,  s_oos,  s_ai,  s_ao,  wf)
    ]:
    ...
    print(f"  {'Year':>5}  {'H097 baseline':>14}  {'H098 best':>10}  {'Delta':>7}")
    print("  "+"-"*52)
    neg_b = neg_n = 0
    for yr in range(2004, 2026):
        ...
        cal.append({"year":yr,"baseline":round(rb,4),"h098":round(rn,4)})
    print(f"  Baseline: {'ZERO' if neg_b==0 else neg_b} neg | H098: {'ZERO' if neg_n==0 else neg_n} neg")
    ...
    if confirmed:
        print(f"\n  *** H098 CONFIRMED — H041a expanded with {best_label} ***")
    else:
        print(f"\n  H098 not confirmed — H041a (15-asset+EWG) unchanged.")
else:
    print("\n[4] Summary …")
    print("  No candidate passed dual-window criterion.")
    print("  H098 not confirmed — H041a (15-asset+EWG) unchanged.")
...
out_path = RESULT_DIR / "h098_results.json"
