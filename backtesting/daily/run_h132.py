"""
H132 — IBS Expansion: Add XLY or IWM as 4th IBS Leg
=====================================================

Current IBS component: XLK (20%), SMH (8%), IGV (2%) = 30% total
All three are tech/semiconductor — highly correlated sector concentration.

H132 tests whether adding a non-tech IBS candidate diversifies the IBS
component and improves portfolio metrics:
  - XLY (Consumer Discretionary): high-beta, driven by consumer spending cycles
  - IWM (Russell 2000 Small Caps): high-vol, strong mean-reversion tendency

Approach:
  1. Tune IBS parameters for each candidate (grid search, IS 2008-2017)
  2. Test standalone IBS performance on each candidate
  3. Test adding best candidate to blend:
     - Existing IBS legs scaled to 80% of current weights (XLK: 0.16, SMH: 0.064, IGV: 0.016)
     - New candidate gets 6% weight
     - Total IBS remains 30%, rotation remains 70%
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 133)]

# IBS parameter grid (coarse search)
BUY_RANGE  = [0.10, 0.15, 0.20, 0.25, 0.30]
SELL_RANGE = [0.70, 0.75, 0.80, 0.85, 0.90]
HOLD_RANGE = [4, 5, 6, 7, 8]
GAP_RANGE  = [-0.015, -0.010, -0.005, 0.0, 0.005]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h132_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h132_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def ibs_stats(eq, start=None, end=None):
    r = to_monthly(eq)
    if start:
        r = r[start:]
    if end:
    ...
    r = r.dropna()
    if len(r) < 6:
        return {"sharpe": 0.0, "cagr": 0.0, "max_drawdown": -1.0, "cumul": 1.0}
    eq_m = (1 + r).cumprod()
    n_yr = len(r) / 12.0
    cagr = float(eq_m.iloc[-1]) ** (1/n_yr) - 1
    vol  = float(r.std(ddof=1)) * np.sqrt(12)
    sharpe = cagr / vol if vol > 0 else 0.0
    max_dd = float((eq_m / eq_m.expanding().max() - 1).min())
    return {"sharpe": round(sharpe, 4), "cagr": round(cagr, 4),
            "max_drawdown": round(max_dd, 4), "cumul": round(float(eq_m.iloc[-1]), 4)}


def tune_ibs(ohlc, is_start, is_end):
    """Grid search IBS params; maximize IS Sharpe."""
    best = {"sharpe": -999, "params": None}
    for buy, sell, hold, gap in product(BUY_RANGE, SELL_RANGE, HOLD_RANGE, GAP_RANGE):
        if buy >= sell - 0.20:
            continue
        eq = ibs_equity_curve(ohlc, buy, sell, hold, gap)
        s  = ibs_stats(eq, is_start, is_end)
        if s["sharpe"] > best["sharpe"] and s["max_drawdown"] > -0.25:
            best = {"sharpe": s["sharpe"], "params": (buy, sell, hold, gap)}
    return best["params"]


# ─────────────────────────────────────────────────────────────────────────────
# Rotation builders (same as H130/H131)
# ─────────────────────────────────────────────────────────────────────────────
...
def blend_vol_targeted(sub_rets, prod_w):
...
def main():
    print("=" * 80)
    print("H132 — IBS Expansion: XLY and IWM as 4th IBS Leg")
    print("=" * 80)
    ...
    for sym in candidates:
        cand_ohlc[sym] = fetch_ohlc(sym, FULL_START, FULL_END)
        print(f"  {sym} fetched  ({len(cand_ohlc[sym])} days)")

    # ── Tune IBS parameters for each candidate ──────────────────────────────
    print("\nTuning IBS parameters (IS 2008-2017 grid search) …")
    cand_params = {}
    for sym in candidates:
        print(f"  Tuning {sym} … ", end="", flush=True)
        params = tune_ibs(cand_ohlc[sym], IS_START, IS_END)
        cand_params[sym] = params
        buy, sell, hold, gap = params
        eq = ibs_equity_curve(cand_ohlc[sym], buy, sell, hold, gap)
        s_is  = ibs_stats(eq, IS_START, IS_END)
        s_oos = ibs_stats(eq, OOS_START)
        print(f"buy<{buy:.2f} sell>{sell:.2f} hold={hold} gap>{gap:.4f}  "
              f"IS Sharpe={s_is['sharpe']:.3f}  OOS Sharpe={s_oos['sharpe']:.3f}  "
              f"OOS MaxDD={s_oos['max_drawdown']*100:.1f}%")

    # ── Pre-compute rotation legs ────────────────────────────────────────────
    print("\nPre-computing rotation legs (H041a 3m, H026 12m, H045 3m) …")
    ...
    # ── Pre-compute current IBS legs ─────────────────────────────────────────
    print("Pre-computing current IBS legs …")
    ibs_rets = {}
    for sym, (buy, sell, hold, gap) in IBS_PROD.items():
    ...
    sub_base = {"h041a": h041a, "h026": h026, "h045": h045, **ibs_rets}
    port_base = blend_vol_targeted(sub_base, PROD_W)
    b_oos, b_alt = print_stats(port_base, "A) Baseline (H130 production)")

    # ── Test each candidate ──────────────────────────────────────────────────
    results = {}
    for sym in candidates:
        buy, sell, hold, gap = cand_params[sym]
        new_ibs = to_monthly(ibs_equity_curve(cand_ohlc[sym], buy, sell, hold, gap))

        # Scale existing IBS to 80% to make room for new leg (6% weight)
        new_w = dict(PROD_W)
        for k in ["XLK", "SMH", "IGV"]:
            new_w[k] = PROD_W[k] * 0.80
        new_w[sym] = 0.06

        sub_test = {"h041a": h041a, "h026": h026, "h045": h045,
                    **{k: ibs_rets[k] for k in ibs_rets}, sym: new_ibs}
        port = blend_vol_targeted(sub_test, new_w)
        label = f"B) + {sym} IBS (XLK→16%/SMH→6.4%/IGV→1.6%/{sym}→6%)"
        oos_s, alt_s = print_stats(port, label)
        results[sym] = (oos_s, alt_s)

    # ── Summary ──────────────────────────────────────────────────────────────
    print("\n" + "─" * 80)
    print("Comparison vs H130 baseline:\n")
    confirmed_any = False
    for sym, (oos_s, alt_s) in results.items():
        ...
        if d_oos > 0 and d_alt > 0:
            confirmed_any = True
        buy, sell, hold, gap = cand_params[sym]
        print(f"  {sym} (buy<{buy:.2f} sell>{sell:.2f} hold={hold} gap>{gap:.4f})")
    ...
    print("─" * 80)
    if confirmed_any:
        best = max(results, key=lambda k: results[k][0]["cumul"] + results[k][1]["cumul"])
        best_oos, best_alt = results[best]
        d_oos = best_oos["cumul"] - b_oos["cumul"]
        d_alt = best_alt["cumul"] - b_alt["cumul"]
        print(f"\n  VERDICT: CONFIRMED — best candidate: {best}")
        ...
        d_oos = best_oos["cumul"] - b_oos["cumul"]
        d_alt = best_alt["cumul"] - b_alt["cumul"]
        print(f"\n  VERDICT: NOT CONFIRMED — no candidate improves both OOS and AltOOS")
        print(f"  Best attempt: {best}  OOS Δ{d_oos:+.4f}  AltOOS Δ{d_alt:+.4f}")
