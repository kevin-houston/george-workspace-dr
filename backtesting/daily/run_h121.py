"""
H121 — Vol-Targeting on Top of H120 Rank Ensemble Signal
=========================================================

H120 baseline: rank ensemble (rank(3m)+rank(6m)+rank(12m)+rank(inv_vol)) with
fixed weights H041a=22%, H026=27%, H045=21% (+ IBS 30%).
H120 OOS: 8.4086, AltOOS: 19.5157, MaxDD -2.4%, NegYrs 0

H121 hypothesis: does vol-targeted weight scaling (H118) still add value when
applied on top of the improved rank ensemble signal (H120)?

H118 confirmed vol-targeting on H116 baseline: +0.09 OOS, +0.46 AltOOS.
H120 improved baseline significantly (+28% OOS). The question is whether
vol-scaling provides incremental gain on an already stronger signal base.

Variants tested (all using rank ensemble rotation signal):
  A) H120 baseline (rank ensemble + fixed weights)
  B) Vol-target all three rotation subs (clamp 0.5x–2x base)
  C) Risk parity: weight ∝ 1/vol (rotation total fixed at 70%)
  D) Vol-target H026 only (highest-vol sub)
  E) Vol-target H041a+H026 only (best variant in H118, IBS + H045 fixed)
"""
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 122)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h121_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h121_{ticker}_close_{start}_{end}.parquet"
...
# ── H120 rank ensemble rotation ───────────────────────────────────────────────

def build_rotation(tickers, start, end, n_hold=1, tsmom_filter=False):
    """Rank ensemble: rank(3m)+rank(6m)+rank(12m)+rank(inv_vol) — H120 signal."""
    closes = {}
    for t in tickers:
        try:
...
def print_stats(r, label):
    ...
    print(f"  {label:<42} IS={is_['sharpe']:.3f}/{is_['cagr']*100:.1f}%  "
...
def blend_vol_targeted(sub_rets, vol_target_keys):
    df = pd.DataFrame(sub_rets).dropna(how="all")
    long_vols = {}
    ...
    for i in range(len(df)):
        ...
        if i >= VOL_WINDOW:
            ...
            for k, rv in realized_vols.items():
                if rv > 0:
                    scalar = np.clip(target_vol / rv, VOL_CLAMP[0], VOL_CLAMP[1])
        ...
        month_ret = sum(float(df[key].iloc[i]) * w
                        for key, w in row_weights.items() if key in df.columns)
        combined.iloc[i] = month_ret

    return combined.dropna()


def blend_risk_parity(sub_rets):
    df = pd.DataFrame(sub_rets).dropna(how="all")
    rot_keys = ["h041a", "h026", "h045"]
    inv_vols = {}
    for k in rot_keys:
...
def main():
    print("=" * 72)
    print("H121 — Vol-Targeting on Top of H120 Rank Ensemble Signal")
    print("=" * 72)
    ...
    print(f"H120 reference: OOS=8.4086, AltOOS=19.5157")
    ...
    print("\nComputing rotation sub-strategies (H120 rank ensemble signal)…")
    ...
    b_oos, b_alt = print_stats(baseline, "A) H120 baseline (rank ensemble + fixed)")
    ...
    vt_eq_oos, vt_eq_alt = print_stats(vt_eq, "E) Vol-target H041a + H026 (H118 best)")
