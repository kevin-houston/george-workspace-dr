...
Purpose:
  H041a is now 19-asset (added EWG/EWQ/EWU/EWD/EWN in H097-H099).
  Remaining untested geographic universe:
    Southern Europe: EWP (Spain), EWI (Italy) — failed singletons in H099 on 16-asset baseline
    Commodity economies: EWA (Australia), EWC (Canada) — failed in H097/H098
    EM: EWZ (Brazil) — failed in H097; EWW (Mexico, starts 1996)

  On the new 19-asset baseline (OOS 3.758), the signal dynamics are different.
  Test these as pairs/groups; the H099 pattern suggests combos > singletons.

  Candidates:
    +EWP+EWI         (Southern Europe pair: Spain+Italy)
    +EWA+EWC         (Commodity pair: Australia+Canada)
    +EWP+EWI+EWA     (Southern Euro + Australia)
    +EWP+EWI+EWA+EWC (Quad: Southern Euro + commodity)
    +EWW              (Mexico singleton — different EM from rest)
    +EWZ+EWW          (EM Americas pair)
...
CANDIDATES = [
    ("+EWP+EWI",         ["EWP","EWI"]),
    ("+EWA+EWC",         ["EWA","EWC"]),
    ("+EWP+EWI+EWA",     ["EWP","EWI","EWA"]),
    ("+EWP+EWI+EWA+EWC", ["EWP","EWI","EWA","EWC"]),
    ("+EWW",             ["EWW"]),
    ("+EWZ+EWW",         ["EWZ","EWW"]),
    ("+EWP+EWI+EWW",     ["EWP","EWI","EWW"]),
]

_PREFIXES = [f"h{i:03d}" for i in range(62, 101)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h101_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h101_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H101 — H041a: Southern Europe + Commodity Economies")
print("="*80)
...
print("\n[1] Building H041a baseline (H099: 19-asset) …")
...
# ── [2] Candidate sweep ──────────────────────────────────────────────────────
print("\n[2] H041a candidate sweep (on 19-asset baseline) …")
print(f"\n  {'Candidate':>22}  {'H041a IS':>9}  {'H041a OOS':>10}  {'H041a AltOOS':>12}  "
      f"{'Port OOS':>9}  {'Port AltOOS':>11}  {'MaxDD':>7}  {'WF':>7}  {'Both↑':>5}")
print("  "+"-"*111)

results = {}
for label, extra in CANDIDATES:
    universe = H041A_FULL + extra
    ...
    print(f"  {label:>22}  {s_is['sharpe']:>9.4f}  {s_oos['sharpe']:>10.4f}  "
...
if winners:
    ...
    for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
        ("H099 baseline (19-asset)", sb_is, sb_oos, sb_ai, sb_ao, wf_b),
        (f"H101 {best_label}",       s_is,  s_oos,  s_ai,  s_ao,  wf)
    ]:
    ...
    print(f"  {'Year':>5}  {'H099 baseline':>14}  {'H101 best':>10}  {'Delta':>7}")
    print("  "+"-"*52)
    neg_b = neg_n = 0
    for yr in range(2004, 2026):
        ...
        cal.append({"year":yr,"baseline":round(rb,4),"h101":round(rn,4)})
    print(f"  Baseline: {'ZERO' if neg_b==0 else neg_b} neg | H101: {'ZERO' if neg_n==0 else neg_n} neg")
    ...
    if confirmed:
        print(f"\n  *** H101 CONFIRMED — H041a expanded with {best_label} ***")
    else:
        print(f"\n  H101 not confirmed — H041a (19-asset) unchanged.")
else:
    print("\n[4] Summary …")
    print("  No candidate passed dual-window criterion.")
    print("  H101 not confirmed — H041a (19-asset) unchanged.")
...
out_path = RESULT_DIR / "h101_results.json"
