"""
H090 — H045 Universe Expansion: ANGL, MBB, FLOT
=================================================

Purpose:
  H045 (bond rotation) current universe: SHY/IEI/IEF/TLT/TIP/HYG/LQD/BKLN/EMB/BIL
  Covers: duration spectrum, TIPS, HY, IG, floating/loans, EM, cash.

  Three potential additions with distinct return cycles:
  - ANGL: VanEck Fallen Angels HY (bonds recently downgraded from IG → distinct
    risk profile from HYG which holds all HY. Fallen angels historically outperform
    original-issue HY due to forced selling by IG-constrained investors)
  - MBB: iShares MBS ETF (agency mortgage-backed, ~30 yr pass-through duration but
    callable/prepayment risk creates distinct cycle from TLT/IEF)
  - FLOT: iShares Floating Rate Bond ETF (IG quality, sub-1yr effective duration,
    floats with LIBOR/SOFR — distinct from BKLN loans and BIL)

  Sweep structure: test each addition individually, then best combinations.
  H089 baseline: OOS 3.4340, AltOOS 3.3697, MaxDD -2.24%, WF 2.386

  Full cross-validation of best candidate(s) if dual-window confirmed.
"""
...
H045_BASE  = ["SHY","IEI","IEF","TLT","TIP","HYG","LQD","BKLN","EMB","BIL"]

# Candidates
H045_ANGL  = H045_BASE + ["ANGL"]   # fallen angels HY
H045_MBB   = H045_BASE + ["MBB"]    # mortgage-backed
H045_FLOT  = H045_BASE + ["FLOT"]   # floating rate IG
...
H045_CANDIDATES = {
    ...
    "+MBB":        H045_MBB,
    "+FLOT":       H045_FLOT,
    "+ANGL+MBB":   H045_BASE + ["ANGL","MBB"],
    "+ANGL+FLOT":  H045_BASE + ["ANGL","FLOT"],
    "+MBB+FLOT":   H045_BASE + ["MBB","FLOT"],
}
...
def fetch_ohlc(ticker, start, end):
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
                   ...
                   "h080","h081","h082","h083","h084","h085","h086","h087","h088","h089"]:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h090_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h090_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H090 — H045 Universe Expansion: ANGL, MBB, FLOT")
print("="*80)
...
print(f"\n  {'Candidate':14}  {'H045 IS':>8}  {'H045 OOS':>9}  {'H045 AltOOS':>12}  "
...
for name, tickers in H045_CANDIDATES.items():
    try:
        ...
        print(f"  {name:14}  {s_is['sharpe']:>8.4f}  {s_oos['sharpe']:>9.4f}  "
...
# ── [2] Full cross-validation of best confirmed candidates ────────────────────
...
if winners:
    ...
    for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
        ("H089 baseline", sb_is, sb_oos, sb_ai, sb_ao, wf_b),
        (f"H090 ({best_name})", s_is, s_oos, s_ai, s_ao, wf)
    ]:
        print(f"  {lbl:22}  {s_i['sharpe']:>7.4f}  {s_o['sharpe']:>7.4f}  "
    ...
    print(f"  {'Year':>5}  {'H089 base':>10}  {f'H090 ({best_name})':>14}  {'Delta':>7}")
    print("  "+"-"*47)
    cal = []
    neg_b = neg_n = 0
    for yr in range(2004, 2026):
        ...
        if rb < 0: neg_b += 1
        if rn < 0: neg_n += 1
        print(f"  {yr:>5}  {rb*100:>9.2f}%  {rn*100:>13.2f}%  {(rn-rb)*100:>+6.2f}pp")
        cal.append({"year":yr,"h089":round(rb,4),"h090":round(rn,4)})
    print(f"  H089: {'ZERO' if neg_b==0 else neg_b} neg yrs  |  H090: {'ZERO' if neg_n==0 else neg_n} neg yrs")
    ...
    print("  H090 not confirmed — H045 universe unchanged.")
...
out_path = RESULT_DIR / "h090_results.json"
