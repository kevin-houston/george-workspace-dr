"""
H087 — Full Cross-Validation: H041a+EWS (Singapore)
=====================================================

Purpose:
  H086's extended sweep found EWS (iShares MSCI Singapore) adds dual-window
  improvement on top of the H086 baseline (OOS 3.3958, AltOOS 3.2800):

    +EWS: Port OOS 3.4138 (+0.0180), AltOOS 3.3470 (+0.0670), WF 2.580 ✓

  EWS previously failed on the 10-asset H041a universe (H084). It passes now on
  the 12-asset universe — Singapore's financial hub role (DBS, OCBC, UOB) and
  its unique position as a USD-linked city-state provides return diversification
  once the full Pacific Rim cluster (Japan, HK, Taiwan, Korea) is assembled.

  Full cross-validation:
    [1] Scorecard — incremental breakdown (H086 base → H087)
    [2] Calendar year 2004-2025
    [3] WF 5-fold detail
    [4] H041a standalone: 12-asset+EWY vs 13-asset+EWS
    [5] H026 standalone (unchanged, top-1 confirmation)
    [6] Further sweep on 13-asset universe
...
# Further sweep candidates on 13-asset universe
EXTEND_EXTRA = {
    "+EWA":   H041A_EWS + ["EWA"],   # Australia (resource/commodity — control)
    "+EWM":   H041A_EWS + ["EWM"],   # Malaysia (ASEAN manufacturing)
    "+EPHE":  H041A_EWS + ["EPHE"],  # Philippines (ASEAN services/BPO)
}
...
def fetch_ohlc(ticker, start, end):
    for prefix in ["h062","h063","h064","h065","h066","h067","h068","h069","h070",
                   ...
                   "h080","h081","h082","h083","h084","h085","h086"]:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h087_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h087_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H087 — Full Cross-Validation: H041a+EWS (Singapore)")
print("="*80)
...
portfolios = {
    "H086 baseline":  (h41_ewy, "12-asset top-1"),
    "H087 (+EWS)":    (h41_ews, "13-asset top-1"),
}
...
base_d = scorecard["H086 baseline"]
new_d  = scorecard["H087 (+EWS)"]
print(f"\n  {'Year':>5}  {'H086 base':>10}  {'H087 (+EWS)':>11}  {'Delta':>7}")
print("  "+"-"*44)
neg_base = neg_new = 0
cal = []
for yr in range(2004, 2026):
    ...
    cal.append({"year":yr,"h086":round(rb,4),"h087":round(rn,4)})
print(f"  H086 baseline: {'ZERO' if neg_base==0 else neg_base} negative years")
print(f"  H087 (+EWS):   {'ZERO' if neg_new==0 else neg_new} negative years")
...
for label, h41 in [("12-asset+EWY top-1", h41_ewy), ("13-asset+EWS top-1", h41_ews)]:
    idx  = h41.index
...
# ── [6] Further sweep on 13-asset universe ────────────────────────────────────
print("\n[6] Further ASEAN/Pacific sweep on 13-asset+EWS (top-1) …")
print(f"\n  {'Candidate':14}  {'H041a OOS':>9}  {'H041a AltOOS':>12}  "
      f"{'Port OOS':>9}  {'Port AltOOS':>11}  {'MaxDD':>7}  {'WF':>7}  {'Both↑':>5}")
print("  "+"-"*97)

h41_ews_oos = stats(h41_ews[oos_mask(h41_ews.index)])["sharpe"]
h41_ews_ao  = stats(h41_ews[ao_mask(h41_ews.index)])["sharpe"]
new_base    = scorecard["H087 (+EWS)"]
s_new_oos   = new_base["oos"]["sharpe"]
s_new_ao    = new_base["ao"]["sharpe"]
print(f"  {'EWS base':14}  {h41_ews_oos:>9.4f}  {h41_ews_ao:>12.4f}  "
...
# ── Summary ───────────────────────────────────────────────────────────────────
print("\n[7] Summary …")
b = scorecard["H086 baseline"]
h = scorecard["H087 (+EWS)"]
...
confirmed = (h["oos"]["sharpe"] > b["oos"]["sharpe"] and
             ...
             h["wf_ok"])
if confirmed:
    print(f"\n  *** H087 CONFIRMED on BOTH OOS WINDOWS ***")
    print(f"  *** H041a: 13-asset+EWS top-1 ***")
else:
    print(f"\n  H087 not confirmed")
...
output = {
    ...
    "calendar": cal,
    "sec6_asean_sweep": sec6_results,
    "confirmed": bool(confirmed),
}
out_path = RESULT_DIR / "h087_results.json"
