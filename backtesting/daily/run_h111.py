...
Purpose:
  H026 has assembled the commodity complex (DBC, GDX, DBA, SLV, GLD) and bond
  spectrum (TLT, IEF, TIP, AGG). Remaining gaps:
  - IWM  (iShares Russell 2000, May 2000): small-cap US equity — distinct risk
           factor from large-cap SPY/QQQ (which are in H041a, not H026). H026's
           equity exposure is only via 11 sectors.
  - UNG  (United States Natural Gas, Apr 2007): pure natural gas play. DBC has
           ~8% natural gas but it's diluted. Pure UNG adds energy seasonality.
  - UUP  (Invesco DB US Dollar Index Bullish, Feb 2007): USD flight-to-safety
           signal. When USD strengthens, global risk assets typically suffer —
           the rotation model could use UUP as a cash-equivalent during crises.
  - CPER (United States Copper, Nov 2011): copper = industrial activity leading
           indicator. Different from DBC/DBA/GDX/SLV.
  - EWZ  (iShares MSCI Brazil, Jul 2000): EM Americas equity. H041a has Asia
           (Japan/HK/Taiwan/Korea/Singapore/Philippines) and Europe (Germany/
           France/UK/Sweden/Netherlands), but NO EM Americas. EWZ failed in H101
           in a combo, but not tested alone on the 21-asset H026 baseline.

  H026_FULL (21-asset) baseline, production weights 22%/27%/21%
...
CANDIDATES = [
    ...
    ("+IWM+EWZ",      ["IWM","EWZ"]),
    ("+UNG+EWZ",      ["UNG","EWZ"]),
    ("+IWM+UNG+UUP",  ["IWM","UNG","UUP"]),
    ("+IWM+UNG+EWZ",  ["IWM","UNG","EWZ"]),
]

_PREFIXES = [f"h{i:03d}" for i in range(62, 111)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h111_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h111_{ticker}_close_{start}_{end}.parquet"
...
print("="*80)
print("H111 — H026 Universe Expansion: IWM, UNG, UUP, CPER, EWZ")
print("="*80)
...
print("\n[1] Building H026 baseline (21-asset) …")
h026_base = build_rotation_monthly(H026_BASE, FULL_START, FULL_END, 1)
cidx_base = common_idx(h41, h026_base, h045, xlk_r, smh_r, igv_r)
rd_base   = {"h041a":h41,"h026":h026_base,"h045":h045,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}
...
header = (f"  {'Candidate':>16}  {'H026 IS':>8}  {'H026 OOS':>9}  {'H026 AltOOS':>12}  "
...
for label, extras in CANDIDATES:
    universe = H026_BASE + extras
    h026_c = build_rotation_monthly(universe, FULL_START, FULL_END, 1)
    cidx_c = common_idx(h41, h026_c, h045, xlk_r, smh_r, igv_r)
    rd_c   = {"h041a":h41,"h026":h026_c,"h045":h045,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}

    h026_is  = stats(h026_c[is_mask(h026_c.index)])
    h026_oos = stats(h026_c[oos_mask(h026_c.index)])
    h026_ao  = stats(h026_c[ao_mask(h026_c.index)])
    ...
    print(f"  {label:>16}  {h026_is['sharpe']:>8.4f}  {h026_oos['sharpe']:>9.4f}  "
          f"{h026_ao['sharpe']:>12.4f}  {port_oos['sharpe']:>9.4f}  "
          f"{port_ao['sharpe']:>11.4f}  {port_oos['max_drawdown']*100:>6.2f}%  "
          f"{ww:>7.3f}  {mark:>5}")

    sweep_results.append({
        "label": label, "extras": extras,
        "h026_is": h026_is["sharpe"], "h026_oos": h026_oos["sharpe"],
        "h026_ao": h026_ao["sharpe"],
...
if confirmed:
    ...
    print(f"\n  Best winner: {best['label']} "
          f"(OOS {best['port_oos']:.4f}, AltOOS {best['port_ao']:.4f})")

    universe_best = H026_BASE + best["extras"]
    h026_best = build_rotation_monthly(universe_best, FULL_START, FULL_END, 1)
    cidx_best = common_idx(h41, h026_best, h045, xlk_r, smh_r, igv_r)
    rd_best   = {"h041a":h41,"h026":h026_best,"h045":h045,"XLK":xlk_r,"SMH":smh_r,"IGV":igv_r}
    ...
    print(f"\n  {'Portfolio':38}  {'IS S':>7}  {'OOS S':>7}  {'AltIS S':>8}  "
          f"{'AltOOS S':>9}  {'CAGR':>7}  {'MaxDD':>8}  {'WF':>7}")
    print("  "+"-"*103)
    for lbl, s_i, s_o, s_a, s_ao_, wf_ in [
        ("H109 baseline (21-asset)", c_is, s_base_oos, c_ai, s_base_ao, wf_base),
        (f"H111 {best['label']}", s_is, s_oos, s_ai, s_ao, wf_b),
    ]:
        print(f"  {lbl:38}  {s_i['sharpe']:>7.4f}  {s_o['sharpe']:>7.4f}  "
    ...
    print(f"\n  *** H111 CONFIRMED — H026 expanded with {best['label']} ***")
...
out_path = RESULT_DIR / "h111_results.json"
