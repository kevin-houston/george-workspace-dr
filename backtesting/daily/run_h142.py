"""
H142 — H045 Top-N Retest on Current System
============================================

Tests whether H045 should hold top-1 or top-3 assets instead of top-2.

Context change from H083 (when top-2 was originally confirmed):
  - H045 universe: 9 assets → now 13 assets (+PCY, +MBB, +FLOT, +BIL)
  - New: 3m TSMOM filter at +1.0% threshold (H141)
  - New: full 4-term ensemble scoring (H134)

With the +1% threshold, the filtered pool is substantially smaller:
  - A (0% threshold) had 5 cash months, i.e. typically 12-13 pass
  - D (1% threshold) has 40 cash months, i.e. often only 5-8 assets pass
  - Selecting top-2 from 5-8 filtered assets is quite concentrated already
  - Top-1 might be better (get the best from a quality-screened pool)
  - Top-3 might add diversification without diluting quality

Baseline: H141 system (H026: 12m +5%, H041a: 3m +0.5%, H045: 3m +1.0%)

Variants:
  A) top-2 (current production)
  B) top-1
  C) top-3

Reference: H141 baseline OOS 27.6154, AltOOS 98.8229, Sharpe 4.981
"""
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 143)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h142_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h142_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def main():
    print("=" * 80)
    print("H142 — H045 Top-N Retest on Current System")
    print("=" * 80)
    print("H141 baseline: OOS 27.6154, AltOOS 98.8229, Sharpe 4.981\n")
    ...
    print("\nPre-computing H026 (+5%) and H041a (+0.5%) …")
    ...
    VARIANTS = [
        ("A) top-2 (current production)", 2),
        ("B) top-1",                       1),
        ("C) top-3",                       3),
    ]

    print("\nBuilding H045 variants (3m TSMOM +1% threshold) …")
    h045_series = {}
    for label, n in VARIANTS:
        ...
        h045_series[tag] = build_rotation_tsmom(
            ...
            n_hold=n, tsmom_lookbacks=[3], tsmom_threshold=H045_TSMOM_THRESHOLD
        )
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H141 baseline (A = top-2):\n")
    confirmed_any = False
    for label, n in VARIANTS:
        if label.startswith("A)"):
            continue
    ...
    if confirmed_any:
        ...
        print(f"\n  VERDICT: NOT CONFIRMED — top-2 remains optimal for H045")
    print()
