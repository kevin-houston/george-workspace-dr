"""
H147 — H026 Weight Upper Bound Search (34%–46%)
================================================

H146 confirmed monotonic improvement from 30% to 34%. The pattern has not
hit a ceiling — every pp increase has confirmed in both OOS windows. The
question is: where does the Pareto frontier break? At what H026 weight does
diminishing returns start (OOS gain per pp H026 shrinks) or reversal occurs?

Upper limit analysis: at 40% H026, H041a and H045 would be 15% each (21%
of 70% rotation). At 46%, they'd be 12% each. Below ~10% these sub-strategies
provide minimal diversification value.

Tests (H041a = H045 = (70% - H026) / 2 each):
  A) H026=34%, H041a=18%, H045=18%  — H146 deployed baseline
  B) H026=36%, H041a=17%, H045=17%
  C) H026=38%, H041a=16%, H045=16%
  D) H026=40%, H041a=15%, H045=15%
  E) H026=43%, H041a=13.5%, H045=13.5%
  F) H026=46%, H041a=12%, H045=12%

Baseline: H146 system — OOS 38.4280, AltOOS 147.4401, Sharpe 4.653
"""
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 148)]

# (label, h041a_w, h026_w, h045_w) — all sum to 0.70
VARIANTS = [
    ("A) H026=34%, H041a=18%, H045=18% (H146)",  0.18,   0.34,   0.18),
    ("B) H026=36%, H041a=17%, H045=17%",          0.17,   0.36,   0.17),
    ("C) H026=38%, H041a=16%, H045=16%",          0.16,   0.38,   0.16),
    ("D) H026=40%, H041a=15%, H045=15%",          0.15,   0.40,   0.15),
    ("E) H026=43%, H041a=13.5%, H045=13.5%",      0.135,  0.43,   0.135),
    ("F) H026=46%, H041a=12%, H045=12%",          0.12,   0.46,   0.12),
]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h147_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h147_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def main():
    print("=" * 80)
    print("H147 — H026 Weight Upper Bound Search (34%–46%)")
    print("=" * 80)
    print("H146 baseline: OOS 38.4280, AltOOS 147.4401, Sharpe 4.653\n")
    ...
    print("\nPre-computing rotation legs (H146 full system) …")
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H146 baseline (A = 34% H026):\n")
    confirmed_any = False
    for label, w041a, w026, w045 in VARIANTS:
        if label.startswith("A)"):
            continue
    ...
    if confirmed_any:
        ...
        print(f"\n  VERDICT: NOT CONFIRMED — 34% H026 weight is the ceiling")
    print()
