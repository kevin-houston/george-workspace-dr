"""
H143 — H026 Vol-Target Level Refinement Post-Threshold
=======================================================

H026 vol-target was set at 15% in H122 on the original 0% TSMOM threshold
system. H139 raised the threshold to +5%, fundamentally changing H026's
composition: only sectors with confirmed strong 12m momentum (>5%) are held.

These higher-quality, trend-confirmed sectors may have:
  - Higher expected return (they're in strong uptrends)
  - Slightly different realized volatility profile
  - Different optimal vol-scaling behavior

With a 15% target: when H026's realized 6m vol is 10%, scalar=1.5x (scale up).
                   When realized 6m vol is 20%, scalar=0.75x (scale down).
                   When realized 6m vol is 30%, scalar=0.5x (max scale down).

Higher target (20%) would scale down less during moderate-vol periods, allowing
more exposure when sectors are in confirmed uptrends. Lower target (12%) would
scale down more, reducing risk further.

Tests H026 vol-target at 12%, 15% (current), 18%, 20%, 25%.

Baseline: H142 system (all threshold + top-1 upgrades applied)

Reference: H142 baseline OOS 27.8759, AltOOS 99.2888, Sharpe 4.979
"""
...
_PREFIXES = [f"h{i:03d}" for i in range(62, 144)]
    ...
    for prefix in _PREFIXES:
        ...
        if cp.exists():
            ...
            if all(c in df.columns for c in ["open","high","low","close"]):
                return df
    cp = CACHE_DIR / f"h143_{ticker}_ohlc_{start}_{end}.parquet"
...
def fetch_daily_close(ticker, start, end):
    ...
    cp = CACHE_DIR / f"h143_{ticker}_close_{start}_{end}.parquet"
    raw = yf.download([ticker], start=start, end=end, auto_adjust=True, progress=False)
    if isinstance(raw.columns, pd.MultiIndex):
...
def blend_vol_targeted(sub_rets, vol_target_h026):
...
def main():
    print("=" * 80)
    print("H143 — H026 Vol-Target Level Refinement Post-Threshold")
    print("=" * 80)
    print("H142 baseline: OOS 27.8759, AltOOS 99.2888, Sharpe 4.979\n")
    ...
    print("\nPre-computing H026 (+5% threshold), H041a (+0.5%), H045 (top-1, +1%) …")
    ...
    VARIANTS = [
        ("A) H026 vol-target=15% (current)", 0.15),
        ("B) H026 vol-target=12%",            0.12),
        ("C) H026 vol-target=18%",            0.18),
        ("D) H026 vol-target=20%",            0.20),
        ("E) H026 vol-target=25%",            0.25),
    ]
    ...
    for label, vt in VARIANTS:
        port = blend_vol_targeted(sub_rets_base, vol_target_h026=vt)
    ...
    print("\n" + "─" * 80)
    print("Comparison vs H142 baseline (A = 15% vol-target):\n")
    confirmed_any = False
    for label, vt in VARIANTS:
        if label.startswith("A)"):
            continue
    ...
    if confirmed_any:
        ...
        print(f"\n  VERDICT: NOT CONFIRMED — 15% vol-target remains optimal for H026")
    print()
