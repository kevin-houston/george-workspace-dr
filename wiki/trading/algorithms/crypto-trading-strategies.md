...
- [Multi-Agent LLM Trading](multi-agent-llm-trading.md) — LLM-as-filter architecture; PortBench findings

## LLM as signal not allocator (PortBench 2026)

PortBench benchmark (arXiv:2604.14199, 2026): 90% of LLMs fail to beat equal-weight portfolio allocation across asset classes including crypto. Key implication for crypto strategy design: use LLMs as filter/signal layer (e.g. regime detection, narrative sentiment) rather than as direct allocation agents. H318 (meta-agent ETF rotation selector) uses LLM as filter with fixed-weight allocation for this reason.