...
h168_status: IN-PROGRESS (2026-05-05, v2) — PEAD Speaker-Weighted FinBERT: run_h168.py WRITTEN + updated. Data: AlphaVantage EARNINGS_CALL_TRANSCRIPT API (25 req/day free tier). AV format: speaker-segmented JSON {speaker, title, content, sentiment}. Weights: Analyst 49%, CFO 30%, Executive 16%, Other 5% (arXiv:2604.13260 Table 4). SCRIPT UPDATES (2026-05-05): (1) GAP_THRESH changed 0.05→0.03 to match H163 (~203 expected events vs 101 at 5%); (2) earnings calendar filter added via yfinance earnings_dates (covers April 2020+; ~12% IS events in Jan-Mar 2020 will be missed); (3) rate-limit sentinel bug fixed — no longer saves [] sentinel on rate-limit hit, uses global _AV_RATE_LIMITED flag to stop pass cleanly. TRANSCRIPT CACHE STATUS: 22 files cached (Q4 2019 earnings calls, downloaded before earnings filter was added — these events no longer in filtered universe; cache retained but will be unused). DOWNLOAD PASSES: Pass 1 (2026-05-05 00:05 UTC, 22 fetches) = 22 Q4 2019 transcripts. Pass 2 (2026-05-05 00:17 UTC) = rate-limited immediately (quota exhausted). First valid events in filtered universe: TSLA 2020-07-23, AMD 2020-07-24, etc. (Q2 2020 earnings). NEXT PASS: run after UTC midnight May 6 when AV quota resets. BLOCKERS: (1) AV rate limit 25/day → ~9 days to download 203 events; (2) H163 IS signal TBD — if IS signal is near-zero, H168 may be deprioritized in favor of H171 (GPT-4o-mini, $0.48 total cost). Script: backtesting/daily/run_h168.py.
...
h163_status: RUNNING (2026-05-05, run 3 of 3) — PEAD-NLP: FinBERT sentiment filter. PID 416, started 23:14 CDT May 4. Progress: 140/203 events scored (68%). IS cache: 137 text files. OOS (2024+) events expected to have 0 FinBERT scores (EDGAR coverage gap for recent filings). Prior run results (20:52 May 4): BLOCKED — OOS with FinBERT scores: 0; baseline OOS: n=85 WR=57.6% MeanRet=1.95%. Current run IS analysis will show: (a) FinBERT score distribution on 112 IS events; (b) IS filtered WR vs baseline WR — key signal strength indicator. If IS FinBERT score provides no IS lift (WR no better than 57-60% filtered vs unfiltered), H168 speaker-weighted approach should be deprioritized. Script: backtesting/daily/run_h163.py. EXPECTED VERDICT: BLOCKED (OOS coverage unavailable); IS analysis in stdout log.
