...
h203_status: QUEUED (2026-05-16) — Risk-Parity Multi-Strategy Blend. Hypothesis: HRP blend of H026 (ETF sector rotation, Sharpe ~1.2 2021-26) + H191 BAB (Sharpe 1.367) + H198 6-1m momentum (Sharpe 1.174) + H201 TOM (Sharpe 0.740) outperforms best single strategy (H192-D OOS Sharpe 1.367) with better drawdown. Method: Hierarchical Risk Parity (HRP) via PyPortfolioOpt or Riskfolio-Lib. Monthly rebalance. Ledoit-Wolf shrinkage covariance. 24-month rolling lookback. IS: 2018–2022, OOS: 2023–2026. Confirm: OOS Sharpe > 1.367 (best single strategy) AND OOS MaxDD < -15%. Secondary: check all 4 strategies contribute positive weight (no strategy fully zeroed by HRP). Source: arXiv:2503.01886 (HRP outperforms MVO for multi-strategy blends); wiki/trading/tools/portfolio-optimization.md. Dependencies: pip install pyportfolioopt riskfolio-lib skfolio. Script: backtesting/daily/run_h203.py.
h202_status: QUEUED (2026-05-16) — ML-Enhanced Cross-Sectional Momentum with Bias Correction. Hypothesis: XGBoost with bias elimination mask outperforms simple 6-1m return rank (H198 baseline, OOS Sharpe 1.174) on the same 30-stock S&P 500 universe. Source: arXiv:2507.07107 — bias mask contributes +0.44 Sharpe-point (first-order driver, exceeds model architecture choice). Features: 6-1m momentum rank, beta, vol, sector dummies, prior month return (as reversal feature). Bias mask: exclude stocks where the rank signal is likely spurious (e.g., stocks with imputed/missing price history, very high bid-ask, low liquidity days). IS: 2013–2020, OOS: 2021–2026. Confirm: OOS Sharpe > 1.174 + 0.3 = 1.474 (H198 + conservative fraction of claimed +0.44 bias-mask lift). Secondary: Corr(H202, H198) < 0.7 (if XGBoost is just re-ranking the same stocks, no portfolio value). Dependencies: pip install xgboost scikit-learn. Script: backtesting/daily/run_h202.py.
