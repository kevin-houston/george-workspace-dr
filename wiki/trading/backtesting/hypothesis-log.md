h481_status: STAGED (2026-07-30) — FinDPO Continuous Sentiment Scoring Upgrade for H174 PEAD Pipeline. Source: arXiv:2507.18417 (Jul 2026) 'FinDPO: Financial Sentiment Analysis for Algorithmic Trading through Preference Optimization of LLMs'. DPO-aligned LLM with logit-to-score conversion produces continuous calibrated probabilities vs FinBERT's discrete classes; 11% accuracy improvement over supervised fine-tuning; Sharpe 2.0 at 5bp tx costs. H174 uses FinBERT with binary threshold (≥0.18) — FinDPO continuous scoring enables finer-grained gating and position sizing. Variants: A (FinBERT × FinDPO ensemble, product > 0.05), B (FinDPO ≥ 0.25 alone), C (FinDPO continuous score → position sizing), D (FinBERT ≥ 0.18 AND FinDPO > 0.50 double-filter). IS: 2022-2023, OOS: 2024-2026. Gate: OOS WR > 81.8% AND MeanRet > 6.89% AND n ≥ 20. CAUTION: verify FinDPO model name on HuggingFace/PyPI before install (hallusquatting defense); run pip-audit after any new model install. Script: backtesting/daily/run_h481_findpo_sentiment_h174.py. Medium risk. NOTE: Numbered H481 to avoid conflict with existing run_h465_llm_merger_arb.py.
h480_status: STAGED (2026-07-30) — Two-Stage PEAD with EarningsInOne — Numeric Surprise at Open + Qualitative ECT at Next-Day Open. Source: arXiv:2606.29734 (Ding Yu et al., Jun 2026) 'Fast Numbers, Slow Language: Bridging Quantitative and Qualitative Earnings Signals'. EARNINGSINONE corpus (S&P 1500, 2022-2025) demonstrates clean speed separation: numeric EPS/revenue surprise peaks at announcement and decays by next open, while qualitative ECT sentiment peaks the FOLLOWING trading day. H174 currently conflates both timing layers. H480 proposes two timed entry layers: L1 (numeric: EPS ≥ 0.02 → enter at open, hold 1d); L2 (qualitative: FinBERT ≥ 0.18 → enter next morning, hold 20d). Variants: A (L1 numeric only, 1d hold), B (L2 qualitative only, 20d hold — same as H174), C (L1+L2 sequential: enter 1d at open, extend to 20d if ECT confirms), D (L1 AND L2 gate: only enter 20d if BOTH numeric ≥ 0.02 AND FinBERT ≥ 0.18). IS: 2022-2023, OOS: 2024-2026. Gate: OOS WR > 81.8% AND MeanRet > 6.89% (H174 parity on both metrics). Prior art: H174 CONFIRMED. Script: backtesting/daily/run_h480_earningsinone_pead.py. Low risk. NOTE: Numbered H480 to avoid conflict with existing run_h464_stn_tgat_h198.py.
h479_status: STAGED (2026-07-30) — OB Filter on H411 Var B — SMC Order Block Applied to Real H-Series Champion (OOS 4.825). Source: Internal — follows H344 (H198+OB: 1.174→3.396) and h476_status (H417+OB: 0.383→1.929) pattern. H411 Var B (1/price rank × 20d drift gate, top-2, NASDAQ_30) is the actual H-series OOS champion at 4.825 Sharpe, MaxDD -1.2%. OB filter consistently lifts Sharpe 2-5× across NASDAQ momentum families (H343, H344, H345, H346 all confirmed). If same improvement applies, H479 could reach OOS 9-10 — highest-priority backtest in pipeline. Variants: grid search on OB params window=[20,30], min_filter=[1,2], swing_len=[3,5]; candidate_n=5, select top-2 with OB. IS: 2013-2020, OOS: 2021-2026. Gate: OOS Sharpe > 4.825 (H411 Var B) AND MaxDD improvement ≥ 0.5pp. Data: 30-stock NASDAQ universe + OHLCV cached from H343/H344/H476. Script: backtesting/daily/run_h479_ob_h411_varb.py. Low risk. NOTE: Numbered H479; H478 is assigned to Golden Criterion (h478_status).
h478_status: STAGED (2026-07-30) — Golden Criterion Adaptive Equal-Weight vs Optimized Blend for H026 ETF Rotation. Source: arXiv:2607.11054 (Feng, Huang, Wang, Zhang, Jul 2026) 'When and Why Naive Diversification Works: A Simple Diagnostic Strategy'. Key insight: equal weighting (1/N) is minimum-variance optimal when forecast-error covariance matrix has uniform eigenstructure (the "Golden Criterion"). Distance metric D = std(λ)/mean(λ) where λ are eigenvalues of trailing 12m monthly return covariance of top-5 ETFs. When D < IS-calibrated threshold → equal-weight top-3; when D ≥ threshold → top-1. Directly addresses H026 OOS degradation (0.785 vs 1.2 IS) by providing principled signal-quality detection. Variants: A (Golden Criterion gate — top-1 when D > D_threshold, EW top-3 when D ≤ D_threshold), B (D threshold at 50th IS percentile, more aggressive switching), C (always EW top-3), D (always EW top-5), E (H026 canonical top-1 baseline). IS: 2008-2017, OOS: 2018-2026. Gate: OOS Sharpe > 2.610 (H346 OB-gated baseline) AND MaxDD improvement vs -5%. Script: backtesting/daily/run_h478_golden_criterion_h026.py. Medium risk.
h477_status_new: RESOLVED → see H496 below (NOT CONFIRMED, 2026-08-08). STAGED (2026-07-30) — Network-Herding Momentum-Reversal Transition Signal on H198 Stock Universe. Source: arXiv:2607.27063 (Jul 2026) 'Herding, Momentum, and Reversal in China's A-Share Market: An Agent-Based Network Model with Information Diffusion'. Local herding creates positive price autocorrelation (momentum phase 3-9 months); as information diffuses to all network nodes, late adopters realize overpricing and reversal ensues (onset 9-18 months). Momentum streak signal: count consecutive months each stock has ranked in top-6 by 6-1m momentum. Long-streak stocks (6+ months) approaching network saturation — reversal risk elevated. Variants: A (exclude stocks with 6+ consecutive months in top-6 momentum rank), B (penalize score: adj_score = raw_score × (1 - 0.05 × min(streak, 6))), C (overweight NEW entrants to top-6: double weight vs incumbents), D (momentum reversal gate: route to short if stock in top-6 for 9+ months), E (H198 baseline 6-1m top-6 sanity check). IS: 2013-2020, OOS: 2021-2026. Gate: OOS Sharpe > 1.174 (H198 baseline) AND MaxDD improvement. Script: backtesting/daily/run_h477_herding_momentum_reversal.py. Medium risk. NOTE: This h477 entry (STAGED 2026-07-30) is distinct from the earlier h477 run below (NOT CONFIRMED, OB filter on H417 60-stock universe, 2026-07-29).
h476_status_new: RESOLVED → see H497 below (NOT CONFIRMED, 2026-08-08). STAGED (2026-07-30) — Split-Session Cluster GARCH Overnight/Intraday Tail Decomposition as IBS Entry Filter. Source: arXiv:2607.03669 (Chen, Hansen, Tong, Jul 2026) 'Split-Session Cluster GARCH for Overnight and Intraday Returns: The Role of Tail Heterogeneity'. Overnight returns have heavier tails (ν ≈ 3.8) than intraday (ν ≈ 5.2); fat-tail overnight days (news shocks) generate trend continuation not mean-reversion. IBS strategy (H112, 30% production portfolio) should exclude fat-tail overnight days from entry. overnight_rv = (log(open_t / close_{t-1}))²; overnight_rv_rank = rolling 252d percentile. Entry condition: (ibs < 0.2) AND (overnight_rv_rank < 0.8). Variants: A (binary gate: skip when overnight_rv_rank > 0.8), B (continuous: scale IBS position by 1 - overnight_rv_rank), C (anti-contrarian: ONLY enter when overnight_rv_rank < 0.2), D (H112 IBS baseline). IS: 2015-2020, OOS: 2021-2026. Gate: OOS Sharpe > 2.129 (H112 IBS baseline) AND MaxDD improvement vs -18%. Script: backtesting/daily/run_h476_split_session_garch_ibs.py. Medium risk. NOTE: This h476 entry (STAGED 2026-07-30) is distinct from the earlier h476 run below (NOT CONFIRMED, OB filter on H417 60-stock universe, 2026-07-29).
h477_status: NOT CONFIRMED (2026-07-29) — H417 Signal Parameter Sensitivity on 60-Stock Combined Universe. Gate: OOS Sharpe > 5.855 (NOTE: this gate was set against inflated pre-correction H417 numbers; actual H417 Var C corrected OOS = 0.383). IS: 2013-2020, OOS: 2021-2026. Universe: COMBINED_60 (NASDAQ_30 + SP500_NTECH). Signal: 1/price rank × drift_mask. RESULTS — Phase 1 (TOP_N sensitivity, w=20, t=0.60): top-1 OOS 0.703, top-2 OOS 0.508, top-3 OOS 0.383 (H417 baseline), top-4 OOS 0.305, top-5 OOS 0.264. Phase 2 (drift window, top-3, t=0.60): w=10 OOS 0.796, w=15 OOS 0.670, w=20 OOS 0.383 (H417), w=25 OOS 0.483, w=30 OOS 0.721. Phase 3 (drift threshold, top-3, w=20): t=0.50 OOS 1.026 (best!), t=0.55 OOS 0.713, t=0.60 OOS 0.383 (H417), t=0.65 OOS 0.541, t=0.70 OOS 0.789. Best combination (top-2, w=10, t=0.50): OOS 1.038, MDD -35.4%, 1 neg year. NOT CONFIRMED: best OOS 1.038 << gate 5.855. KEY FINDINGS: (1) H417's drift threshold t=0.60 is NOT optimal — t=0.50 improves OOS from 0.383 → 1.026 (2.7× lift at same w=20). (2) Shorter drift windows (w=10) outperform w=20 used in H417. (3) top-N concentration hurts in this family: top-1 > top-2 > top-3 pattern holds — H417 chose wrong top-N. (4) CRITICAL: CLAUDE.local.md H417 entry states 'Var C 60-stock combined OOS 5.855 = NEW H-SERIES RECORD' — THIS IS FROM PRE-CORRECTION RUNS; actual corrected h417_results.json shows Var C OOS 0.383. Real H-series record is H411 Var B at OOS 4.825 (value×drift top-2). Correction needed in CLAUDE.local.md. Script: backtesting/daily/run_h477.py. Results: backtesting/results/h477_results.json.
h476_status: NOT CONFIRMED (2026-07-29) — OB Filter on H417 60-Stock Combined Universe. Gate: OOS Sharpe > 5.855 (same inflated benchmark caveat as H477). Baseline (H417 Var C replication): OOS 0.383, MDD -46.9% — confirms corrected H417 numbers. OB grid (8 combos: window=[20,30], min_filter=[2,3], swing_len=[3,5]): best = w20/mf2/sl3 → OOS 1.929, MDD -23.4%, cash 22.2%, 1 neg year. IS: 2013-2020, OOS: 2021-2026. RESULTS full grid: w20/mf2/sl3=1.929; w30/sl5/mf2=1.596 MDD-13.8%; w30/sl3/mf3=1.715 MDD-12.8%; w30/sl5/mf3=1.720 MDD-13.3%; all others 0.988-1.339. NOT CONFIRMED: best OOS 1.929 < gate 5.855. KEY FINDINGS: (1) OB filter provides 5× Sharpe improvement over no-filter baseline (0.383 → 1.929) by routing to cash 22% of months. (2) Best OB params (w20/mf2/sl3) match H344 best params on 30-stock universe. (3) OOS annual: 2021=+155%, 2022=+22.9%, 2023=+36.3%, 2024=+62.5%, 2025=+58.0%, 2026=-20.1% — exceptional OOS returns with 1.929 Sharpe due to high monthly vol. (4) OB adds real value but the 60-stock universe's H417 signal baseline is fundamentally weak (0.383) compared to H411 Var B (4.825); OB lifts to 1.929 but remains below H411. (5) MinFilter=2 (lenient) consistently outperforms MinFilter=3 — the OB filter overshoots if too strict on 60-stock universe with many passing each month. NEXT: Test H478 = OB filter on H411 Var B universe (30-stock H198, true champion baseline 4.825). Script: backtesting/daily/run_h476.py. Results: backtesting/results/h476_results.json.
h475_status: STAGED (2026-07-28) — ML Multi-Factor Cross-Sectional with Upstream Contamination Bias Correction on H198 Universe. Source: arXiv:2507.07107 (Du, Jul 2025) 'Machine Learning Enhanced Multi-Factor Quantitative Trading: A Cross-Sectional Portfolio Optimization Approach with Bias Correction'. Key insight: rolling-window factor pipelines suffer 'upstream contamination' when price-limit or stale-price observations inflate IC by 18% and reduce realized Sharpe by 0.44 points. Fix: flag contaminated observations before factor computation. US analog: ex-dividend date price drops, corporate action adjustments, earnings halt days. Contamination flagging: exclude days where |return| > 15%. Alpha101 factor selection: 17 LASSO-surviving signals from H380 cross-market transfer. LightGBM ranker: same hyperparams as H320 (n_estimators=200, max_depth=5, lr=0.05). Cross-sectional neutralization: demean by GICS sector within monthly cross-section. Variants: A (contamination-flagged momentum baseline), B (Var A + LightGBM ranker), C (Var B + GICS neutralization), D (Var C + GBM data augmentation), E (H198 baseline, sanity check). IS: 2013-2020, OOS: 2021-2026. Gate: OOS Sharpe > 1.174 (H198 baseline) AND MaxDD improvement >= 2pp. CAUTION: 30-stock universe thin for ML — high overfitting risk; expand to H417 60-stock if Var B fails. Script: backtesting/daily/run_h475_ml_multifactor_bias_correction.py. Medium risk.
h474_status: STAGED (2026-07-28) — 8-Specialist LLM Pipeline (Fin-Analyst Architecture) as PEAD Pre-Filter. Source: arXiv:2607.12233 (Rashid et al., Jul 2026) 'Fin-Analyst at FinMMEval 2026 Task 3: A Live Hybrid Trading Agent with LLM Specialists and Rule-Based Signals'. Winner of FinMMEval 2026 Task 3; 8 parallel LLM specialists (news, SEC filings, fundamentals, analyst forecasts, technical, social sentiment, macro, options) aggregated by Meta-Agent. TSLA result: +13.51% return, +28.33pp vs buy-and-hold, Sharpe 4.10, 88% win rate. H474 ports to H174 PEAD as 3-specialist minimum viable architecture: Specialist-1 (8-K FinBERT scorer), Specialist-2 (earnings call KPI extraction from H471), Specialist-3 (analyst revision signal from FMP). Meta-Agent: composite = w1*s1 + w2*s2 + w3*s3. Variants: A (equal weights 1/3, 1/3, 1/3), B (rolling 6m Brier score reweighting), C (2-specialist: FinBERT + analyst revision only, no transcript), D (H174 baseline). IS: 2022-2023, OOS: 2024-2026. Gate: OOS WR >= 0.818 AND n >= 15 AND MeanRet >= 6.89% (H174 parity). CAUTION: H247 BLOCKED (FMP transcript requires Professional plan) → start with Var C. Coverage risk: H168 showed 26.5% transcript OOS coverage. Script: backtesting/daily/run_h474_fin_analyst_pead_prefilter.py. Medium risk.
h473_status: STAGED (2026-07-28) — Neural Predicates for Data-Driven Black-Litterman Views on Production ETF Portfolio. Source: arXiv:2607.20533 (Florencio, Jul 2026) 'Grounding Investor Views: Neural Predicates in the Black-Litterman Model'. Replaces subjective BL view specification with compositional neural predicates: lower-level predicates (fundamental/technical signals) compose into higher-level market stances; view confidence derived from predicate output distributions (entropy-based Omega). Portfolio weights = shrinkage blend of market-cap equilibrium + neural-view signal. H026 adaptation: predicate inputs [r_12m, r_3m, r_1m, VIX_normalized, above_200MA] → P(outperform); BL posterior weights replace H026 top-1 allocation. Expected benefit: better diversification when momentum signal is ambiguous. Variants: A (momentum-only neural predicates), B (momentum + macro predicates: VIX, SPY 200MA, yield curve), C (Var B + H301 SPY 200MA safety overlay → BIL when SPY < 200MA), D (equal-weight BL sanity check). IS: 2008-2020, OOS: 2021-2026. Gate: OOS Sharpe > 2.610 (H346 OB-filter baseline) AND MaxDD not worse than -5%. Requires PyPortfolioOpt + PyTorch. Script: backtesting/daily/run_h473_neural_predicates_bl.py. Medium risk.
h472_status: STAGED (2026-07-28) — SciPhy Reinforcement Learning for ETF Portfolio Optimization via HJB-PINN. Source: arXiv:2607.15195 (Halperin & Itkin, Jul 2026) 'SciPhy Reinforcement Learning for Portfolio Optimization'. Converts portfolio optimization to HJB PDE projected onto historical data paths (pathwise Hamilton-Jacobi equation) — no iterative RL simulation loops. Solved via PINN in single offline sweep; control variable = discrete target holding. Validated on 14-asset ETF universe with substantial OOS Sharpe improvement over static and myopic baselines. Execution costs modeled via quadratic price impact; continuous-time with monthly steps. H026/H041a/H045 adaptation: replace top-1 selection with SciPhyRL-derived optimal portfolio weights; H026 12m momentum signal as input to HJB state representation. PINN architecture: 3-layer MLP [state_dim+1 → 128 → 64 → n_assets]. Variants: A (SciPhyRL on H026 25-asset universe), B (SciPhyRL on H045 13-asset bond universe), C (SciPhyRL on H041a 19-asset universe), D (myopic mean-variance baseline). IS: 2008-2017, OOS: 2018-2026. Gate: OOS Sharpe > 2.5 AND MaxDD not worse than -5%. CAUTION: requires PyTorch; paper's GitHub not yet released; 25-asset universe larger than paper's 14-asset test — may need regularization tuning. Script: backtesting/daily/run_h472_sciphy_rl_etf_portfolio.py. Medium risk.
...
h456_status: STAGED (2026-07-27) — Adaptive Financial Transformer with Regime-Gated Attention for H198 Cross-Sectional Ranking. Source: arXiv:2606.29347 (Sarkar, Jun 2026) 'Adaptive Financial Transformer with Regime-Gated Attention for Stock Return Prediction'. Key finding: AFT groups 95 financial features into 11 semantic categories (momentum_short, momentum_long, reversal, volatility, trend, illusion_mom, low_vol, price_level, skewness, drawdown, regime_macro); uses Market Regime Encoder + Adaptive Gate Network to bias self-attention per detected regime; paper corrects sequence-alignment look-ahead bugs in prior transformer work; PyTorch code available. H456 ports to H198 30-stock NASDAQ universe: lightweight IS-calibrated IC-weighted attention (tractable proxy for learned attention). Variants: A (full AFT regime+gate), B (gate only, no regime encoder), C (regime encoder, uniform attention), D (VIX+SPY 200MA regime proxy, momentum reranked), E (H198 baseline). Gate: OOS Sharpe > 1.174 AND MaxDD improvement. IS: 2013-2020, OOS: 2021-2026. Script: backtesting/daily/run_h456_aft_regime_gated_attention.py. Medium risk.
h449_status: NOT CONFIRMED (2026-07-25) — Momentum Consistency (Risk-Adjusted Momentum) on H198 Universe. Hypothesis: rank H198 30-stock universe by Sharpe ratio over 6m formation period (return / return std) instead of raw return; consistent compounders preferred over volatile spikes. Motivation: Bandarchuk & Hilscher (2013), Rachwalski & Wen (2016) — momentum profits concentrated in low-dispersion stocks. Gate: OOS Sharpe > 1.174 (H198 baseline). IS: 2013-2020, OOS: 2021-2026. Variants: A (pure 6m Sharpe rank, top-6); B (0.5×mom_rank + 0.5×Sharpe_rank, top-6); C (top-12 by momentum → sort by Sharpe, take top-6); D (drawdown-adjusted momentum: 6m return / (1+worst_month_abs), top-6); E (H198 baseline 6-1m momentum top-6); F (sanity: top-12 by momentum → highest vol 6). RESULTS — IS: A=1.592, B=1.828, C=1.618, D=1.904, E=1.953, F=1.759. OOS: A=0.868, B=1.048, C=0.958, D=1.021, E=0.937, F=0.997. NOT CONFIRMED: best OOS Var B 1.048 < gate 1.174. KEY FINDINGS: (1) Consistency metrics (Sharpe rank, drawdown-adj) add ~10-11% Sharpe lift over raw momentum (B 1.048, D 1.021 vs E 0.937); (2) MaxDD improvement: Var B -30.7% vs baseline -36.7% (+6pp); (3) Sanity check Var F (high-vol momentum winners) at 0.997 BEATS pure momentum baseline 0.937 — 2021-2026 period rewarded high-vol AI/tech names (NVDA effect); (4) Pure Sharpe-ranked momentum (Var A 0.868) WORSE than baseline — consistent compounders (MSFT/AAPL) underperform vs pure momentum winners (NVDA) in bull market. PATTERN: H448/H449 join H435/H436/H437 showing H198 momentum baseline degraded to 0.937 in 2021-2026 OOS period (vs confirmed 1.174) — possible structural regime shift in large-cap tech momentum. Script: backtesting/daily/run_h449_momentum_consistency.py. Results: backtesting/results/h449_results.json.
h448_status: NOT CONFIRMED (2026-07-25) — Low-Volatility Stock Anomaly on H198 Universe. Hypothesis: Ang et al. (2006) low-vol anomaly — rank H198 30-stock large-cap universe by lowest realized vol (20d or 60d), take top-6. Baker et al. (2011): low-vol premium from institutional benchmark-hugging causing overpricing of high-vol stocks. H113 tested at ETF level (NOT CONFIRMED). Gate: OOS Sharpe > 1.174 (H198 momentum baseline). IS: 2013-2020, OOS: 2021-2026. Variants: A (pure low-vol 20d, top-6); B (pure low-vol 60d, top-6); C (0.5 mom + 0.5 inv_vol dual rank); D (top-6 momentum → filter to lowest-3 vol); E (H198 baseline momentum top-6); F (sanity: high-vol top-6). RESULTS — IS: A=1.614, B=1.519, C=1.673, D=1.669, E=1.953, F=1.809. OOS: A=0.974, B=1.045, C=0.932, D=0.687, E=0.937, F=0.830. NOT CONFIRMED: best OOS Var B (60d low-vol) 1.045 < gate 1.174. KEY FINDINGS: (1) Low-vol anomaly EXISTS at stock level — Var B low-vol (1.045) > Var F high-vol (0.830), direction correct; (2) Pure low-vol (Var B 1.045) slightly BEATS pure momentum (Var E 0.937) OOS — low-vol stocks are better compounders in 2021-2026 bull market with lower MaxDD (-24.0% vs -36.7%); (3) Momentum-vol combination HURTS — dual rank Var C 0.932 WORSE than pure mom, vol filter Var D 0.687 catastrophic (filter selects SLOWER-momentum winners which miss the AI spike months); (4) Structural insight: in H198 concentrated 30-stock large-cap universe, 2021-2026 performance dominated by NVDA/TSLA episodic spikes — low-vol selector misses these; (5) H448 might confirm on broader universe with actual cross-sectional vol spread. PRODUCTION NOTE: Var B 60d low-vol at 1.045 OOS, MaxDD -24.0%, 2 neg years — slightly worse Sharpe but much better MaxDD than H198 momentum. Not additive to production (high correlation with H198). Script: backtesting/daily/run_h448_lowvol_stock_selection.py. Results: backtesting/results/h448_results.json.
h447_status: STAGED (2026-07-25) — Wasserstein-Tracked Explainable Regime-Aware Investing on SPY/TLT/GLD/DBC. Source: arXiv:2603.04441 (Boukardagha, Columbia 2026). Extends H429 (rolling Wasserstein-HMM confirmed) to cross-asset MVO allocation: BIC dynamic K-selection (K=2..4 per rolling 3Y window), 2-Wasserstein state matching across re-fits (Bures metric), Transaction-Cost-aware MVO (TC-MVO) with L2 turnover penalty. Variants: A (fixed K=2 + TC-MVO), B (BIC K + TC-MVO), C (fixed K=2 + EW ablation), D (BIC K + EW ablation). IS: 2005-2017, OOS: 2018-2026. Gate: OOS Sharpe > 1.0 AND MaxDD > -20%. Baseline: EW-4 SPY/TLT/GLD/DBC (H311 reference). Script: backtesting/daily/run_h447_wasserstein_hmm_regime.py. (Note: renumbered from H444 — H444 taken by realized-vol gate confirmed 2026-07-24.) COMPUTE NOTE (2026-07-25): Script timed out at 8 min even with monthly re-fit (every 21 days). GaussianHMM on 4D data with BIC K-selection (3 seeds × 3 K values = 9 fits per refit × ~230 refits) non-convergent. Needs redesign: reduce to semi-annual refit (H429 approach) or use diagonal covariance to speed convergence. Deferred to future session.
h446_status: STAGED (2026-07-25) — Supply Chain Network Momentum via FinBERT 10-K MD&A Embeddings. Source: arXiv:2606.29290 (Alswaidan et al. 2026) 'Supply Chain Propagation of Textual Signals: LLM Embeddings and Cross-Sectional Return Predictability'. Key finding: FinBERT embeddings of 10-K MD&A sections propagated through Bloomberg SPLC supply chain graph produce FF5-alpha 7.27%/yr (NW t=2.64) on 255 S&P 500 firms 2011-2025. Mechanism: positive sentiment in upstream supplier 10-Ks predicts downstream customer returns — supply chain information asymmetry. H446 implementation: EDGAR 10-K MD&A text → FinBERT embeddings → cosine similarity-weighted supply chain signal → cross-sectional ranking on H198/H026 universe. Variants: A (embedding cosine, top-3 long), B (sentiment propagation, top-6), C (direct supply-chain-adjacent only), D (blend with H198 6-1m momentum). Gate: OOS Sharpe > 1.174 AND FF5-alpha t > 2.0. IS: 2013-2020, OOS: 2021-2026. Script: backtesting/daily/run_h446_supply_chain_momentum.py.
h445_status: RESOLVED → see H499 below (NOT COMPLETED — compute defect, 2026-08-08). STAGED (2026-07-25) — Heavy-Tail HMM Emission Families for Regime-Conditional VaR on H026. Source: arXiv:2606.23492 (Alswaidan, Jin, Varner 2026) 'Continuous Hidden Markov Models for Equity Returns: Heavy-Tail Emission Families and Regime-Conditional Value-at-Risk'. Key finding: Student-t / Laplace / GE emission families under shared forward-backward recursions outperform Gaussian HMM on regime-conditional VaR coverage tests. On daily US equities, interpretable Markov model with heavy tails suffices (no deep learning needed). H445 implementation: 3 emission families tested (Gaussian baseline, Student-t, Laplace) on daily H026 ETF portfolio returns; use filtered regime probabilities to set position size (BIL when high-vol regime, full investment otherwise); VaR coverage as secondary gate. Variants: A (Gaussian HMM), B (Student-t HMM), C (Laplace HMM), D (ensemble by predictive likelihood). IS: 2008-2017, OOS: 2018-2026. Gate: OOS Sharpe > 1.0 AND Kupiec coverage test p > 0.05. Script: backtesting/daily/run_h445_heavytail_hmm_h026.py.
h444_status: CONFIRMED (Var B) (2026-07-24) — Realized Volatility Regime Gate for H198 Momentum. Source: Daniel & Moskowitz (2016) "Momentum Crashes" (JFE); Choi (2026) arXiv:2607.14174 (portfolio-level vol signal motivation). Signal: rolling 20-day annualized realized vol of H198 EW-portfolio, IS-calibrated 80th-percentile threshold (IS 80th pct = 27.6% annualized; 70th pct = 20.5%). Gate: OOS Sharpe > 1.174 (H198 baseline) AND MaxDD improvement. IS: 2013-2020, OOS: 2021-2026. Variants: A (top-3 in high-vol, else top-6); B (BIL full exit in high-vol); C (continuous weight: scale = max(0.4, 1 - 0.4×(vol_ratio-1))); D (TSMOM gate combined: 3m return positive AND not high-vol); E (baseline top-6); F (70th pct threshold). RESULTS — OOS: VarA=1.139 FAIL, VarB=1.275 PASS, VarC=1.218 PASS, VarD=0.922 FAIL, VarE=1.203 PASS, VarF=1.082 FAIL. MaxDD: A=-35.1%, B=-24.9%, C=-24.3%, D=-54.8%, E=-25.5%, F=-33.7%. CONFIRMED: Var B (BIL exit) OOS 1.275 > gate 1.174, MaxDD -24.9% vs baseline -25.5% (marginal improvement). Var C (continuous scaling) OOS 1.218, MaxDD -24.3% — slight improvement. KEY FINDINGS: (1) Full BIL exit (Var B) outperforms partial reduction (Var A 1.139) — when vol is high, momentum crashes hard; half-measures don't help. (2) TSMOM combined gate (Var D) is catastrophic (MaxDD -54.8%) — momentum stock corrections are severe enough that waiting for 3m return to turn negative is too slow. (3) 70th pct threshold (Var F) tighter gate hurts — triggers more often but the regime signal is noisier at 20.5% annualized vs 27.6%. (4) Continuous weighting (Var C) beats discrete top-3 reduction (Var A) — gradual position scaling better than abrupt concentration. PRODUCTION NOTE: Var B OOS improvement is modest (+0.072 Sharpe vs baseline, MaxDD -0.6pp). Primary value is tail-risk reduction during momentum crash regimes. Script: backtesting/daily/run_h444_realized_vol_gate_h198.py. Results: backtesting/results/h444_results.json.
h428_status: NOT CONFIRMED (2026-07-24) — FRI Magnitude-Only Mean Reversion: IBS Entry Timing for XLK/SMH/IGV. Source: arXiv:2606.29591 (Portnaya 2026) 'The Bounce Has No Direction: Sign, Magnitude, and the Microstructure of Equity Return Predictability'. Key finding: SPY lag-1 autocorrelation is entirely magnitude-driven (p<10⁻¹²), sign channel not significant (p=0.11). Gate: OOS Sharpe > 2.129 (H112 IBS baseline). IS: 2015-2020, OOS: 2021-2026. Universe: XLK, SMH, IGV. Variants: A/B/C (fixed |return| thresholds 1.0%/1.5%/2.0%); D (relaxed IBS to 0.3); E (unfiltered IBS baseline); F (IS-calibrated 60th-pct threshold). RESULTS — OOS: VarA=0.418, VarB=0.377, VarC=0.243, VarD=0.550, VarF=0.646; VarE baseline=0.636. Per-ticker Var E: XLK=0.760, SMH=1.227, IGV=-0.079. NOT CONFIRMED: best OOS 0.646 (Var F) vs gate 2.129. KEY FINDINGS: (1) Magnitude filter DOES NOT improve IBS — concentration on high-magnitude days removes valid reversal opportunities, dramatically reducing trade count without improving win rate. (2) FRI theory is for SPY as a whole — the magnitude effect reflects market-wide bid-ask bounce; individual sector ETF (XLK/SMH/IGV) daily moves are dominated by common factor (SPY) plus idiosyncratic, making the magnitude filter noisier. (3) SMH standalone 1.227 is interesting but below H112 gate; IGV -0.079 drags pooled performance. (4) VarF (IS-calibrated) only marginally beats unfiltered baseline (0.646 vs 0.636) — essentially no filtering at IS-60th pct. FRI magnitude effect exists at SPY universe level but does not translate to sector-ETF IBS timing. Script: backtesting/daily/run_h428_fri_ibs.py. Results: not saved (run confirmed NOT CONFIRMED in terminal output).
...
h399_status: NOT CONFIRMED (2026-07-13) — 5-Factor Extensions and 12m-MOM Substitution on H398 Var A. Gate: OOS Sharpe > 4.068. MOM120 (12-month no-skip momentum) highly correlated with both MOM60 (0.702) and IMOM12 (0.720) — adding it as a 5th factor dilutes rather than diversifies. RESULTS: Var A (5-factor equal) OOS 3.451; Var B (3-factor, drop MOMs) OOS 2.037 MaxDD -26.2% (MOMs are load-bearing — their removal is catastrophic); Var C (replace MOM60 with MOM120) OOS 3.504; Var D (H398A top-3) OOS 3.956 MaxDD -4.3%; Var E (H398A sanity) OOS 4.068 = tied not beats gate. KEY FINDING: MOM120 adds no value on this universe — it's 70% correlated with MOM60 and 72% with IMOM12. The signal space from IMOM6+MOM60+LowVol+IMOM12 is already well-diversified; the 4th orthogonal factor needs to come from a different domain entirely (sentiment, quality, short-interest). H398 Var A remains champion. Script: backtesting/daily/run_h399.py. Results: backtesting/results/h399_results.json.
h398_status: CONFIRMED (Var A) (2026-07-13) — 12-Month IMOM as 4th Signal on H395 Var C Composite. Gate: OOS Sharpe > 3.962. IMOM12 = compound_return_12m − arithmetic_sum_12m (consistent compounders over full year). Key stats: IMOM12 avg cross-sectional spread 0.1530 vs IMOM6's 0.0476 (3.2× wider); corr(IMOM6, IMOM12) = 0.484 (partially independent — captures compounders over both 6m and 12m horizons). Var A (equal-weight 4-signal: 0.25×IMOM6 + 0.25×MOM60 + 0.25×LowVol + 0.25×IMOM12, top-2): IS 3.793, OOS 4.068, MaxDD -4.7%, CAGR 78.8%, 0 neg years. OOS annual: 2021 +124%, 2022 +60%, 2023 +138%, 2024 +130%, 2025 +103%, 2026 +35%. NEW H198 CHAMPION (beats H395 Var C OOS 3.962 → 4.068; MaxDD improves -8.6% → -4.7%). Script: backtesting/daily/run_h398.py. Results: backtesting/results/h398_results.json.
h397_status: STUB (2026-07-13) — EFS Evolutionary Factor Search on H198 Universe (arXiv:2507.17211). Luo, Zhang & Liu reformulate asset selection as LLM-generated top-m ranking with evolutionary feedback: mutation and crossover based on realized IC across a test window. Outperforms baselines on FF datasets + US50. H397 design: (1) Seed EFS with H395 Var C formula (0.33×IMOM6 + 0.33×MOM60 + 0.33×LowVol) as starting factor. (2) Evolutionary loop: LLM generates 10-15 mutation candidates per round (e.g., add MOM12, change IMOM window, add realized skew, try momentum-of-vol). (3) Evaluate each candidate's rolling IC on IS 2013-2020. (4) Keep top-3 by IS IC; cross-breed to generate next generation (3-5 rounds). (5) Final OOS backtest on 2021-2026 with gate: OOS Sharpe > 3.962 (H395 Var C) OR novel factor expression not in existing hypothesis log. Gate rationale: evolutionary search should outperform human-designed baseline if it has real discriminative power. Cost: ~$20-35 OpenAI (moderate, ~50-70 factor evaluations). Script: backtesting/daily/run_h397_efs_mining.py (stub). Low risk.
h396_status: STUB (2026-07-13) — AlphaMemo-Style Edit-Motif Mining from Hypothesis Log (arXiv:2606.20625). AlphaMemo (Univ. Sydney + Edinburgh, June 2026) shows self-evolving alpha mining agents achieve better fixed-budget discovery by recording reusable 'edit motifs' (AST-level modification patterns) from parent→child factor evolution. Tested on CSI 500 and S&P 500; confidence-gated memory suppresses known failures, amplifies confirmed patterns. George's hypothesis-log.md (395+ experiments) is a ready-made search ledger with structured outcomes. H396 design: (1) Parse hypothesis-log.md to extract (parent_signal, modification, outcome) triples — e.g., ('H198 6-1m', 'add low-vol tiebreaker', 'CONFIRMED +Sharpe 2.69→3.96'), ('H198 6-1m', 'add ILLIQ tiebreaker', 'NOT CONFIRMED — large-cap homogeneous'). (2) Cluster into motif categories: tiebreaker additions, universe expansions, filter overlays, window changes, skip-month variants. (3) Build confidence-weighted motif library. (4) Use library as context window for H381 (AlphaLogics) and H382 (FactorEngine) LLM prompts: 'Motifs that work on H198: low-vol tiebreaker, no-skip momentum; Motifs that fail: ILLIQ, quality GP/A, MAX tiebreaker on large-cap'. Gate: at least one H381/H382 motif-seeded run outperforms un-seeded run on OOS Sharpe. Script: backtesting/daily/run_h396_alphamemo_motifs.py (stub). Cost: ~$5 LLM calls for motif extraction. Low risk.
h390_status: PROPOSED (2026-07-08) — OpenFinGym Multi-Task Quant Agent Evaluation on H026/H174 Tasks. Source: Zhang et al. (2026) arXiv:2606.26350, University of Edinburgh/UCL/Alan Turing Institute/Oxford. OpenFinGym provides unified gym environment + containerized leakage-resistant verifier across forecasting, strategy construction, real-time trading, and fraud detection. Phase 1: Encode H026 ETF rotation as forecasting task (predict next-month top-1 ETF) and H174 PEAD as trading task (event-driven entry/exit on 8-K signal); run against OpenFinGym verifier for independent alpha validation. Phase 2: Use OpenFinGym RL post-training to fine-tune small LLM on H174 task definitions. Gate: Phase 1 — both strategies score above equal-weight/random baseline; Phase 2 — H174 LLM-augmented WR > 81.8%. Script: backtesting/daily/run_h390_openfingym.py (stub). Medium risk. Primary value: independent leakage-resistant verification of H026/H174 (addresses H256 look-ahead bias incident pattern).
h389_status: WIKI (2026-07-08) — Alpha Illusion — LLM Trading Agent Validation Checklist. Source: Sheng et al. arXiv:2605.16895 (2025). Created wiki/trading/algorithms/llm-alpha-validation.md with 6 structural validity tests: (1) Temporal integrity; (2) Real-world frictions; (3) Counterfactual robustness; (4) Predictive calibration; (5) Numerical execution; (6) Multi-agent disaggregation. Applied to checklist for H274, H381, H382, H383, H384 and all future LLM hypotheses. Cross-referenced systems: FinCon, FinMem, TradingAgents, FinAgent, QuantAgent, FLAG-Trader. Added LLM sub-checklist to wiki/trading/shared-eval-checklist.md.
h388_status: PROPOSED (2026-07-08) — PEAD Revived: Elastic Net on 12-Quarter SUE History as H174 Pre-Filter. Source: Kaczmarek & Zaremba (Finance Research Letters, 2025, DOI:10.1016/j.frl.2025.108751). Replace raw EPS surprise >= 0.02 gate in H174 with elastic-net ML score trained on 12-quarter standardized unexpected earnings (SUE) history. Pattern of earnings surprises across 12 quarters (streaks, acceleration, reversals, magnitude trends) predicts returns better than latest surprise alone; effect strongest in large-cap stocks (our H174 universe). Paper: elastic net on SUE history nearly doubles Sharpe vs single-quarter SUE. Mechanism: latest EPS surprise quickly priced in; 12-quarter patterns remain overlooked. Maintain FinBERT score >= 0.18 gate. Gate: OOS WR > 81.8% AND n >= 15 (H174 baseline). IS: 2019-2022; OOS: 2023-present. Script: backtesting/daily/run_h388_pead_ml_sue.py (to be written). Low risk.
h387_status: PROPOSED (2026-07-08) — Wasserstein HMM Explainable Regime-Aware Portfolio Allocation on SPY/TLT/GLD/BIL. Source: Boukardagha (2026) arXiv:2603.04441, Columbia University. Wasserstein HMM combines rolling Gaussian HMM with predictive model-order selection and template-based identity tracking using 2-Wasserstein distance between Gaussian components — solves the label-switching problem that caused H251 to collapse to single-regime OOS. Dynamic regime count (not fixed 3-state) + transaction-cost-aware mean-variance optimization penalizes excess rebalancing. Paper: Sharpe 2.18 vs 1.59 equal-weight; MaxDD -5.43% vs -14.62% SPY. Validated on real 2025 "Liberation Day" selloff (dynamically reduced equity). Gate: OOS Sharpe > 1.532 (H311 EW-4+VIX<20) AND MaxDD < -10%. IS: 2004-2017; OOS: 2018-2026. Script: backtesting/daily/run_h387_wasserstein_hmm.py (to be written). Medium risk. CAVEAT: use filtered (not smoothed) HMM probabilities per Regime Detection Design Note in CLAUDE.local.md.
h385_status: STUB (2026-07-08) — Illusion Momentum (IMOM) Cross-Sectional Signal on H198 30-Stock Universe. Source: Iwanaga & Hirose (2026) Pacific-Basin Finance Journal Vol 96 (DOI:10.1016/j.pacfin.2026.103063); working paper Iwanaga (2024). FORMULA: IMOM = [Π(1+r_t)−1] − Σ(r_t), i.e. compound return minus arithmetic sum. High IMOM = sustained directional gains (compounding worked in the stock's favor); low IMOM = volatile round-trip (compounding hurt). Cognitive bias mechanism: investors anchor to arithmetic sum and underestimate compound return → underreaction to consistent momentum → predictable drift. Primary spec: 6-month formation NO skip-month (same as H377). Paper alpha: 1.39% raw / 1.42% risk-adjusted (6M). Decay by decade: 2.62% (1990s), 1.42% (2000s), 0.55% (2010s+) — effect persists but smaller in recent data. Mechanism distinct from MAX/HTP: paper rejects limited-attention explanation; IMOM strength grows with |IMOM| magnitude. Gate: OOS Sharpe > 1.174 AND MaxDD > −30%. IS: 2013-2020, OOS: 2021-2026. Script: backtesting/daily/run_h385.py (stub).
h384_status: PROPOSED (2026-07-08) — ReCAP Regime-Adaptive Continual Learning for H026 ETF Rotation. Source: arXiv:2606.00143 (May 2026). ReCAP integrates continual learning into portfolio management: (1) Adaptive Regime Detection via change-point detection (not fixed IS/OOS splits); (2) Policy Library of regime-specific allocation policies; (3) Rapid Adaptation — retrieves most similar historical regime policy and fine-tunes online (no catastrophic forgetting). H384 tests ReCAP as meta-layer on H026: train lightweight DRL policy per detected regime learning which ETF selection heuristics work best in each regime. Gate: OOS Sharpe > 2.610 (H346 canonical baseline). IS: 2008-2020 (policy library); OOS: 2021-2026. Script: backtesting/daily/run_h384_recap_continual.py (to be written). Requires: PyTorch, ruptures, stable-baselines3. RISK: high.
h383_status: PROPOSED (2026-07-07) — HMM + RL Regime-Based Allocation on SPY/TLT/GLD Production Sleeve. Source: Verma et al. (2026) arXiv:2605.27848. 3-state Gaussian HMM (BIC-selected: Low-Vol/Transitional/High-Vol) + RL policy (state=regime probabilities, action=discrete allocation) trained to maximize Sharpe conditional on regime. H383 tests on SPY/TLT/GLD daily 2004-2026. Gate: OOS Sharpe > 1.532 (H311 EW-4+VIX<20 best variant). IS: 2004-2017; OOS: 2018-2026. Script: backtesting/daily/run_h383_hmm_rl_allocation.py (to be written). Requires: hmmlearn, stable-baselines3. CAVEAT: use filtered (not smoothed) HMM probabilities per Regime Detection Design Note in CLAUDE.local.md.
h382_status: PROPOSED (2026-07-07) — FactorEngine Program-Level LLM Factor Mining on H198 Universe. Source: Lin et al. arXiv:2603.16365 (Mar 2026). Casts factors as Turing-complete code; separates logic revision from parameter optimization; LLM-guided directional search + Bayesian HPO; Experience Knowledge Base avoids re-exploring failed directions. H382 bootstraps from hypothesis-log.md (380+ prior experiments with results) as starting knowledge base. Gate: OOS Sharpe > 1.174; IC > 0.05 OOS for at least one generated factor. IS: 2013-2020; OOS: 2021-2026. API cost: ~$20-50 OpenAI. Script: backtesting/daily/run_h382_factorengine.py (to be written).
h381_status: PROPOSED (2026-07-07) — AlphaLogics Market-Logic-Driven Multi-Agent Alpha Mining on H198. Source: Weng et al. arXiv:2603.20247 (Mar 2026). Three-component system: (1) Market Logic Mining — reverse-engineers market logic from H026/H198 confirmed factors; (2) Factor Generation & Optimization — LLM-guided code generation with backtest-feedback loop; (3) Market Logic Generation & Optimization — new logics conditioned on existing library. Key innovation over QuantaAlpha (H349): explicit 'market logic' abstraction layer makes discovery interpretable. Validated on S&P500 with significant improvement over representative baselines. Gate: OOS Sharpe > 1.174. IS: 2013-2020; OOS: 2021-2026. API cost: ~$15-40 OpenAI. Script: backtesting/daily/run_h381_alphalogics.py (to be written).
h380_status: PROPOSED (2026-07-07) — Cross-Market Alpha191 on US S&P500 via Double-Selection LASSO. Source: Du, Walter & Ulrich arXiv:2601.06499 (2026). 17 of 168 Alpha191 Chinese A-share short-term trading factors survive double-selection LASSO controlling for 151 established US factors, showing statistically significant incremental explanatory power for S&P500 cross-sectional returns. Surviving factors span 6 domains: Volume & Flow, Mean Reversion, Trend & Momentum, Volatility & Risk, Liquidity & VWAP, Price Action. H380 tests these 17 factors on H198 30-stock universe as standalone and as tiebreaker/augmentation over H198 6-1m momentum. Gate: OOS Sharpe > 1.174. IS: 2013-2020; OOS: 2021-2026. CAVEAT: paper covers 2002-2022; OOS 2021-2026 partially overlaps. Script: backtesting/daily/run_h380.py (to be written).
h379_status: PROPOSED (2026-07-07) — Network Momentum via Cross-ETF Lead-Lag Spillover. Source: Li & Ferreira (2025) "Follow the Leader: Enhancing Systematic Trend-Following Using Network Momentum" arXiv:2501.07135. Sharpe 1.74 on 64 commodity futures (20yr). H379 adapts to H026+H041a 40-ETF universe: rolling cross-correlation matrix (2-wk lags, 6-mo window); when ETF is top-ranked AND is a lead-lag "leader", increase size by 25%; include "follower" ETFs (lagging leader by 2-4 wks) alongside leader instead of strict top-1. Gate: OOS Sharpe > 1.300. IS: 2008-2017, OOS: 2018-2026. Script: backtesting/daily/run_h379.py (to be written). Note: Originally staged as H377 by dream cycle scan 2026-07-06; renumbered to H379 as H377 was reserved for 6-0m no-skip follow-up.
h378_status: PROPOSED (2026-07-07) — EarningsInOne Fast/Slow Dual-Signal PEAD Upgrade. Source: Yu, Liu, Zhang, He (2026) "Fast Numbers, Slow Language: Bridging Quantitative and Qualitative Earnings Signals" arXiv:2606.29734. Speed separation: EPS surprise peak eliminated by next open; qualitative ECT sentiment peaks next day, directionally tradeable under correct evaluation (directional accuracy vs MSE). H378 adds slow ECT layer to H174: (1) H174 OPG entry unchanged; (2) At 4 PM, score ECT via FinBERT/GPT-4o-mini; (3) ECT >= 0.15 AND profitable → hold 2 extra trading days beyond 20-day exit; (4) ECT < -0.05 → early exit at 4 PM. Universe: H174 confirmed events (score >= 0.18 AND surprise >= 0.02). Gate: OOS WR > 0.818, mean_ret > 6.89%, n >= 15. IS: 2019-2022, OOS: 2023-present. Caveat: FMP transcript 403 (H247); EDGAR MD&A or Alpaca news as fallback. Script: backtesting/daily/run_h378.py (to be written). Note: Renumbered from proposal's H376 — H376 already assigned to MAX Factor Composite.
h377_status: STUB (2026-07-06) — 6-0m No-Skip Momentum H198 Standalone. Source: H376 discovery — pure 6-0m no-skip baseline achieves OOS 3.120 / MaxDD -8.4%, strongest result in H198 family; MAX composite on 6-0m (2.790) slightly dilutes it. H277 NASDAQ finding (skip-month hurts on tech-heavy universe) now confirmed on H198 broader large-cap universe. H377 tests 6-0m no-skip as standalone with full variant analysis (position sizing, window sensitivity, universe sensitivity). Gate: OOS Sharpe > 1.174 AND MaxDD < -30%. Script: backtesting/daily/run_h377.py (to be written).
h376_status: CONFIRMED (Variants A/B/D) (2026-07-06) — MAX Factor Composite on H198 Top-6 Equal-Weight. Source: Tandfonline 2025 MAX×Momentum interaction; H277 no-skip finding. Universe: H198 30-stock large-cap. IS 2013-2020, OOS 2021-2026. Gate: OOS Sharpe > 1.174 AND MaxDD > -30%. Var A (0.7·mom_1skip+0.3·max) OOS 1.179 PASS; Var B (0.5·mom_1skip+0.5·max) OOS 1.271 PASS; Var D (0.7·mom_0skip+0.3·max) OOS 2.790 PASS. KEY FINDING: 6-0m no-skip pure baseline OOS 3.120/MaxDD -8.4% — strongest H198 result (warrants H377). MAX dilutes the 6-0m edge. Script: backtesting/daily/run_h376.py. Results: backtesting/results/h376_results.json.
...
h343_status: CONFIRMED (Variants C/D) / NOT CONFIRMED (Variants A/B) — (2026-06-28) — FVG / Order Block Filter on H198 6-1m Momentum. Source: smartmoneyconcepts Python library (joshyattridge/smart-money-concepts, v0.0.27, 1,788 stars); ICT Inner Circle Trader methodology. Hypothesis: at month-end, among H198 top-ranked 6-1m momentum stocks, enter only those with an unmitigated bullish Fair Value Gap (FVG) or Order Block (OB) on daily bars within the last 30 trading days. Universe: same 30 large-cap stocks as H198. Signal: 6-1m momentum rank → SMC filter confirmation. Portfolio: top-6 filtered, equal-weight, monthly rebalance. IS: 2013-2020, OOS: 2021-2026. Gate: OOS Sharpe > 1.174 (H198 baseline). Script: backtesting/daily/run_h343.py. Results: backtesting/results/h343_results.json. VARIANTS TESTED — A (FVG strict, hold cash if <3 pass): IS=0.000, OOS=0.000 — always cash in OOS (all large-cap FVGs mitigated within 30-day window; FVG too short-lived on liquid large-caps at monthly rebalance frequency). B (FVG lenient, fill slots from unfiltered): IS=1.779, OOS=1.174, MaxDD=-22.7%, NegYrs=1 — identical to baseline; FVG strict filter never fires so B degrades to pure H198. C (OB strict, hold cash if <3 pass): IS=3.406, OOS=3.182, MaxDD=-5.4%, NegYrs=0, CAGR=67.8%. D (OB lenient, fill slots from unfiltered): IS=2.633, OOS=2.334, MaxDD=-13.6%, NegYrs=0, CAGR=57.3%. E (baseline 6-1m, no filter): IS=1.779, OOS=1.174, MaxDD=-22.7%, NegYrs=1. SPY: OOS=0.954, MaxDD=-23.9%, NegYrs=1. VERDICT — Variant C: OOS 3.182 >> gate 1.174. BEATS GATE. WF=0.934 (healthy). Variant D: OOS 2.334 >> gate 1.174. BEATS GATE. WF=0.887 (acceptable). Variants A/B: fail gate. CASH MECHANICS (Variant C) — 9/160 months cash (5.6%). IS cash months: 2015×1, 2018×2, 2020×1. OOS cash months: 2022×2, 2023×1, 2024×1, 2025×1. Critical OOS 2022 behavior: C returned +48.5% while baseline returned -10.2% — the OB strict filter acted as an implicit bear-market detector (in drawdowns, fewer large-cap stocks maintain unmitigated bullish OBs → triggers cash mode). OB MECHANISM INTERPRETATION — Order Block = consolidation zone before strong impulse move, detected via swing_highs_lows then ob(). During bull regimes, ≥3 stocks always have active unmitigated bullish OBs → strategy stays fully invested. During corrections/bear markets, OBs get "mitigated" (price breaks below the zone) or new bullish OBs don't form → <3 stocks qualify → cash. This is a structural regime filter, not a pure pattern filter. The OB detection is NOT look-ahead biased: the swing_highs_lows function looks N candles forward within the window ending at as_of, but all candles are historical. FVG vs OB COMPARISON — FVG fails on monthly-rebalance large-caps: FVGs are 3-candle imbalances that fill (get mitigated) within days on liquid stocks. OBs are multi-candle consolidation zones that persist for weeks/months. FVG window mismatch: 30-day window is too long for FVGs (all filled) and too short for FVGs to persist as monthly entry signals. OB window is appropriate. CAUTION FLAGS — (1) Extraordinary magnitude: 3.182 OOS Sharpe on a 30-stock large-cap universe with monthly rebalance is historically unusual; warrants additional rolling-window validation before live. (2) Survivorship bias: same H198 caveat applies — 30 stocks are current S&P 500 large-caps. (3) Only 9 cash months provides limited regime-filter testing; more bear markets needed to validate stability. (4) ICT/SMC methodology has no peer-reviewed academic backing — the OB filter's effectiveness may reflect the market structure of 2013-2026 specifically. RECOMMENDED NEXT STEPS — (1) Rolling-window WF cross-validation on Variant C. (2) Sensitivity: vary FVG_WINDOW (15/30/45d), MIN_FILTER (2/3/4), swing_length (3/5/7). (3) Test Variant C as production add-on to existing H198 allocation (new standalone slot vs OB-gated H198 replacement). (4) H344 spin-off: OB entry timing for PEAD (H174) — use OB pullback rather than OPG market order.
h339_status: NOT CONFIRMED (2026-06-26) — Price-Based Momentum Filter Gates (Proxy for LLM Sentiment Gate, arXiv:2510.26228). Source: LLM momentum filter paper proposes skipping momentum trades when LLM news sentiment is negative. Implemented with price-based proxies (no NewsAPI history available): 7 variants tested using 1m return gate, 3m return gate, acceleration gate (recent 3m > prior 3m), absolute momentum filter, and combinations. Universe: H198 30-stock S&P 500. IS 2013-2020, OOS 2021-2026. Baseline OOS Sharpe=1.055. RESULTS — Var A (pure momentum baseline): IS=1.582, OOS=1.055. Var B (1m>0): OOS=0.615 (skip when last month negative). Var C (3m>0): OOS=0.925. Var D (acceleration): OOS=0.419. Var E (1m+accel): OOS=0.343. Var F (abs mom>0): OOS=1.055 (identical to baseline — top-1 by 6m momentum always has positive 6m return). Var G (3m+accel): OOS=0.419. NOT CONFIRMED: no filter beats gate 1.174. ROOT CAUSE: (1) Price filters REDUCE Sharpe by excluding months that momentum would have been correct — the top-1 6-1m momentum stock already has strong 3m/6m trend by selection, so momentum acceleration filters are nearly always triggered anyway. Var F (abs mom>0) is trivially equivalent to baseline because the top-ranked stock by 6-1m momentum always has positive 6m momentum. (2) 1m gate (Var B, 0.615) actually HURTS — short-term 1m reversal is common even for strong momentum stocks (the skip-month effect); filtering out negative-1m stocks removes valid momentum continuation cases. (3) The H198 universe's top-1 is concentrated in mega-cap tech (NVDA/TSLA/NVDA dominating OOS); adding sentiment gates creates cash drag in years when the momentum winner consolidates briefly. METHODOLOGY NOTE: True LLM sentiment filter (NewsAPI + VADER/GPT-4o-mini) was not tested due to limited NewsAPI history and rate limits. The price-based proxy is a weaker test — the full LLM version (H279 queued) remains untested. CONCLUSION: Price-based momentum filter gates do NOT improve over H198 baseline. LLM semantic sentiment (H279) remains queued as a separate hypothesis. Script: backtesting/daily/run_h339.py. Results: backtesting/results/h339_results.json.
h338_status: NOT CONFIRMED (2026-06-26) — Multi-Asset Trend + Carry on Combined Bond+Equity ETF Universe. Source: Koijen et al. (2018) "Carry" (JFE); Asness, Moskowitz & Pedersen (2013) "Value and Momentum Everywhere". Combined universe: H045 bond ETFs (SHY/IEI/IEF/TLT/TIP/HYG/LQD) + H026 equity/alts subset (XLK/XLV/XLE/XLF/XLU/GLD/DBC/EEM/QQQ/SPY) = 17 assets. Signal: 12m momentum + TTM dividend yield carry, various blend weights. IS 2008-2017, OOS 2018-2026. Gate: OOS Sharpe > H045 baseline 1.351. RESULTS — Var A (momentum-only top-2): IS=0.201, OOS=1.109, MaxDD=-17.2%, NegYrs=1. Var B (carry-only top-2): IS=0.954, OOS=0.619. Vars C/D/F identical to A because carry signal = 0 (dividend data unavailable). Var E (momentum top-3): OOS=0.920. NOT CONFIRMED: best OOS=1.109 < gate 1.351. ROOT CAUSES: (1) DATA ISSUE: yfinance Ticker.dividends returns empty for ETFs in this environment (likely OneCLI proxy interference with yfinance requests); carry signal was effectively zero for all variants, making C/D/F identical to pure momentum A. (2) STRUCTURAL: even momentum-only top-2 on combined 17-asset universe underperforms H045's pure 7-bond rotation by 0.242 Sharpe. Adding 10 equity ETFs to the bond universe introduces correlated equity noise that dilutes the bond momentum signal. (3) WF RATIO ANOMALY: IS Sharpe=0.201 vs OOS=1.109 → WF=5.518, extremely high, indicating the 2008-2017 IS period was structurally bad for multi-asset momentum (GFC recovery regime where bonds dominated). The OOS improvement reflects the more volatile 2018-2026 period, not genuine IS→OOS generalization. The H045 pure-bond universe already outperforms at 1.351. Combining with equity ETFs is structurally dilutive, not additive. FUTURE: To properly test carry, need to source ETF dividend data from a different endpoint (e.g., EDGAR, direct ETF issuer data, or AlphaVantage). Script: backtesting/daily/run_h338.py. Results: backtesting/results/h338_results.json.
h337_status: NOT CONFIRMED (2026-06-26) — Quality-Momentum Dual Ranking (GP/A and ROE) on H198 Universe. Source: Novy-Marx (2013) "The Other Side of Value" (JFE); Asness, Frazzini & Pedersen (2014) "Quality Minus Junk". Hypothesis: dual ranking by momentum + quality (gross profitability = GP/A) improves over pure momentum. Quality from yfinance quarterly financials (3-month reporting lag). Universe: H198 30-stock S&P 500. IS 2013-2020, OOS 2021-2026. Gate: OOS Sharpe > 1.174 (H198 baseline). Data coverage: GP/A 6.7%, ROE 6.1% (yfinance quarterly data goes back only ~4-5 years with gaps). RESULTS — Var A (pure momentum): IS=1.582, OOS=1.055. Var B (0.5mom+0.5gp_a): OOS=0.802. Var C (0.7mom+0.3gp_a): OOS=0.832. Var D (0.5mom+0.5roe): OOS=0.792. Var E (mom+GP/A median filter): OOS=0.812. NOT CONFIRMED: all quality-composite variants WORSE than pure momentum. ROOT CAUSES: (1) LOW COVERAGE (6.7%): yfinance quarterly financials only available for recent 4-5 years; in periods with no quality data the strategy falls back to pure momentum, but in periods WITH quality data (mostly OOS) the quality tiebreaker selects WRONG stocks. (2) CONCENTRATION: with only 30 large-cap stocks, all have high-quality businesses — cross-sectional GP/A variation is insufficient to add signal; the academic GP/A premium requires broad universe (hundreds/thousands of stocks) with real quality variation between industry leaders and laggards. (3) QUALITY TIEBREAKER IN MOMENTUM UNIVERSE: momentum already selects the highest-performing stock; quality signal routes away from the winner (e.g., quality says IBM over NVDA); OOS top-1 is heavily NVDA (quality signal would actually de-rank NVDA in years when fabless chipmakers score differently on GP/A). (4) FMP API UNAVAILABLE: SSL cert failure (OneCLI proxy certificate not trusted by FMP); yfinance fallback had 6.7% coverage — insufficient for a robust test. CONCLUSION: Quality-Momentum dual ranking does NOT improve over H198 in a concentrated 30-stock large-cap universe. The quality premium (GP/A, ROE) may require: (a) broader universe (200+ stocks with real quality spread), (b) FMP API with proper CA bundle, (c) explicit exclusion of financials (banks inflate ROE). Quality family NOT closed — H337b proposed with broader 200-stock universe and FMP data when SSL issue resolved. Script: backtesting/daily/run_h337.py. Results: backtesting/results/h337_results.json.
h336_status: NOT CONFIRMED (2026-06-26) — 52-Week High Proximity Momentum on H198 Universe. Source: George & Hwang (2004) "The 52-Week High and Momentum Investing" (JF). Previous H291 tested on 50-stock large-cap universe (OOS Sharpe 0.764, gate 0.9). This retest used H198 30-stock NASDAQ-heavy universe with higher gate (1.174). 5 variants tested: A (pure R52 top-1), B (50%R52+50%mom), C (30%R52+70%mom), D (momentum with R52>0.85 filter, enter else skip), E (70%R52+30%mom). IS 2013-2020, OOS 2021-2026. Baseline OOS Sharpe=1.055. RESULTS — Var A (pure R52): IS=0.972, OOS=0.342, MaxDD=-45.1%, NegYrs=3. Var B (50/50): IS=1.392, OOS=0.407, MaxDD=-34.1%. Var C (30/70 mom-dominant): IS=1.536, OOS=0.811. Var D (filter R52>0.85): IS=1.396, OOS=0.977. Var E (70/30 R52-dominant): IS=1.189, OOS=0.366. NOT CONFIRMED: best OOS=0.977 (Var D) < gate 1.174. ROOT CAUSES: (1) In a 30-stock large-cap NASDAQ universe, almost all stocks are continuously near their 52-week highs in a bull market — H291's same finding applies. The cross-sectional R52 signal has near-zero dispersion in a sustained bull (2021-2025). (2) Var D filter (require R52>0.85 to enter) is most promising at 0.977 but still falls short — the filter correctly excludes broken momentum stocks but can't improve the signal when most stocks qualify anyway. (3) H198's momentum IS Sharpe (1.582 in this run) vs H291's 50-stock IS (1.031) — H198 momentum is already a strong signal on this concentrated universe; adding R52 simply degrades it by routing away from the top momentum winner. (4) Pure R52 Sharpe of 0.342 OOS confirms H291 finding: anchoring bias signal requires wider universe with genuine 52w high variation. GEORGE & HWANG (2004) FINDING: 52w high works best in small/mid-cap universe and in bear/recovery markets — both conditions absent in 2021-2026 bull run. Script: backtesting/daily/run_h336.py. Results: backtesting/results/h336_results.json.
h335_status: NOT CONFIRMED (2026-06-26) — Bond ETF Momentum Window Optimization (H045 Universe). Source: H045 confirmed (OOS Sharpe 1.351). Question: is 12m the optimal momentum window for bond ETFs? Variants A-F tested: 3m, 6m, 9m, 12m, 12m-no-vol, and 3m+12m composite. Universe: SHY/IEI/IEF/TLT/TIP/HYG/LQD (H045 bond universe). IS 2007-2016, OOS 2017-2026. All variants fail gate 1.351. Best OOS Sharpe = (see run results). Script: backtesting/daily/run_h335.py. Results: backtesting/results/h335_results.json.
h334_status: NOT CONFIRMED (2026-06-26) — Return Seasonality × Momentum Composite on H198 Universe. Source: H292 (seasonality confirmed OOS 0.970) + H198 (6-1m momentum confirmed OOS 1.174). Composite of IS-calibrated seasonal calendar-month score × 6-1m momentum. 3 weight variants (0.7/0.3, 0.5/0.5, 0.3/0.7 mom/sea). IS 2013-2020, OOS 2021-2026. Best OOS Sharpe=0.707 vs gate 1.174. NOT CONFIRMED. Root cause: H292 note ("useful as signal layer not standalone") — the seasonality signal on a 30-stock universe with IS-calibrated scores provides insufficient cross-sectional information when overlaid on the already-strong H198 momentum signal. Script: backtesting/daily/run_h334.py. Results: backtesting/results/h334_results.json.
...
h289_status: QUEUED (2026-06-12) — News Entropy Macro Signal (Regime Gating Layer). Source: arXiv:2309.05560 'New News is Bad News'. Entropy of news text distribution predicts negative market returns; unexplained by existing factors. Design: month-over-month KL divergence of NewsAPI topic word-vectors as entropy proxy. High KL (>0.30) → BIL override on H249 regime framework. Gate: OOS Sharpe improvement >0.05 vs H249 baseline (1.031). Prerequisite: NewsAPI historical data or GDELT proxy. Scaffold: backtesting/daily/run_h289.py.
h288_status: QUEUED (2026-06-12) — LLM-Constrained Factor Discovery via DSL Audit Trail. Source: arXiv:2604.26747. OOS Sharpe 1.55, 44.55% annualized on crypto OOS 2024-2026 (5bp costs). Adapt to US equity 30-stock universe. LLM proposes factor expressions in restricted DSL; fixed IS/OOS protocol enforced. Ridge-combine surviving factors. Gate: composite OOS Sharpe >1.3 (vs H217 1.559). Scaffold: backtesting/daily/run_h288.py.
h287_status: QUEUED (2026-06-12) — Janus-Q Event-Type Annotation for PEAD Entry Filter. Source: arXiv:2602.19919. 62,400-article news corpus with 10 event types + CAR labels; +102% Sharpe improvement vs baselines. Design: classify H174 8-K text into 10 event types (EarningsBeat/Miss/GuidanceRaise/Cut/etc) via GPT-4o-mini; compute IS CAR per event type; OOS entry only when event type has mean IS CAR >5%. Gate: OOS WR >81.8% (H174 baseline), n >=15. IS 2018-2022, OOS 2023-2025. Scaffold: backtesting/daily/run_h287.py.


---
updated: 2026-06-14
h286_status: CONFIRMED (2026-06-12) — Macro Regime-Gated FCF/P: COWZ/SPY Cross-Momentum + BIL Escape. Source: Yartseva 2025 (FCF/P premium); H249 regime-conditional regime framework; COWZ ETF (Pacer US Cash Cows 100, launched 2016) as FCF yield factor proxy. Hypothesis: pure COWZ (FCF yield) B&H underperforms SPY (OOS 2020+ Sharpe 0.737 vs SPY 0.920), but regime-gating with macro signals improves risk-adjusted returns. Three variants tested on IS 2017-2020, OOS 2021-2025. Var A (SPY > 200MA AND VIX < 25 → COWZ, else BIL): IS Sharpe=0.013, OOS Sharpe=0.637, MaxDD=-17.6%, NegYrs=1. Regime: COWZ 75% / BIL 25% of OOS months. Var B (COWZ/SPY cross-momentum — hold COWZ when COWZ 6m > SPY 6m, hold SPY otherwise; escape to BIL when SPY < 200MA): IS Sharpe=0.624, OOS Sharpe=1.031, MaxDD=-16.2%, NegYrs=2, Corr(SPY)=0.596. Var C (VIX-switched: COWZ VIX<20 / SPY 20-30 / BIL ≥30): IS Sharpe=0.283, OOS Sharpe=0.682, MaxDD=-23.2%, NegYrs=1. COWZ B&H OOS (2021-2025): Sharpe=0.893. SPY OOS (2021-2025): Sharpe=0.998. CONFIRMED: Var B OOS Sharpe=1.031 > COWZ B&H baseline (0.893) > SPY OOS (0.998). CRITICAL FINDINGS: (1) Var A (strict 200MA+VIX) HURTS performance vs COWZ B&H — the 2021-2022 period had VIX > 25 frequently but SPY > 200MA; the combined gate was too restrictive, parking capital in BIL during 2021 bull run (COWZ returned +31.8% in 2021). The IS Sharpe 0.013 shows the gate is miscalibrated in-sample. (2) Var B's cross-momentum signal (COWZ 6m vs SPY 6m) adaptively switches between FCF-value factor and broad equity momentum — this captures the FCF yield premium when value outperforms (2022: COWZ -9.7% vs SPY -18.2%; Var B avoided worst drawdown via BIL escape) while rotating to SPY during growth/momentum regimes (2023-2024 when large-cap tech dominated). (3) Corr(B,SPY)=0.596 is LOW for an equity strategy — below the 0.70 threshold used for diversification assessment. Var B offers genuine diversification from production portfolio. (4) Key caveat: 2021-2025 OOS is only 60 months; COWZ/SPY 6m cross-momentum may overfit the 2022 value outperformance year (the ONLY year value beat growth in the OOS window). One-year dominance is not a robust pattern. PRODUCTION ASSESSMENT: NOT recommended for immediate production deployment. (a) IS Sharpe 0.624 < OOS Sharpe 1.031 — IS underfits, suggesting OOS gain is regime-specific (2022 value regime); (b) Only 1 year (2022) of genuine FCF/value outperformance in OOS to validate the signal; (c) Better path: apply Var B logic to H270 universe (which has 12 years of live data). Track COWZ separately as paper trade until 3 more years of OOS data available. Script: backtesting/daily/run_h286.py. Results: backtesting/results/h286_results.json.
h285_status: CONFIRMED-WEAK (2026-06-12) — Earnings Quality Factor: Quality ETF Rotation (6m Momentum, Top-1). Source: Sloan (1996) Accruals Anomaly; Asness, Frazzini & Pedersen (2014) "Quality Minus Junk" (QMJ); Novy-Marx (2013) Gross Profitability. Hypothesis: rotating among quality-factor ETFs (QUAL/SPHQ/DGRW/SPY/BIL) by 6-month momentum selects the best-performing quality-factor expression each month, earning the quality premium with downside protection. Universe: QUAL (iShares MSCI USA Quality), SPHQ (Invesco S&P 500 Quality), DGRW (WisdomTree Quality Dividend Growth), SPY, BIL. All ETFs launched 2013-2014. IS 2014-2019, OOS 2020-2025. RESULTS — Rotation A (6m mom, top-1): IS Sharpe=1.033, OOS Sharpe=0.932, MaxDD=-19.4%, NegYrs=1, Corr(SPY)=0.969. Holdings: SPY 27% / DGRW 26% / SPHQ 25% / QUAL 23% — near-equal distribution across all candidates. Buy-and-Hold comparison (OOS): SPHQ=0.959, DGRW=0.885, QUAL=0.794, MTUM=0.831, VLUE=0.746, SPY=0.921. QUAL excess return over SPY (OOS): mean -0.133%/mo, annualized -1.59%, t-stat=-1.246 (not statistically significant). CONFIRMED: Rotation OOS Sharpe=0.932 > 0.9 gate; OOS Sharpe (0.932) > SPY OOS (0.921). Walkforward ratio=0.902. CRITICAL WEAKNESSES: (1) Corr(Rotation, SPY)=0.969 — near-perfect correlation with SPY. The rotation adds NO meaningful diversification from the production portfolio. This is essentially an SPY-substitute with marginal quality tilt. (2) Holdings distribution is nearly uniform — the 6m momentum signal is non-discriminating for quality ETFs, rotating roughly randomly among all 4 candidates including SPY itself (held 27% of time). This means the "quality rotation" is 73% in quality ETFs and 27% in plain SPY — the rotation is not adding a quality premium, it's just holding 3 slightly-different SPY variants. (3) QUAL B&H OOS Sharpe=0.794 — QUAL actually underperforms SPY B&H by -1.59%/yr (annualized, t-stat -1.25). The quality factor does not generate alpha beyond SPY in a simple buy-and-hold over 2020-2025. (4) The 2020-2025 OOS period was dominated by mega-cap tech (NVDA/META/MSFT) — quality factors miss this concentrated momentum. CONCLUSION: Quality ETF rotation technically meets the gate (Sharpe 0.932 > 0.9) but is a WEAK confirmation. The extreme SPY correlation (0.969) makes it non-additive to the production portfolio, and QUAL B&H itself underperforms SPY. The quality factor premium (as captured by QUAL/SPHQ) was negative OOS in 2020-2025. NOT RECOMMENDED for production portfolio. Script: backtesting/daily/run_h285.py. Results: backtesting/results/h285_results.json.
h284_status: CONFIRMED (2026-06-13) — FCF/P Stock Screener (Free Cash Flow Yield via FMP). ⚠️⚠️ STRONG CAVEATS — see below. Source: Yartseva 2025 "What Makes a Multibagger?". REVISION: Original v1 NOT CONFIRMED due to yfinance/FMP data gaps. Rewritten using FMP stable/key-metrics endpoint (free tier) which provides pre-computed freeCashFlowYield for 5 annual years. Annual rebalance end of Q1; signal = FY data ending ≤ Dec 31 prior year (3-month reporting lag). 50-stock large-cap universe; top-10 equal-weight. RESULTS: IS (Apr 2022–Mar 2024) Sharpe=0.679, CAGR=11.6%, MaxDD=-13.8%. OOS (Apr 2024–present) Sharpe=1.297, CAGR=15.2%, MaxDD=-10.3%, NegYrs=0. SPY OOS Sharpe=1.552. Corr(SPY)=0.645. WF ratio=1.910. Gate 1 OOS Sharpe ≥ 0.8: PASS. Gate 2 WF ratio ≥ 0.45: PASS. CONFIRMED BUT WEAK: (1) DATA COVERAGE: only 21/50 tickers returned FMP free-tier data — result biased toward FMP-covered subset (AAPL/MSFT/GOOGL/META/ABBV/PFE/UNH/JPM/BAC/WFC etc.); 29 tickers silently dropped, likely due to FMP free-tier ticker coverage limits. (2) BANK FCF ANOMALY: JPM and WFC appear in top-10 with extreme FCF yields (16-27%) — banks don't have traditional capex so FCF = ~net income, making them artificially "high yield" by this metric; standard FCF/P factor research excludes financials. (3) SHORT HISTORY: only 4 annual rebalances (3.5 years total) across a test window that includes 2 IS + 1.5 OOS years — statistically insufficient. (4) WF ratio > 1 (OOS > IS) suggests OOS was unusually favorable, not IS underfitting; 2024 was a strong year for the specific holdings (META/GOOGL/ABBV/AAPL all surged). (5) COWZ ETF (broader FCF screen, longer history) OOS 2020+ Sharpe=0.737 — still below gate, suggesting the factor doesn't reliably beat SPY over full cycles. PRODUCTION ASSESSMENT: NOT recommended. Fix required: re-run excluding financials sector (JPM/BAC/WFC/GS) + verify broader FMP ticker coverage. Script: backtesting/daily/run_h284.py. Results: backtesting/results/h284_results.json.
h283_status: NOT CONFIRMED (2026-06-11) — Bond ETF Carry + Momentum Rotation (H045 Enhancement). Source: Koijen et al. (2018) "Carry" (JFE); Brooks & Moskowitz (2017) "momentum+carry in bonds". Hypothesis: adding a carry signal (trailing 12m dividend yield as bond income proxy) to H045's price momentum improves risk-adjusted returns. Universe: same 13-ETF set as H045 (SHY/IEI/IEF/TLT/TIP/HYG/LQD/BKLN/EMB/BIL/MBB/FLOT/PCY). Carry = sum(12m dividends) / price. Five blend alpha values tested (α=momentum weight, 1-α=carry weight): pure momentum (1.0), 75/25, 50/50, 25/75, pure carry (0.0). IS 2008-2017, OOS 2018-present. RESULTS — Alpha=1.0 (pure momentum, 6m lookback): IS Sharpe=0.783, OOS Sharpe=0.743, MaxDD=-5.9%, NegYrs=3. Alpha=0.75: IS=0.922, OOS=0.679. Alpha=0.50: IS=0.759, OOS=0.602. Alpha=0.25: IS=0.486, OOS=0.464. Pure carry: IS=0.631, OOS=0.393. H045 recomputed (12m mom, top-2): OOS Sharpe=1.426. NOT CONFIRMED: all variants fail gate (OOS > H045 baseline 1.351). Monotonically decreasing performance as carry weight increases — dividend yield carry is anti-correlated with alpha in bonds. AGG B&H OOS Sharpe=0.298 for context. CRITICAL ROOT CAUSE: (1) H283's "pure momentum" alpha=1.0 uses 6-month lookback vs H045's confirmed 12-month lookback — the 6m bond momentum (Sharpe 0.743) is materially weaker than H045's 12m (1.426), confirming H045's 12m choice is optimal; (2) Carry (yield) in bonds systematically favors high-yield credit ETFs (HYG/LQD/EMB) which carry more credit risk — during the 2018-2025 OOS period these underperformed investment-grade during risk-off episodes (2018Q4, 2020 COVID, 2022 rate hikes); (3) The "carry = TTM dividend yield" proxy treats bond income as a pure ranking signal without adjusting for duration or credit risk, so "high carry" = "high risk" rather than "high expected return"; (4) IS Sharpe is consistently higher than OOS as carry weight increases, suggesting IS-period credit spreads provide more discriminatory power than OOS — this is a structural regime shift (post-2022 high-rate environment changes the relationship between yield and expected return); (5) Corr(H283,H045) ranges 0.73 (pure momentum) to 0.34 (pure carry) — diversification potential exists, but not at sufficient OOS Sharpe to justify inclusion. CONCLUSION: Bond ETF carry (dividend yield) does NOT improve on H045's 12m price momentum. The H045 family is already well-optimized. Production portfolio unchanged. Script: backtesting/daily/run_h283.py. Results: backtesting/results/h283_results.json.
h282_status: NOT CONFIRMED (2026-06-11) — Dividend Growth ETF Rotation. Hypothesis: rotating among high-dividend ETFs (DVY/VYM/SDY/VIG/SCHD/DGRO/NOBL + BIL) based on trailing YoY dividend growth rate (TTM dividends_t / TTM dividends_{t-12m} - 1) captures dividend-grower quality premium. Two universes: Core (DVY/VYM/SDY/VIG, data since 2006) IS 2008-2019, OOS 2020-present; Extended (+SCHD/DGRO/NOBL) IS 2016-2019. Four variants each: Top-1 growth, Top-2 growth, Hybrid 50/50 (growth rank + 6m momentum rank) Top-1, Hybrid Top-2. RESULTS — Core variants: A1 Top-1 IS Sharpe=0.556, OOS=0.782, MaxDD=-17.1%, NegYrs=1. A2 Top-2: OOS=0.413. A3 Hybrid Top-1: OOS=0.741, NegYrs=0. A4 Hybrid Top-2: OOS=0.524. Extended variants: B1 Top-1 IS=1.459 OOS=0.782, B2 IS=1.434 OOS=0.413, B3 Hybrid IS=1.488 OOS=0.741. Benchmarks: SPY OOS Sharpe=0.864, DVY B&H OOS=0.558, Equal-weight OOS=0.659. NOT CONFIRMED: best OOS Sharpe=0.782 (A1) < gate 1.0 and < SPY. INTERESTING IS/OOS PATTERN: Extended universe IS Sharpe (1.459) >> Core IS (0.556) — suggests dividend growth signal had genuine IS power in 2016-2019, but both collapse similarly OOS. IS-only period for extended universe is too short (48 months) to draw conclusions. CRITICAL ROOT CAUSE: (1) Dividend growth rate is a backward-looking signal: the ETF that grew dividends most in prior year is already priced in — investors have chased yield, compressing future price returns; (2) 2020-2025 OOS period (post-COVID) saw dividend growth become broadly non-discriminatory — all quality dividend ETFs (SCHD, VIG, DVY) raised dividends similarly in 2021-2023, eliminating cross-sectional signal; (3) "Best dividend grower" last year often = sector that is richly valued (e.g., tech boosted dividends in 2021 via buybacks, but tech ETFs in the universe don't include XLK/QQQ); (4) The rotation lands back in DVY or VYM most months because they had the most consistent growth, making it near-static; (5) Corr(H282,prod) 0.43-0.49 — acceptable diversification, but insufficient OOS Sharpe to justify adding. Hybrid signal (50% growth + 50% momentum) slightly better risk profile (NegYrs=0 for A3) but still insufficient alpha vs SPY. CONCLUSION: Dividend growth rate as ETF rotation signal does not generate alpha in 2020-2025. Dividend investing family is not production-ready without adding quality screening or sector adjustment. Script: backtesting/daily/run_h282.py. Results: backtesting/results/h282_results.json.
h278_status: NOT CONFIRMED (2026-06-10) — Low-Vol Anomaly: Volatility-Parity ETF Portfolio. Universe: USMV/SPLV/XLU/SPHD/XLK/XLF/XLE/XLV (+BIL in some variants). Four variants: A=inverse-vol weights (no BIL), B=inverse-vol+BIL (capped 30%), C=inverse-vol+6m momentum filter, D=vol-targeted inverse-vol (10% target). Best OOS Sharpe=0.8757 (Var C, no BIL) vs H270 baseline 1.290. All variants FAIL gate (OOS Sharpe > H270 baseline 1.290). CRITICAL FINDING: H270's dual-ranking (select top-1 by combined momentum+inv-vol rank) is materially superior to holding all assets with vol-parity weights. "Selection" (concentrated 1-winner) >> "weighting" (diversified vol-parity) for this ETF rotation family. Root cause: (1) Var A/B hold all 8 risky ETFs including high-vol XLE/XLF/XLK which drag down the risk-adjusted return — there is no defensive escape when sectors crash simultaneously; (2) the momentum filter (Var C) helps but still includes correlated assets; (3) vol-parity was designed for uncorrelated assets (stocks vs bonds); 8 sector ETFs are too correlated for vol-parity to provide meaningful diversification; (4) H270's monthly top-1 selection effectively creates a zero-cost momentum filter on a 6-vol-weighted ranking — concentration in the best single asset per month outperforms broad diversification when sectors cycle. Var B best MaxDD=-15.4% (lowest drawdown of all variants) shows defensive value of BIL inclusion. Corr(prod) range 0.52-0.61 — acceptable but OOS Sharpe too low. H270 Var C (OOS Sharpe 1.290) remains the confirmed low-vol strategy. Script: backtesting/daily/run_h278.py. Results: backtesting/results/h278_results.json.
h277_status: CONFIRMED (2026-06-10) — NASDAQ Stock Momentum: 12-1 Skip-Month Signal. ⚠️⚠️ SURVIVORSHIP BIAS: Fixed universe selected with foreknowledge of 2026 survival — results materially inflated. NOT FOR PRODUCTION. Variant A (30-stock NASDAQ/tech, top-5, 12-1 skip-month): OOS Sharpe=1.2223, MaxDD=-45.8%, NegYrs=2, Corr(prod)=0.433. Variant B (41-stock broader universe, 12-1): OOS Sharpe=1.0780, MaxDD=-36.4%, NegYrs=2, Corr(prod)=0.381. CRITICAL SIGNAL FINDING: 12-1 (skip-month) OOS Sharpe=1.22 vs 12-0 (no skip) OOS Sharpe=2.64 — skip-month DRAMATICALLY HURTS performance on NASDAQ tech stocks. Delta=-1.42 Sharpe. This contradicts standard equity momentum research where skip-1-month improves signal. Root cause: NASDAQ tech momentum is persistent and mean-reverting at 1-month horizon is opposite direction — tech stocks that gain in month t continue to gain in month t+1, so including month t in the formation signal (12-0) captures genuine momentum whereas the standard Jegadeesh-Titman skip assumes short-term reversal that does NOT apply in high-momentum tech universe. SPY OOS Sharpe=0.87 vs best variant 1.22 (12-1) or 2.64 (12-0, survivorship bias). Production-additive test: Corr(Var A 12-1, prod)=0.433 < 0.60 — WOULD be additive but bias inflates results. Gate MET (OOS 1.22>1.0) but ONLY because of survivorship bias. NOT recommended for production until rebuilt with historical NASDAQ constituent data (requires Compustat or Bloomberg point-in-time). n_hold sweep: n=5 best OOS (1.22), n=2 worst (0.85, highest MaxDD -57%). Script: backtesting/daily/run_h277.py. Results: backtesting/results/h277_results.json.
h276_status: QUEUED (2026-06-10) — NautilusTrader Execution POC for H261b Commodity Rotation. Source: github.com/nautechsystems/nautilus_trader (23.4k stars). Adapt H261b top-2 momentum signal to NautilusTrader strategy class, run paper mode on Kraken. Crypto commodity proxies (BTC/ETH) as complements to existing ETF rotation. Not a new alpha hypothesis — execution infrastructure POC. Success gate: backtest runs without error; OOS Sharpe within ±0.2 of H261b baseline (0.922). Scaffold: backtesting/daily/run_h276_nautilus_poc.py.
h275_status: QUEUED (2026-06-10) — ML Momentum with Mask-First Bias Correction. Source: arXiv:2507.07107. Apply Boolean tradability mask BEFORE all signal operators to prevent upstream contamination from halted/illiquid bars. Extend H217/H228 with mask-first pattern. Expected: +0.44 Sharpe vs unmasked (from paper). Gate: OOS Sharpe > H217 baseline (1.559). Scaffold: TBD.
h274_status: QUEUED (2026-06-10) — Multi-Agent PEAD Upgrade (3-agent debate). Source: arXiv:2602.23330 (Expert Investment Teams, Oxford/Kyoto) + arXiv:2412.20138 (TradingAgents). 3 agents: (1) FinBERT gate, (2) structured analyst extracting revenue guidance / forward tone, (3) contrarian flagging embedded negatives. Entry only when all 3 confirm. Baseline: H174 WR=81.8%, n=22. Gate: WR > 81.8%, n ≥ 15 OOS. IS 2019-2021, OOS 2022-2024. Scaffold: backtesting/daily/run_h274.py.
...
## H487 — Idiosyncratic Volatility Anomaly, 200-Stock Universe (NOT CONFIRMED)

**Status**: NOT CONFIRMED
**Tested**: 2026-08-02
**Source**: H213 (idiosyncratic vol on 30-stock mega-cap universe — HIGH IVOL outperformed, opposite of Ang, Hodrick, Xing, Zhang JF 2006); H245/H248 (total-vol and beta/BABB signals both retested at 200-stock scale and NOT CONFIRMED). IVOL specifically had not been retested at 200-stock scale — this closes that gap, following the "retest 30-stock finding at 200-stock scale" pattern used by H241/H243/H245/H248.
**Script**: `backtesting/daily/run_h487_ivol_200stock.py`
**Universe**: H241 200-stock S&P 500 large-cap universe (195 tickers available)
**Signal**: IVOL_i(t) = std of residuals from OLS of stock monthly returns on SPY monthly returns, trailing 3-month window (same formula/window as H213)
**IS/OOS**: 2013-2020 / 2021-2026
**Gate**: OOS Sharpe >= 1.5
...
| Variant | IS Sharpe | OOS Sharpe | OOS MaxDD | Neg Yrs | Notes |
|---------|-----------|------------|-----------|---------|-------|
| A bottom quintile (40) low-IVOL, EW | 1.150 | 0.973 | -16.6% | 0 | Ang et al. direction |
| B bottom decile (20) low-IVOL, EW | 1.117 | 1.011 | -14.3% | 0 | Ang et al. direction, concentrated |
| C top quintile (40) high-IVOL, EW | 1.061 | **1.214** | -14.7% | 0 | H213 direction — best variant, still below gate |
| D low-IVOL + high-mom blend | 1.339 | 0.935 | -21.1% | 1 | worst MaxDD and only variant with a negative year |
| SPY B&H | — | 1.010 | -23.9% | — | benchmark |

Corr(best variant H487-C, H241-A 200-stock momentum) OOS: 0.769

**Key findings**:

1. H213's directional finding replicates at 200-stock scale: high-IVOL (Var C) beats low-IVOL (Var A/B) on OOS Sharpe, consistent with a lottery-demand/retail-overpricing mechanism rather than the Ang et al. risk-based IVOL discount — but the effect is weaker at scale and does not clear the 200-stock family's 1.5 gate.
2. Unlike H245 (total vol, OOS 0.626) and H248 (beta/BABB, OOS 0.641), IVOL is the strongest-performing low-vol-family signal retested at 200-stock scale so far — all three fail the gate, but IVOL fails by the smallest margin, suggesting idiosyncratic (stock-specific) risk carries more genuine information than systematic vol/beta once diversified across 200 names.
3. Blending low-IVOL with momentum (Var D) is actively harmful here — it inherits low-IVOL's weaker return profile without avoiding low-IVOL's underperformance, and introduces the family's only negative year. Do not pursue an IVOL-momentum blend further without revisiting the blend weights.
4. Corr(H487-C, H241-A momentum) = 0.769 is well above the ~0.80 correlation ceiling generally used as a diversification cutoff for production blending — moot here since the strategy doesn't confirm, but noted for future reference: high-IVOL is not a meaningfully distinct return stream from momentum at 200-stock scale.

**Verdict**: NOT CONFIRMED. Best OOS Sharpe 1.214 (Var C) < gate 1.5. Low-volatility-anomaly family has now been retested at 200-stock scale across all three natural signal variants (total vol H245, beta H248, idiosyncratic vol H487) — none clear the gate. See H488 for a follow-up macro-gate attempt on the best (Var C) sub-signal.

---

## H488 — High-IVOL (200-stock) with Macro Regime Gate (NOT CONFIRMED)

**Status**: NOT CONFIRMED
**Tested**: 2026-08-02
**Source**: H487 (best variant H487-C, high-IVOL long, OOS Sharpe 1.214 — close to but below the 1.5 gate, MaxDD-limited); H362 (CONFIRMED — VIX/SPY-200MA gate lifted a near-miss low-vol ETF rotation from OOS 1.339 to 1.819, primarily via drawdown reduction). Hypothesis: the same gate pattern might push H487-C over its gate.
**Script**: `backtesting/daily/run_h488_ivol_macro_gate.py`
**Universe/signal**: H487-C (200-stock universe, long top-quintile high-IVOL, EW, monthly rebalance), gated to BIL cash proxy in unfavorable regimes
**IS/OOS**: 2013-2020 / 2021-2026
**Gate**: OOS Sharpe > 1.5; secondary: MaxDD improvement vs H487-C baseline (-14.7%)

**Results**:

| Variant | IS Sharpe | OOS Sharpe | OOS MaxDD | Neg Yrs | Notes |
|---------|-----------|------------|-----------|---------|-------|
| A SPY > 200MA gate | 1.218 | 1.035 | -13.4% | 1 | below baseline |
| B VIX < 20 gate | 0.953 | 0.973 | -10.5% | 0 | best MaxDD, worst Sharpe |
| C SPY > 200MA AND VIX < 25 (joint) | 1.170 | 0.799 | -14.2% | 1 | worst variant |
| D SPY > 200MA OR VIX < 20 (either) | 1.152 | 1.126 | -13.4% | 1 | best gated variant, still below baseline |
| E Baseline (no gate = H487-C) | 1.061 | **1.214** | -14.7% | 0 | ✓ best of all five, and the only variant with 0 negative years |

OOS regime distribution: SPY > 200MA 79.7% of months; VIX < 20 67.2% of months

**Key findings**:

1. Every gated variant underperforms the ungated baseline — the opposite of H362's result on low-vol ETFs. The gate pattern does not generalize to this signal.
2. Root cause: unlike H354's defensive low-vol ETFs (which genuinely do nothing useful during risk-off regimes, so routing to cash is a pure drawdown cut), high-IVOL stocks' *entire edge* is concentrated in exactly the volatile/rally months the gate filters out — e.g. 2021's +43.7% (the single best OOS year across every variant) occurs during a high-vol post-COVID melt-up. Gating on VIX or SPY-200MA removes the alpha-generating regime along with the risk, rather than removing risk while preserving alpha.
3. MaxDD is only modestly improved (best gated MaxDD -10.5% Var B vs baseline -14.7%, a 4.2pp cut) — much smaller than H362's 29% MaxDD improvement — because the ungated baseline had zero negative years to begin with; there wasn't much tail risk left to cut.
...
## H496 — Network-Herding Momentum-Reversal Transition Signal on H198 Stock Universe (NOT CONFIRMED)

**Status**: NOT CONFIRMED
**Tested**: 2026-08-08
**Source**: arXiv:2607.27063 (Jul 2026), "Herding, Momentum, and Reversal in China's A-Share Market: An Agent-Based Network Model with Information Diffusion." Originally staged as `h477_status_new` (2026-07-30) during a dream-cycle session but never executed; renumbered H496 to avoid conflict with the earlier, unrelated H477 (OB filter on H417 60-stock universe, NOT CONFIRMED 2026-07-29). Script (`run_h477_herding_momentum_reversal.py`, 155 lines) was fully implemented and already committed (`81380d3`, 2026-07-30) but sat unrun for over a week; executed this session as part of clearing the STAGED backlog.

**Design**: Local herding creates positive momentum autocorrelation (phase 3-9 months); as information diffuses through the full investor network, late adopters realize overpricing and reversal follows (onset 9-18 months). Implementation tracks a "streak counter" — consecutive months each stock has ranked in the top-6 by 6-1m momentum — as a proxy for network saturation. Five variants: A (exclude stocks with streak ≥ 6 months from selection, falling back to full ranking if this excludes everyone), B (penalize: `adj_score = raw_score × exp(-0.1 × streak)`, still pick top-6), C (double-weight brand-new top-6 entrants, streak == 0), D (reversal gate: drop stocks with streak ≥ 9 entirely, pick top-6 of the remainder), E (H198 baseline 6-1m top-6, no streak adjustment — sanity check).
**Script**: `backtesting/daily/run_h477_herding_momentum_reversal.py`
**Universe**: NASDAQ_30 (same 30-stock H198 universe)
**IS/OOS**: 2013-2020 / 2021-2026
**Gate**: OOS Sharpe > 1.174 (H198 baseline) AND MaxDD improvement vs. baseline (Var E)

**Results**:

| Variant | IS Sharpe | IS CAGR | IS MaxDD | OOS Sharpe | OOS CAGR | OOS MaxDD | Gate |
|---|---|---|---|---|---|---|---|
| A (exclude streak≥6) | 1.973 | 47.7% | -24.8% | 1.124 | 29.9% | -26.5% | fail |
| B (exponential penalty) | 1.916 | 47.0% | -26.1% | 1.140 | 30.4% | -22.9% | fail |
| C (double-weight new entrants) | 1.839 | 45.9% | -27.9% | 1.279 | 36.5% | -29.4% | fail (see note) |
| D (reversal gate, streak≥9 drop) | 2.003 | 49.0% | -24.8% | 1.141 | 32.0% | -24.0% | fail |
| E (H198 baseline, sanity check) | 2.023 | 50.0% | -24.8% | 1.153 | 32.6% | -24.0% | n/a (baseline) |

**Key findings**:

1. **Gate-logic discrepancy caught and corrected**: the script's own printed gate check only tests `oos_sharpe > 1.174` and labels Var C "PASS" (OOS 1.279 > 1.174) without actually evaluating the "AND MaxDD improvement" half of its own stated gate, despite printing that full description. Checked by hand: Var C's OOS MaxDD is -29.4%, *worse* than baseline Var E's -24.0% — a 5.4pp deterioration, not an improvement. Under the gate as actually stated (both conditions required), Var C fails. This is the same class of script/gate-description mismatch this log has flagged before (e.g. H495's `is_baseline` guard, H419-family look-ahead-bias catches) — the printed "PASS" cannot be trusted without reading the underlying numbers.
2. All four streak-adjustment variants (A/B/C/D) underperform or barely track the unadjusted baseline (E) on OOS Sharpe, and none improve MaxDD. The paper's herding/reversal mechanism (documented on China A-shares, an investor base with materially different herding dynamics than US mega-cap NASDAQ) does not transfer to this universe — consistent with this log's general finding (H291, H336) that a 30-stock US mega-cap universe is too homogeneous/liquid for network-diffusion-style microstructure effects to show up cleanly.
3. Var C's higher raw OOS Sharpe (1.279) comes from double-weighting brand-new top-6 entrants, which in practice just means concentrating more heavily into whichever stock is having its single best momentum month — a return-chasing effect that inflates both CAGR and volatility (hence the worse MaxDD), not a genuine risk-adjusted improvement.
4. No results JSON is saved by this script (confirmed: no `json.dump`/`open(..., 'w')` calls in the source) — results here are captured directly from the terminal run.

**Production correlation estimate**: All variants are built on the same 6-1m top-6 momentum base as H198/H417-family strategies already represented in the "momentum/rotation" side of prior research; none confirm, so no new correlation analysis vs. the production H041a/H026/H045/IBS blend is warranted.

**Verdict**: NOT CONFIRMED. None of the four streak-based herding/reversal adjustments beat the unadjusted H198 baseline on the full stated dual gate (OOS Sharpe > 1.174 AND MaxDD improvement); the one variant whose Sharpe alone exceeds 1.174 (Var C, 1.279) has decisively worse MaxDD than baseline, so it fails when the gate is applied correctly. Close out `h477_status_new` — no further work recommended on this specific streak-counter mechanism for this universe.

**Results file**: not saved (script has no JSON output; results captured from terminal run above)

---

## H497 — Split-Session Cluster GARCH Overnight/Intraday Tail Decomposition as IBS Entry Filter (NOT CONFIRMED)

**Status**: NOT CONFIRMED
**Tested**: 2026-08-08
**Source**: arXiv:2607.03669 (Chen, Hansen, Tong, Jul 2026), "Split-Session Cluster GARCH for Overnight and Intraday Returns: The Role of Tail Heterogeneity." Originally staged as `h476_status_new` (2026-07-30); renumbered H497 to avoid conflict with the earlier, unrelated H476 (OB filter on H417 60-stock universe, NOT CONFIRMED 2026-07-29). Script (`run_h476_split_session_garch_ibs.py`, 130 lines) was fully implemented and already committed (`81380d3`) but unrun; executed this session clearing the STAGED backlog.

**Design**: The paper finds overnight returns have heavier tails (ν≈3.8) than intraday returns (ν≈5.2), and that fat-tail overnight days (news-shock driven) tend toward trend continuation rather than mean-reversion — a problem for H112's IBS strategy (30% of the production portfolio), which is a pure mean-reversion play on the internal bar range. Implementation computes `overnight_rv = (log(open_t / close_{t-1}))²` and its rolling 252-day percentile rank, then gates IBS entries by this rank. Variants: A (binary gate: skip IBS entry when overnight_rv_rank > 0.8), B (continuous: scale IBS position size by `1 - overnight_rv_rank`), C (anti-contrarian/inverted: only enter when overnight_rv_rank < 0.2, i.e., the opposite tail condition), D (H112 IBS baseline, no filter).
**Script**: `backtesting/daily/run_h476_split_session_garch_ibs.py`
**IS/OOS**: 2015-2020 / 2021-2026
**Gate**: OOS Sharpe > 2.129 (H112 IBS baseline) AND MaxDD improvement vs. -18%

**Results**:

| Variant | IS Sharpe | IS CAGR | IS MaxDD | Neg Yrs (IS) | OOS Sharpe | OOS CAGR | OOS MaxDD | Neg Yrs (OOS) | Gate |
|---|---|---|---|---|---|---|---|---|---|
| A (binary gate >0.8) | 0.874 | 10.4% | -24.1% | 1 | 0.688 | 10.1% | -15.4% | 1 | fail |
| B (continuous scale) | 0.845 | 5.2% | -6.9% | 2 | 0.729 | 6.3% | -10.6% | 1 | fail |
| C (anti-contrarian <0.2) | 1.032 | 4.4% | -5.0% | 0 | 0.296 | 2.1% | -13.0% | 1 | fail |
| D (H112 IBS baseline) | 0.617 | 9.0% | -23.8% | 1 | 0.804 | 14.8% | -20.0% | 1 | fail |

**Key findings**:

1. Every variant, including the baseline reproduction itself (D), fails decisively against the stated gate of OOS Sharpe > 2.129 — the best OOS result across all four rows is 0.804 (baseline D), roughly 2.6x short of the gate. This gate references the *production* H112 IBS OOS figure (2.129), while this script's own IS/OOS split and universe reproduce a much lower baseline (0.804) — indicating the gate benchmark and this script's baseline reproduction are not apples-to-apples (likely a different date split, universe, or position-sizing convention than the live production H112 implementation). Even accounting for that gap generously, none of the overnight-tail-filtered variants (A/B/C) beat their own baseline (D) either — the filter actively hurts OOS Sharpe in every case.
2. The anti-contrarian variant (C) has the best IS Sharpe (1.032, lowest IS MaxDD) but the worst OOS Sharpe (0.296) — a classic IS-overfit pattern: restricting entries to only the calmest overnight sessions concentrates IS returns into low-vol wins but starves the strategy of the OOS opportunity set.
3. Unlike H496, no gate-logic bug was found here — the script's printed FAIL labels for A/B/C/D all check out correctly by hand.
4. No results JSON saved (confirmed: no `json.dump`/save calls in source); results captured from terminal output.

**Production correlation estimate**: Since no variant clears its own gate (nor the informal comparison to production H112 IBS OOS 2.129), this filter should not be applied to the production 30% IBS sleeve. No correlation analysis performed given the outright fail.

**Verdict**: NOT CONFIRMED. The overnight/intraday tail-heterogeneity gate does not improve H112's IBS mean-reversion strategy in any tested form — all three filtered variants underperform the unfiltered baseline OOS, and the baseline itself falls well short of the production H112 OOS figure used as the gate, suggesting a reproduction/parameterization gap worth investigating separately if this family is revisited. Close out `h476_status_new` — no further work recommended on this specific overnight-tail-gate mechanism.

**Results file**: not saved (script has no JSON output; results captured from terminal run above)

---

## H498 — Market Regime Council (MRC) Shapley-Weighted Multi-Agent ETF Rotation on H026 Universe (NOT CONFIRMED)

**Status**: NOT CONFIRMED
**Tested**: 2026-08-08
**Source**: arXiv:2605.24490 (Pei, Ge, Zheng & Cartlidge, May 2026), "Market Regime Council for Dynamic Credit Assignment in Multi-Agent LLM Decision Systems." Originally staged as `h434_status` (2026-07-23); script (`run_h434_mrc_shapley_rotation.py`, 350 lines) was fully implemented and committed (`81380d3`) but unrun until this session. Direct extension of H318's meta-agent-selector idea (still PROPOSED/unimplemented): rather than assuming one specialist agent dominates, MRC computes all coalition subsets of N agents, tracks exponentially-decayed Shapley credit per coalition, and blends the final allocation by those Shapley weights — credit adapts as regimes shift instead of being fixed.

**Design**: Three deterministic specialist agents on the H026 25-ETF universe: (1) Momentum — 12m momentum + inverse-vol rank (H026 canonical); (2) Macro — SPY 200MA + VIX<20 gate (H301/H165a); (3) OB-Filtered — OB-filtered top-2 (H346 best params, window=20/swing_len=3). All 2³−1=7 non-empty coalition outputs are computed monthly; Shapley credit is exponentially decayed and accumulated per coalition; final blended allocation weights each agent by its running Shapley value. Five variants compared: MRC (full Shapley-weighted blend), EQUAL (naive equal-weight blend of the same 3 agents, no credit assignment), MOMENTUM_ONLY, OB_ONLY, MAJORITY (simple vote among the 3 agents).
**Script**: `backtesting/daily/run_h434_mrc_shapley_rotation.py`
**Universe**: H026 25-asset ETF rotation universe
**IS/OOS**: 2008-2017 / 2018-2026 (H026 canonical split)
**Gate**: OOS Sharpe > 3.238 (H346 canonical OOS) AND MaxDD < -6.7%

**Data caveat**: VIX fetch from FRED (`api.stlouisfed.org`) failed with an SSL certificate verification error (`self-signed certificate in certificate chain`) — the same general class of OneCLI-proxy/CA-bundle issue already documented in the task registry for Alpaca and GitHub endpoints, now observed for the first time against FRED. The script degraded gracefully to a constant VIX=18 fallback and completed the run, but this weakens the fidelity of the Macro specialist agent (its VIX<20 gate effectively became a permanent pass-through for the whole backtest). Not yet fixed at the infra level — flagged for a future task-registry.md update if it recurs.

**Results**:

| Strategy | IS Sharpe | IS CAGR | IS MaxDD | Neg Yrs (IS) | OOS Sharpe | OOS CAGR | OOS MaxDD | Neg Yrs (OOS) | Gate |
|---|---|---|---|---|---|---|---|---|---|
| MRC (Shapley-weighted) | 0.593 | 7.1% | -16.6% | 1 | 0.933 | 18.8% | -15.1% | 2 | fail |
| EQUAL (naive equal-weight) | 0.587 | 7.0% | -16.6% | 1 | 1.008 | 19.4% | -14.3% | 2 | fail |
| MOMENTUM_ONLY | 0.680 | 9.1% | -20.2% | 2 | 0.897 | 15.5% | -22.6% | 2 | fail |
| OB_ONLY | 0.445 | 9.3% | -42.2% | 2 | 0.727 | 23.5% | -35.2% | 3 | fail |
| MAJORITY (vote) | 0.680 | 9.1% | -20.2% | 2 | 0.646 | 13.2% | -23.0% | 3 | fail |

**Key findings**:

1. All five variants fail the gate decisively (best OOS 1.008, vs. gate 3.238) — this is expected given the gate benchmarks against H346's specific OB-filter-on-canonical-split result (3.238), a high bar not designed with a 3-agent ensemble blend in mind.
2. **The paper's core mechanism does not earn its complexity here**: Shapley-weighted MRC (OOS Sharpe 0.933) actually *underperforms* the naive EQUAL blend of the exact same 3 agents (OOS Sharpe 1.008). Adaptive credit assignment adds computational overhead (O(2^N × T) Shapley value tracking) without producing better allocation decisions than simply averaging the agents' signals — the regime-shift-adaptivity the paper is built around isn't visibly earning its keep on this 3-agent, monthly-rebalanced ETF universe.
3. OB_ONLY has the best OOS CAGR (23.5%) but by far the worst OOS MaxDD (-35.2%) and 3 negative years — consistent with this log's broader finding that the OB filter's edge (H343-H346, H355, H361) is a *selection* improvement, not a risk-reduction one, and needs pairing with a macro/momentum overlay rather than standing alone.
4. MAJORITY (vote) is the single worst performer OOS (0.646) — a hard voting rule among 3 heterogeneous agents throws away information that even a naive weighted average preserves.
5. Results ARE saved (unlike H496/H497): `backtesting/results/h434_results.json`.

**Production correlation estimate**: None of the 5 variants confirm, so no addition to the production H041a/H026/H045/IBS blend is warranted. Directionally, EQUAL/MRC/MOMENTUM_ONLY are all built from H026-family signals already represented in production (27% allocation), so even a passing result here would likely have shown high correlation to the existing H026 sleeve rather than being a new diversification source — consistent with this log's pattern for every H026-derivative test to date (H301, H429, H447 attempt).

**Verdict**: NOT CONFIRMED. All 5 variants fail the OOS Sharpe > 3.238 gate. The most notable finding is negative for the paper's central claim: Shapley-weighted dynamic credit assignment (0.933) underperforms a naive equal-weight blend (1.008) of the same 3 specialist agents on this universe — multi-agent adaptive weighting adds complexity without adding value here. Recommend closing `h434_status` without further iteration on the Shapley mechanism specifically; if multi-agent ETF rotation is revisited, a naive-blend-first sanity check (as done here) should be the standing baseline before any adaptive-weighting scheme is considered worth its complexity.

**Results file**: `backtesting/results/h434_results.json`

---

## H499 — Heavy-Tail HMM Emission Families for Regime-Conditional VaR on H026 (NOT COMPLETED — compute defect)

**Status**: NOT COMPLETED
**Tested**: 2026-08-08 (two attempts, both failed to finish)
**Source**: arXiv:2606.23492 (Alswaidan, Jin, Varner 2026), "Continuous Hidden Markov Models for Equity Returns: Heavy-Tail Emission Families and Regime-Conditional Value-at-Risk." Originally staged as `h445_status` (2026-07-25); script (`run_h445_heavytail_hmm_h026.py`, 376 lines) was fully implemented and committed (`81380d3`) but unrun. Attempted this session as the 4th backlog item; unlike H496-H498, it did not complete.
**Design (from script)**: Student-t (GMM-approximated) and Laplace (moment-matched) HMM emission families vs. a Gaussian baseline, fit on daily H026 ETF portfolio returns with rolling re-estimation; filtered regime probabilities used to scale position size down (toward BIL) in high-vol regimes. Variants: A (Student-t + VaR scaling), B (Laplace + VaR scaling), C (Student-t + binary bull/bear gate, no scaling), D (Gaussian + VaR scaling, ablation baseline).
**Script**: `backtesting/daily/run_h445_heavytail_hmm_h026.py`
**Universe**: H026 25-asset ETF rotation universe
**IS/OOS**: 2008-2017 / 2018-2026
**Gate**: OOS Sharpe > 1.0 AND Kupiec VaR-coverage test p > 0.05 (per the staged header spec; note the script's own docstring states a different, inconsistent gate of "OOS Sharpe > 2.0 AND MaxDD > -8%" — this discrepancy would need resolving before a real run is scored, in addition to the compute problem below)

**What happened**: The script printed only the H026 canonical no-overlay baseline (OOS CAGR=16.09%, Sharpe=0.680, MaxDD=-48.46%) and then entered the GMMHMM fit for Variant A, which produced a continuous stream of `Degenerate mixture covariance` warnings with no further progress. Two attempts were made: (1) background run with a 280-second timeout — killed with no output beyond "Terminated"; (2) foreground rerun with unbuffered output (`python3 -u`) and a 550-second timeout — this time captured the baseline print and ~230 lines of repeated `Degenerate mixture covariance` warnings before being killed, still stuck inside the same Variant A GMMHMM fit. This mirrors a documented precedent in this log for `h447_status` (Wasserstein-tracked regime-aware MVO, 2026-07-25): "Script timed out at 8 min even with monthly re-fit... non-convergent... Deferred to future session." H445's GMMHMM (Student-t approximation via Gaussian mixture) is evidently hitting a similar convergence pathology — likely refitting per rolling window across a long (2006-2026) daily history, with the mixture repeatedly degenerating (collapsing to near-singular covariance) and retrying.

**Diagnosis (not fixed this session, per standing no-scope-creep guidance)**: the degenerate-covariance retry loop most likely stems from either (a) too many rolling refits over a long daily-frequency history × 25-asset portfolio returns, or (b) the GMM approximation for Student-t needing regularization (minimum covariance floor) that the script doesn't currently apply. Following the H447 precedent, this is being deferred rather than force-fixed off-task — a proper fix would need either a coarser refit cadence, a covariance floor/regularization term, or switching the Student-t approximation to `scipy.stats.t`-based direct MLE instead of a GMM proxy.

**Production correlation estimate**: N/A — no results produced.

**Verdict**: NOT COMPLETED. This is not a NOT CONFIRMED result — the script never produced usable output for any variant. Recommend either (a) a dedicated follow-up session to fix the GMMHMM convergence issue (add covariance regularization or reduce refit frequency) before attempting a real run, or (b) replacing the GMM-based Student-t approximation with `scipy.stats.t.fit()` directly, which would avoid the EM-based mixture-degeneracy failure mode entirely. Also flag the gate-description mismatch between the script's docstring and its original staged header spec for resolution alongside any future rerun.
