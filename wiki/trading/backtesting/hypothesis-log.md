...
h168_status: IN-PROGRESS (2026-05-06, v2) — PEAD Speaker-Weighted FinBERT: run_h168.py WRITTEN + updated. Data: AlphaVantage EARNINGS_CALL_TRANSCRIPT API (25 req/day free tier). AV format: speaker-segmented JSON {speaker, title, content, sentiment}. Weights: Analyst 49%, CFO 30%, Executive 16%, Other 5% (arXiv:2604.13260 Table 4). SCRIPT UPDATES (2026-05-05): (1) GAP_THRESH changed 0.05→0.03 to match H163 (~203 expected events vs 101 at 5%); (2) earnings calendar filter added via yfinance earnings_dates (covers April 2020+; ~12% IS events in Jan-Mar 2020 will be missed); (3) rate-limit sentinel bug fixed — no longer saves [] sentinel on rate-limit hit, uses global _AV_RATE_LIMITED flag to stop pass cleanly. TRANSCRIPT CACHE STATUS: 22 files cached total; only 1 of 224 filtered-universe events has a cached transcript (LOW is NOT in filtered universe — MSFT_2020Q3 and AAPL_2020Q1 are likely the 1 in-universe hit). Q4-2019 era downloads from pass 1 are retained but unused. DOWNLOAD PASSES: Pass 1 (2026-05-05 00:05 UTC, 22 fetches) = 22 Q4 2019 transcripts. Pass 2 (2026-05-05 00:17 UTC) = rate-limited immediately (quota exhausted by pass 1). Pass 3 (2026-05-06 00:15 UTC) = rate-limited on first call despite being past UTC midnight — AV quota may not reset at exactly 00:00 UTC; retry queued for 02:00 UTC. First valid events in filtered universe: TSLA 2020-07-23, AMD 2020-07-24, etc. (Q2 2020 earnings). BLOCKERS: (1) AV rate limit 25/day → ~9 passes needed; quota reset time appears to be after 00:15 UTC (not exactly midnight); (2) H163 IS signal TBD — if IS signal is near-zero, H168 may be deprioritized in favor of H171 (GPT-4o-mini, $0.48 total cost). Script: backtesting/daily/run_h168.py.
