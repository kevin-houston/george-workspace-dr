---
updated: 2026-05-04
h169_status: QUEUED (dream cycle 2026-05-04) — LLM-Augmented Pair Selection for H160+: use 10-K text embeddings + LLM edge filtering to improve equity pair selection beyond ad-hoc choices. arXiv:2604.19476 (April 2026): sparse candidate graph from sentence-BERT 10-K embeddings → LLM classifies economic relations → cointegration-filtered pairs. S&P 500 2011-2019: Sharpe 0.742→0.820, MaxDD -10.47%→-7.85%. Method: (1) download 10-K Item 1 (business description) for target universe via edgartools, (2) compute sentence-BERT embeddings (all-MiniLM-L6-v2, 22M params, fast), (3) build candidate pairs graph (cosine sim > 0.7), (4) LLM filter: for each candidate pair, ask 'Does company A and company B share common risk factors that would cause cointegration?' (yes/no + reasoning), (5) run cointegration test on LLM-approved pairs only. Libraries: sentence-transformers (pip install sentence-transformers), edgartools. Data: 10-K annual filings via EDGAR. PREREQUISITE: H160 (factor-residualized pairs) must reach PARTIAL or CONFIRMED first. Estimated 15h. Queue after H160.
h168_status: QUEUED (dream cycle 2026-05-04) — PEAD Speaker-Weighted FinBERT: upgrade H163 design by parsing earnings call transcripts into speaker segments (prepared remarks vs Q&A, CEO vs CFO vs analyst) and applying empirically-derived weights (Analyst 49%, CFO 30%, Executive 16%, Other 5%) to the FinBERT sentiment score. arXiv:2604.13260 (April 2026): 16,428 S&P 500 earnings calls 2015-2025, OOS Spearman IC = 0.142, monthly long-short alpha = 2.03% (t=6.49), subsumes LM dictionary entirely (LM t=0.86). Key insight: analyst Q&A tone has the highest predictive weight — analysts' pointed questions extract non-obvious signals the prepared remarks conceal. Implementation: segment 8-K transcript by speaker tag, run FinBERT per segment, compute weighted_score = 0.49*analyst_score + 0.30*cfo_score + 0.16*exec_score + 0.05*other_score. Data: edgartools Item 2.02 + transcript section parsing. Estimated 10h. Queue after H163 (which tests flat-average baseline first).
...
h165_status: QUEUED (dream cycle 2026-05-03) — TradingAgents macro-regime layer: run LLM analyst agent (TauricResearch/TradingAgents v0.2.4, arXiv 2412.20138) nightly to produce bullish/bearish/neutral macro signal, use as a gate on H026 (H018 ETF rotation). Supports Claude 4.x natively. Use case 1: regime gate → exit to BIL faster than 12m TSMOM alone during macro stress events (COVID 2020, rate hike 2022). Use case 2: news-analyst filter on H159b PEAD entries. Estimated 6h. Prerequisite: benchmark against simple VIX threshold (free) before committing API costs. Queue after H163/H164.
h164_status: QUEUED (dream cycle 2026-05-03) — PEAD-ML: elastic-net regression over 8 prior quarters of standardized earnings surprises (SUE history) to predict 60-day drift. Finance Research Letters 2025 reports this "near-doubles Sharpe" vs single-quarter SUE. Signal: ElasticNetCV(l1_ratio=[0.1..1.0]) on [SUE_q-1 .. SUE_q-8], target = 60-day abnormal return. Data: FMP /v3/earnings-surprises/{symbol} (250 req/day free; $FMP_API_KEY in vault). Requires 8 quarters of EPS history. Best combined with H159b beta-hedge. Estimated 12h. Queue after H159b.
h163_status: QUEUED (dream cycle 2026-05-03) — PEAD-NLP: FinBERT sentiment filter on earnings call transcripts as additive signal layer for H159b. ACL 2025 FinNLP-2: FinBERT (ProsusAI/finbert, Apache 2.0, ~110MB) achieves 57.6–58.3% directional accuracy. Pipeline: download SEC 8-K Item 2.02 → segment by speaker → sentence-level FinBERT → finbert_score = mean(positive) − mean(negative) → filter: enter H159b position only if finbert_score > +0.1. Expected: base win rate 63.9% → 68–72% on retained events (~20% fewer trades). Libraries: transformers>=4.30, torch, sec-edgar-downloader. Estimated 8h. Queue after H159b. Test on existing 374 OOS events from H159.
h162_status: QUEUED — Covered calls around ex-dividend: sell 2% OTM call 10 trading days before ex-dividend date, close at ex-date. Signal: find tickers with ex-dates exactly 10 trading days away. Prices call via Black-Scholes. Based on Ernesto/R25 result: Sharpe +2.643, 3× better than static covered calls. Universe: 50 dividend-paying large-caps. Benchmark: covered call ETF (JEPI). Requires: options pricing model (scipy/vollib), dividend calendar data (yfinance). Queued after H161.
h161_status: QUEUED — Dividend Raise Signal: company raises quarterly dividend ≥10% vs prior payment → enter on ex-dividend date, hold 40 trading days, equal-weight $500/trade, max 10 positions. Based on Ernesto/R27: Sharpe +4.403 per-trade (n=345 events, 10yr), WR 64.9%, p=0.000. Universe: 50 large-cap dividend payers. Data: yfinance Ticker.dividends (quarterly payments). Signal: pct change in div ≥10% vs prior. Queued after H160.
...
h160_status: QUEUED — Factor residualization for equity pairs: remove market + sector factor from price series before computing z-scores. Regress each stock against SPY (and sector ETF) on rolling 252-day window, use residuals for spread. Ernesto/R29 result: Sharpe 0.44 → 1.38 (3× improvement). Best pairs from Ernesto: MSFT/TXN, TXN/META, AMZN/TSLA, BAC/GS, JNJ/UNH. Test on same pairs + add cross-sector pairs to confirm orthogonality. Requires: statsmodels rolling OLS, same z-score framework as H152-H154. Queued after H159.
