...
h284_status: NOT CONFIRMED (2026-06-12) — FCF/P Stock Screener (Free Cash Flow Yield Factor). Source: Yartseva 2025 "What Makes a Multibagger?" (FCF/P = #1 predictor of multibagger outperformance, cross-section 2000-2023, 15%/yr quintile spread). Hypothesis: long top-quintile FCF yield (TTM Free Cash Flow / Market Cap) stocks outperform on risk-adjusted basis. Design: attempted stock-level screener (50 S&P 500 large-cap stocks, quarterly rebalance using yfinance fundamental data) and ETF-based proxy (COWZ/CALF/FCEF). DATA LIMITATION: yfinance quarterly_cashflow provides only ~7 quarters of history per request (back to ~Q3 2024), insufficient for the 2008-2017 IS period. FMP API key-metrics endpoint returns 403 (requires paid plan). Could not construct stock-level FCF yield time series beyond 2024. FALLBACK (ETF proxy): COWZ (Pacer US Cash Cows 100 ETF — top 100 Russell 1000 by FCF yield, quarterly rebalance) IS 2017-2019 Sharpe=0.615, OOS (2020+) Sharpe=0.737, MaxDD=-23.8%, Corr(SPY)=0.816. CALF (small-cap cash cows): OOS Sharpe=0.463. SPY OOS (2020+): Sharpe=0.920. NOT CONFIRMED: COWZ OOS Sharpe=0.737 < gate 0.8, and COWZ underperforms SPY by -1.83pp Sharpe. ROOT CAUSE: (1) FCF yield is a value signal — it underperforms in growth/momentum regimes (2020-2025 dominated by NVDA/TSLA/META which have low FCF yield due to high valuations but extreme price appreciation); (2) COWZ screens for high FCF yield but excludes high-multiple growth stocks, exactly the names that drove 2020-2025 returns; (3) The 2022 value outperformance year (COWZ +0% vs SPY -18%) is real but one year's outperformance is insufficient to lift OOS Sharpe above SPY's consistent 2020-2025 bull market performance; (4) Corr(COWZ,SPY)=0.816 — while not adding alpha, the correlation is high enough that COWZ doesn't provide meaningful diversification either. IMPORTANT NOTE: H286 shows the FCF/P factor becomes more competitive when regime-gated (Var B OOS Sharpe=1.031) — but standalone FCF yield factor ETF does not outperform SPY. DATA PREREQUISITE for proper test: FMP Professional plan or Compustat point-in-time data for 10-year fundamental history. Script: backtesting/daily/run_h284.py. Results: backtesting/results/h284_results.json.
