---
updated: 2026-05-18
h206_status: QUEUED (2026-05-18) — Halloween Effect (Sell in May) on SPY with TOM Composite. Source: Bouman & Jacobsen (2002); Schroeder (IJFS, Nov 2025, doi:10.3390/ijfs13040208) — SEC disclosures 17% higher in winter (Nov–Apr), February peak, September trough; 22% more insider trading, 473% more annual reports in winter. Mechanism: structural information-flow driver tied to fiscal year-end and regulatory cycles — durable, not easily arbitraged. Recent edge reduced (~2.8%/yr annualized 2010–2024) but TOM composite may restore it. Design: H206-A (Halloween standalone: hold SPY Nov–Apr, BIL May–Oct); H206-B (Halloween+TOM composite: hold SPY only during TOM windows in Nov–Apr). IS: 2003–2017, OOS: 2018–2026. Confirm gates: H206-A OOS Sharpe > 0.6; H206-B OOS Sharpe > 0.8. Script: backtesting/daily/run_h206.py. Prerequisites: H205 complete. Staged from dream cycle 2026-05-18.
...
h205_status: QUEUED (2026-05-17) — TOM Calendar Overlay on H192-D BAB. Motivation: H203 showed that TOM timing (low correlation with momentum signals, Corr(S1,S3)=0.190) cannot be beneficially blended via HRP weight allocation because HRP over-indexes on TOM's low variance. Alternative: use TOM as a TIMING GATE on the best satellite strategy (H192-D sector-neutral BAB, OOS Sharpe=1.367). Design: hold H192-D BAB stock positions only during TOM windows (last 2 + first 2 trading days), hold BIL otherwise. Monthly decision: at each month-end, select the 6 lowest-beta-within-sector stocks (H192-D signal), but only hold them from day -2 to day +2 around month-end; outside this window, hold BIL (T-bill). Hypothesis: the BAB signal identifies which stocks to hold; the TOM overlay times WHEN to hold them. Predict: OOS Sharpe > H192-D pure (1.367) because adding TOM calendar filter concentrates exposure in the high-premium window while reducing market exposure the rest of the time. Risk: if BAB alpha is spread uniformly across the month (not calendar-concentrated), TOM overlay will hurt by reducing time in market and CAGR. IS: 2013–2020, OOS: 2021–2026. Confirm: OOS Sharpe > 1.5 (H192-D 1.367 + meaningful TOM overlay benefit). Secondary: Cumul(overlay) > Cumul(H192-D pure) (the timing lift must overcome the reduced time-in-market). Script: backtesting/daily/run_h205.py.
h204_status: QUEUED (2026-05-16) — Deep Reinforcement Learning vs Momentum Baseline. Source: AI4Finance-Foundation/FinRL; arXiv:2511.12120 (PPO+A2C+DDPG ensemble); arXiv:2508.20103 (DDPG-TiDE, Sharpe 1.13). Hypothesis: A PPO agent trained on our 30-stock IS (2013–2020) achieves OOS Sharpe > H198 6-1m momentum baseline (1.174) on 2021–2026. Algorithm: PPO (stable-baselines3), 5-seed ensemble, average weights. State: 60-day return history per stock (60×30 rolling window) + current portfolio weights (30). Action: continuous portfolio weights [-1, 1] per stock (softmax normalized). Reward: raw daily return minus 10bps turnover cost. IS: 2013–2020 (strict temporal cutoff). OOS: 2021–2026 (same as H198). Universe: same 30 S&P 500 stocks as H198. Confirm: OOS Sharpe > 0.8 (secondary: realistic expectation 0.8–1.2 given 2022 bear market in OOS). Secondary: Corr(H204, H198) < 0.6 — useful for H203 blend extension regardless of absolute Sharpe. CRITICAL SAFEGUARD: state at step t must use only data from t-252 to t-1 — no look-ahead in normalization windows. Honest expectation: OOS Sharpe likely 0.8–1.2 (below H198) because training period 2013-2020 is mostly bull market; OOS 2021-2026 includes 2022 bear and elevated macro volatility. Value regardless of absolute Sharpe: Corr(H204, H198) < 0.6 makes it useful for blending in H203 extension. Dependencies: pip install stable-baselines3 gymnasium. See: wiki/trading/algorithms/deep-rl-trading.md. Script: backtesting/daily/run_h204.py.
