...
h159b_status: QUEUED — Beta-hedged PEAD: same gap >5% signal but pair each long position with a proportional SPY short to remove market beta (beta estimated from rolling 60-day regression). Entry: long stock at open, short SPY at open in proportion to beta. Hold 20 days, close both legs. Goal: capture the 4.39% mean 20-day drift as pure alpha without equity beta drawdown. Expected: if beta-hedging removes the correlation with SPY (currently 0.59–0.67), the Sharpe could approach the per-event t-stat of 5.64. Requires: beta estimation, SPY short simulation (assume 0 margin cost for simplicity). Queued after H159.
