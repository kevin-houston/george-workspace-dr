...
Last scan completed: 2026-08-09 (2 staged proposals, 2 new wiki pages)

## [2026-08-09] dream-cycle | arXiv scan + wiki expansion (Impact Investing)
Pages created: 2 (sources/impact-irr-modern-portfolio-theory-2025.md, concepts/esg-tail-risk-stress-resilience-2026.md). Pages updated: 6 (index, concepts/impact-investing.md, concepts/esg-factor-integration.md, concepts/impact-measurement-standards.md, concepts/three-dimensional-portfolio-framework.md, trading/algorithms/regime-detection.md, trading/algorithms/market-timing-overlays.md). Hypotheses staged: 0 (Impact Investing is a wiki-knowledge section, not a backtestable hypothesis line — staged 2 wiki_update proposals instead, see dream_cycle/staged/2026-08-09/). Sources reviewed: arXiv:2509.22600 (Soliman, Impact IRR — MPT-derived scalar for the impact dimension), arXiv:2606.05631 (Hu, Yi, Chen, Sun, Zhan — ESG tail-risk/stress resilience via double ML, finds effect is stress-conditional not static), arXiv:2601.08676 (ESGAgent — multi-agent LLM system for ESG analysis, added as a subsection to esg-factor-integration.md rather than a standalone page). Wiki expansion target: Impact Investing — chosen over trading/ and wiki/sources/ (both touched earlier today per git log) because its foundational pages (impact-investing.md, three-dimensional-portfolio-framework.md, jpmorgan-social-finance.md, yasemin-saltuk.md) were stale since 2026-05-05 despite 20 existing entries. Cross-linked both new pages bidirectionally into existing Impact Investing pages and into the trading side (regime-detection.md, market-timing-overlays.md) as regime-gate design candidates alongside H165/H249/H301/H362. Index page count: 299 → 301.

## [2026-08-09] ingest | Foundations of Reinforcement Learning with Applications in Finance (Rao & Jelvis)
Pages created: 3 (sources/rl-for-finance-book-rao-jelvis.md, concepts/mdp-bellman-equations.md, people/ashwin-rao.md). Pages updated: 5 (index, trading/algorithms/deep-rl-trading.md, trading/algorithms/sciphy-rl-neural-bl-portfolio.md, trading/algorithms/market-microstructure.md, people/igor-halperin.md). Source: https://stanford.edu/~ashlearn/RLForFinanceBook/book.pdf, shared by Kevin (538-page textbook, Ashwin Rao & Tikhon Jelvis, Stanford). Key content extracted: MDP/Bellman/HJB theoretical framework (DP vs RL as two solutions to the same recursion); Merton's Portfolio Problem (dynamic asset allocation + consumption, CARA utility, discrete-time closed form); Derivatives Pricing (American option optimal exercise as MDP, incomplete-market hedging — same lineage as Halperin's QLBS); Order-Book Trading (linear-impact optimal execution closed form N*_t=R_t/(T-t), i.e. TWAP is exactly optimal not just a heuristic; full HJB derivation of the Avellaneda-Stoikov market-making model already referenced without derivation in market-microstructure.md). Key entities: Ashwin Rao (new person page), Tikhon Jelvis (co-author, mentioned on source page), Igor Halperin (cross-referenced — QLBS/SciPhyRL share this book's MDP lineage). Index page count: 296 → 299.
...
## [2026-08-09] lint | Health check

Issues found: 38. Auto-fixed: 3. Needs review: 35.

**Method**: Mechanical index/orphan/broken-link check done directly (grep link-extraction + Python relative-path resolution + `comm` diff across all 3 index files against all 311 wiki files). Semantic checks (stale content, contradictions, missing cross-references, thin topics) delegated to 3 parallel subagents scoped by directory: (1) trading/algorithms+data-sources+sources+prediction-markets+research-log, (2) trading/backtesting+strategies+paper-trading+tools, (3) everything outside trading/ (ai-industry, concepts, dr, orgs, people, sources, tools).

**Mechanical checks — clean:**
- Index gaps: 0. All 311 files resolve to a link from one of the 3 index.md files.
- Broken links: 0 real (1 false positive — `wiki/index.md` line 13's `[Page title](path.md)` is documentation template text describing the index's own entry format, not an actual link).
- True orphans (zero inbound links from anywhere, including indexes): 0.
- "Index-only" pages (cataloged in an index but not cross-linked from another content page's body): 156, mostly `trading/research-log/` daily entries (chronological journal, not meant to be heavily cross-linked) and `tools/*-notes.md` reference-only stubs — consistent with the wiki's existing design pattern, not treated as defects.

**Auto-fixed (mechanical, applied immediately):**
1. `tools/fireworks-tech-graph.md` — broken relative link `../trading/tools/quant-mind.md` (file moved/merged in a prior lint pass) corrected to `quant-mind-notes.md`.
2. `trading/prediction-markets/polymarket.md` — removed the false claim "Prediction Markets section... currently has no multi-agent-LLM-specific page" and added links to `algorithmic-strategies.md` and `ai-model-benchmarks.md`, which already cover PolySwarm.
3. `trading/tools/portfolio-optimization.md` ↔ `trading/paper-trading/performance-attribution.md` — added reciprocal cross-links; both cover Riskfolio-Lib/skfolio portfolio construction with no prior link between them.

**Needs review (flagged for Kevin, not silently resolved) — top priority first:**

1. **[Live bug, highest priority] H417/H418 look-ahead-bias correction never propagated to `trading/backtesting/hypothesis-log.md`.** The canonical log still shows H417 CONFIRMED (OOS 5.855) / H418 PARTIAL CONFIRMED (OOS 5.328), both dated 2026-07-20. `trading/research-log/2026-07-21.md` documents a look-ahead bug fix that corrected these to H417 NOT CONFIRMED (OOS 0.383) / H418 NOT CONFIRMED (best corrected OOS 1.565). `trading/algorithms/momentum-strategies.md` reflects the correction but the hypothesis-log entries were never fixed — and **H483** (2026-07-31, 10 days later) is built directly on the retracted H411 Var B 4.825 baseline.
2. **Production portfolio description stale in `trading/paper-trading/index.md` and `trading/paper-trading/h122-alpaca.md`** — both describe H149 ("100% H026 sector rotation," OOS Sharpe 3.007) as the live production strategy. The actual current production baseline used as the pass/fail gate throughout later hypothesis-log entries (H354, H398, H429, H434, H445...) is the H112 six-sleeve blend (H041a 22% / H026 27% / H045 21% / XLK IBS 20% / SMH IBS 8% / IGV IBS 2%, OOS Sharpe 4.158, MaxDD -3.60%), matching the operator's own standing notes.
3. **H026 OOS Sharpe/MaxDD stated 3 different ways**: `trading/algorithms/position-sizing.md` has two internally conflicting tables (Sharpe 2.665/MaxDD -5.7% vs. Sharpe 3.007/MaxDD -14.2%); `momentum-strategies.md`/root `index.md` cite Sharpe 3.007 with a third MaxDD, -9.6%.
4. **Numeric contradictions, unreconciled across files**: Kalshi fee structure (flat 7% formula in `kalshi.md` vs. tiered 7/5/3/1% by monthly volume in `other-platforms.md`); Kalshi base URL/auth scheme stale in `algorithmic-strategies.md`/`automated-pipeline.md` (old `trading-api.kalshi.com` + PKCS1v15) vs. current in `kalshi.md` (`api.elections.kalshi.com` + RSA-PSS); Alpaca rate limit given as 200/min, 200-1000/min, and 10,000/min across 3 files; IEX market-volume coverage given as ~2% and ~8-10% across 2 files; FMP API v3 marked "Blocked Aug 2025" in one file but documented as live, undeprecated in two younger files.
5. **Model-version staleness**: `ai-industry/ai-infrastructure-2026.md` and `sources/prediction-arena-2026.md` still describe the runtime model as Claude Sonnet/Opus 4.x; actual current model is Sonnet 5. `ai-industry/model-landscape-2026.md`'s own context-window table wasn't extended when its July addendum added Opus 5/Sonnet 5/Kimi K3/DeepSeek V4/Gemini 3.5.
6. **Duplicate/conflicting hypothesis-number assignment**: `ai-industry/llm-finance-benchmarks-2026.md` labels arXiv:2605.27848 as "H362" (which is actually a different, already-confirmed strategy); `ai-industry/llm-alpha-mining-systems-2026.md` labels the same paper "H383." Separately, `trading/algorithms/low-volatility-etf-rotation.md` labels an OB-filter hypothesis "H356" (OOS 2.312) while `low-volatility.md`/hypothesis-log call the same concept H361 (OOS 1.903) — no "H356" exists in the log.
7. **DR/backup docs internally inconsistent**: `dr/overview.md` states both "~237 pages" and "~284 pages" in the same file; `dr/runbook-2026.md` (same update date, marked CURRENT) says "209 pages as of 2026-07-16." Actual current count per `index.md` frontmatter: 299-311 depending on measurement time. `dr/overview.md` also states the dream-cycle scan runs "2:30 AM CT" while `dr/runbook-2026.md` (written to supersede it) and the task registry agree on ~11 PM CT. `dr/git-backup.md` still documents the old 2am push schedule and a credential-helper approach known not to work under the OneCLI proxy.
8. **Remaining findings** (thin/stub topics, additional missing cross-refs, minor staleness): H417/H418-adjacent Kalshi/FMP items aside, ~15 lower-priority items — full detail available on request, including: `calendar-anomalies.md` frontmatter/body mismatch on H205-H208 status; `nowcasting-playbook.md` says no prediction-market SDK exists though `pmxt` is documented elsewhere; `deep-rl-trading.md` (H204) unrun for a month while literature keeps accumulating; `options-data.md` provider-key status 3 months stale, silently blocking newer options-IV proposals; `llm-forecasting-methodology.md`'s PolySwarm redesign recommendation never applied back to `algorithmic-strategies.md`; several tool-notes stub pages (`openknowledge-ai-notes.md`, `visual-plan-skill.md`) over 6 weeks stale with no follow-up; thin org/person stub pages (`jpmorgan-social-finance.md`, `yasemin-saltuk.md` — latter also has a stale 2012-era personal phone/email, minor PII-retention note); inconsistent frontmatter schemas across ~65 non-trading pages, several with no date fields at all.

**Not fixed**: none of the above were auto-resolved — all involve picking which of several numbers/labels is correct, which is a judgment call for Kevin, not a mechanical fix.
...
## [2026-08-20] ingest | Alpha Architect — VIX Trend Following: Out of Sample
Pages created: 0. Pages updated: 2 (`trading/algorithms/market-timing-overlays.md` new §10; `trading/backtesting/hypothesis-log.md` new H524 entry). Index updated: 1 (`wiki/index.md`).
Kevin shared the article URL and asked to note + test it. Primary URL was Cloudflare-protected and could not be fetched directly (curl and agent-browser both blocked by JS challenge); sourced the 2017 original via a Yahoo Finance full-text syndication mirror, and the 2026 follow-up only via WebSearch snippets (flagged as lower-confidence sourcing in both wiki pages). Implemented and backtested the mechanism as H524 (VIX-regime-conditioned momentum lookback, FRED VIXCLS-driven, SPY/VXF/EFA/AGG universe) rather than relying on the article's own reported figures. Caught and fixed a look-ahead bias bug in the first draft (signal used the holding month's own closing price) before reporting any results — pre-fix OOS Sharpe was inflated to ~2.0, corrected to 0.673-0.843. Verdict: NOT CONFIRMED — all 4 variants (regime/static × Top1/Top2) fail the walk-forward stability gate (deeply negative worst-fold Sharpe), and the regime-conditioning effect itself is inconsistent in sign between Top1 and Top2.
