#!/usr/bin/env python3
"""
Dividend Strategy #1: Dividend Raise Signal Paper Trader
Signal: company raises dividend >= 10% → buy at ex-date close, hold 40 trading days.
...
PORTFOLIO_FILE = Path(__file__).parent / 'div_raise_portfolio.json'
VIRTUAL_CAPITAL = 5000.0
POSITION_SIZE   = 500.0    # $500 per trade
MAX_POSITIONS   = 10
HOLD_DAYS       = 40       # trading days
MIN_RAISE_PCT   = 0.10     # 10% minimum dividend raise
...
def load() -> dict:
    ...
    return {
        'strategy': 'Dividend Raise Signal (>=10%, hold 40d)',
        'backtest_sharpe': 4.403,
...
def detect_raise_events(ticker: str) -> list:
    """
    Download dividend history, detect raises >= 10%.
...
def run():
    ...
    print(f"\n=== Dividend Raise Signal Paper Trader — {today} ===")
